...
`,qi=class extends HTMLElement{shadow;inserter=new Nr;activeCatId=Ar[0].id;connectedCallback(){this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${Ki}</style>
<div class="dlg">
...
<p>표 삽입 → 삽입 메뉴 → 표를 사용해보세요.</p>`;document.getElementById(`btn-get-html`).addEventListener(`click`,()=>{Ms.style.display=`block`,Ms.textContent=$.getHTML()}),document.getElementById(`btn-set-html`).addEventListener(`click`,()=>{$.setHTML(Ns),Ms.style.display=`none`}),document.getElementById(`btn-clear`).addEventListener(`click`,()=>{$.setHTML(``),Ms.style.display=`none`}),document.getElementById(`btn-find`).addEventListener(`click`,()=>{$.querySelector(`.poa-editor-content`)?.focus(),$.dispatchEvent(new KeyboardEvent(`keydown`,{key:`f`,ctrlKey:!0,bubbles:!0}))}),document.getElementById(`btn-replace`).addEventListener(`click`,()=>{$.querySelector(`.poa-editor-content`)?.focus(),$.dispatchEvent(new KeyboardEvent(`keydown`,{key:`h`,ctrlKey:!0,bubbles:!0}))}),document.getElementById(`btn-undo`).addEventListener(`click`,()=>{$.querySelector(`.poa-editor-content`)?.focus(),document.execCommand(`undo`)}),document.getElementById(`btn-redo`).addEventListener(`click`,()=>{$.querySelector(`.poa-editor-content`)?.focus(),document.execCommand(`redo`)}),si()!==`user`&&setTimeout(()=>$.setHTML(Ns),100);