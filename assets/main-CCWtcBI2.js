...
`,fi=class extends HTMLElement{shadow;targetCell=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${di}</style>
...
`,mi=class extends HTMLElement{shadow;activeTab=`video`;constructor(){super(),this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${pi}</style>`}open(e=`video`){this.activeTab=e,this.setAttribute(`open`,``),this.render()}close(){this.removeAttribute(`open`)}render(){this.shadow.innerHTML=`<style>${pi}</style>
<div class="overlay" id="overlay">
  ...
  </svg>`},gi={text:`텍스트`,textarea:`여러 줄`,checkbox:`체크박스`,radio:`라디오`,select:`목록 선택`,button:`버튼`,date:`날짜 선택`},_i=[`text`,`textarea`,`checkbox`,`radio`,`select`,`button`,`date`],vi=`
...
`;function J(e,t,n=``){return`<div class="field">
  ...
  </div>`}function Y(e,t=``,n=`text`){return`<input type="${n}" id="${e}" placeholder="${t}">`}function yi(e,t){return`<select id="${e}">${t.map(([e,t])=>`<option value="${e}">${t}</option>`).join(``)}</select>`}var bi=class extends HTMLElement{shadow;activeType=`text`;connectedCallback(){this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${vi}</style>${this._tpl()}`,this._buildTypeCards(),this._bindEvents(),this._setType(`text`)}open(e){let t=this.shadow.getElementById(`dlg-title`),n=this.shadow.getElementById(`btn-confirm`);t.textContent=e?`입력 요소 편집`:`입력 요소 추가`,n.textContent=e?`수정하기`:`추가하기`,e?(this._setType(e.type),this._fillFields(e)):(this._setType(`text`),this._clearFields()),this.setAttribute(`open`,``),this.shadow.getElementById(`f-label`)?.focus()}close(){this.removeAttribute(`open`)}_tpl(){return`<div class="dlg">
  <div class="hdr">
    ...
    <div class="adv-body" data-adv-body>${e}</div>`}_secText(){return`<div class="specific-section" id="sec-text">
      ...
      ${this._adv(`
        <div class="field">
          ...
          ${yi(`f-btn-style`,[[`default`,`기본 (회색)`],[`primary`,`강조 (파랑)`],[`danger`,`경고 (빨강)`]])}
        </div>
...
`,Bi=class extends HTMLElement{shadow;inserter=new jr;activeCatId=Or[0].id;connectedCallback(){this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${zi}</style>
<div class="dlg">
...
.hidden { display: none !important; }
`,Hi=class extends HTMLElement{shadow;mode=`add`;editId=null;entries=[];connectedCallback(){this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${Vi}</style>
<div class="dlg">
  <div class="hdr">
...
`,Wi=class extends HTMLElement{shadow;targetInput=null;connectedCallback(){this.shadow||(this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${Ui}</style>${this._tpl()}`,this._bindEvents())}open(e){this.shadow||this.connectedCallback(),this.targetInput=e,this._populate(e),this.setAttribute(`open`,``)}close(){this.removeAttribute(`open`)}_tpl(){return`<div class="dlg">
  <div class="hdr">
    <h3>입력 요소 속성</h3>
    <button class="x-btn" aria-label="닫기">✕</button>
  </div>
...
`,qi=class extends HTMLElement{shadow;_activeTab=`edit`;busHandler=({tab:e})=>{this._activeTab=e,this.updateActive()};constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){let e=Gi.map(e=>`<button class="tab${e.id===this._activeTab?` active`:``}" data-tab="${e.id}">${e.label}</button>`).join(``);this.shadow.innerHTML=`<style>${Ki}</style><div class="menubar">${e}</div>`,this.shadow.querySelector(`.menubar`).addEventListener(`mousedown`,e=>{e.preventDefault();let t=e.target.dataset.tab;t&&N.emit(P.MENUBAR_CHANGE,{tab:t})}),N.on(P.MENUBAR_CHANGE,this.busHandler)}disconnectedCallback(){N.off(P.MENUBAR_CHANGE,this.busHandler)}updateActive(){this.shadow.querySelectorAll(`.tab`).forEach(e=>{e.classList.toggle(`active`,e.dataset.tab===this._activeTab)})}},Ji=`
<svg
...
`;function Z(e){return e.replace(/width="24"/,`width="16"`).replace(/height="24"/,`height="16"`)}var Q={bold:Z(ta),italic:Z(ba),underline:Z(lo),strike:Z(Xa),undo:Z(uo),redo:Z(Ra),alignLeft:Z(oo),alignCenter:Z(ro),alignRight:Z(io),alignJustify:Z(ao),indent:Z(Da),outdent:Z(Ea),foreColor:Z($i),backColor:Z(ga),lineHeight:Z(ea),letterSpacing:Z(Na),fileNew:Z(fa),fileOpen:Z(ma),save:Z(Ha),print:Z(La),settings:Z(Ga),clock:Z(sa),cut:Z(Ua),copy:Z(ua),paste:Z(oa),pastePlain:Z(aa),selectAll:Z(Ya),search:Z(Wa),image:Z(_a),images:Z(va),link:Z(Ta),linkOff:Z(wa),bookmark:Z(ra),calendar:Z(ia),hr:Z(ja),symbol:Z(Pa),video:Z(fo),youtube:Z(Ma),viewDesign:Z(Sa),viewHtml:Z(ca),viewPreview:Z(da),viewText:Z(co),viewPage:Z(pa),fullscreen:Z(Aa),ruler:Z(Va),grid:Z(ha),hiddenBorder:Z(Ya),table:Z(no),tableProps:Z(to),cellProps:Z(Ca),mergeCells:Z($a),splitCell:Z(eo),rowAbove:Z(Qi),rowBelow:Z(Yi),colLeft:Z(Xi),colRight:Z(Zi),rowDelete:Z(Ba),colDelete:Z(la),tableDelete:Z(so),painterCopy:Z(Fa),painterPaste:Z(Ia),formatClear:Z(za),ul:Z(ka),ol:Z(Oa),sup:Z(Qa),sub:Z(Za),a11y:Z(Ji),privacy:Z(Ka),form:Z(Ja),calc:Z(qa),shortcuts:Z(xa),guide:Z(na),about:Z(ya)},po={"file:new":`fileNew`,"file:open":`fileOpen`,"file:save":`save`,"file:saveas":`save`,"file:print":`print`,settings:`settings`,"edit:cut":`cut`,"edit:copy":`copy`,"edit:paste":`paste`,"edit:paste-plain":`pastePlain`,"edit:select-all":`selectAll`,"find-replace":`search`,"edit:image-edit":`image`,image:`image`,"insert:multi-image":`images`,"insert:link":`link`,"insert:bookmark":`bookmark`,"insert:datetime":`calendar`,"insert:hr":`hr`,"insert:symbol":`symbol`,"view:design":`viewDesign`,"view:html":`viewHtml`,"view:preview":`viewPreview`,"view:text":`viewText`,"view:page":`viewPage`,"view:fullscreen":`fullscreen`,"view:ruler":`ruler`,"view:grid":`grid`,"view:hidden-border":`hiddenBorder`,table:`table`,"table:table-props":`tableProps`,"table:cell-props":`cellProps`,"table:merge":`mergeCells`,"table:split-cell":`splitCell`,"table:row-above":`rowAbove`,"table:row-below":`rowBelow`,"table:col-left":`colLeft`,"table:col-right":`colRight`,"table:row-delete":`rowDelete`,"table:col-delete":`colDelete`,"table:delete":`tableDelete`,"format:painter-copy":`painterCopy`,"format:painter-paste":`painterPaste`,"format:clear":`formatClear`,"format:ul":`ul`,"format:ol":`ol`,"format:sup":`sup`,"format:sub":`sub`,"misc:a11y":`a11y`,"misc:privacy":`privacy`,"misc:form":`form`,"misc:calc":`calc`,"help:shortcuts":`shortcuts`,"help:guide":`guide`,"help:about":`about`},mo={text:`T`,textarea:`☰`,number:`#`,date:`📅`},ho={file:[[[`새 문서`,`file:new`],[`열기`,`file:open`],[`저장`,`file:save`],[`다른 이름으로 저장`,`file:saveas`]],[[`인쇄`,`file:print`],[`환경설정`,`settings`]]],edit:[[[`잘라내기`,`edit:cut`],[`복사`,`edit:copy`],[`붙여넣기`,`edit:paste`],[`서식 없이 붙여넣기`,`edit:paste-plain`]],[[`찾기·바꾸기`,`find-replace`],[`이미지 편집`,`edit:image-edit`],[`전체 선택`,`edit:select-all`]]],insert:[[[`이미지`,`image`]],[[`하이퍼링크`,`insert:link`],[`책갈피`,`insert:bookmark`]],[[`서명`,`insert:signature`],[`이모지`,`insert:emoji`]],[[`툴팁`,`insert:tooltip`],[`툴팁 관리`,`insert:tooltip-list`]],[[`날짜·시간`,`insert:datetime`],[`가로줄`,`insert:hr`],[`기호`,`insert:symbol`]],[{dropdown:!0,id:`doc-field`,label:`양식 필드`,items:$r.map(e=>[e.label,`insert:field`,e.id,mo[e.type]])}]],view:[[[`디자인`,`view:design`],[`HTML`,`view:html`],[`미리보기`,`view:preview`],[`텍스트`,`view:text`],[`페이지`,`view:page`]],[[`전체화면`,`view:fullscreen`],[`눈금자`,`view:ruler`],[`그리드`,`view:grid`],[`숨김 테두리`,`view:hidden-border`]]],table:[[[`표 삽입`,`table`],[`표 속성`,`table:table-props`],[`셀 속성`,`table:cell-props`]],[[`셀 병합`,`table:merge`],[`셀 나누기`,`table:split-cell`]],[[`위에 행 삽입`,`table:row-above`],[`아래에 행 삽입`,`table:row-below`],[`왼쪽에 열 삽입`,`table:col-left`],[`오른쪽에 열 삽입`,`table:col-right`]],[[`행 삭제`,`table:row-delete`],[`열 삭제`,`table:col-delete`],[`표 삭제`,`table:delete`]]],format:[[[`서식 복사`,`format:painter-copy`],[`서식 붙여넣기`,`format:painter-paste`],[`서식 제거`,`format:clear`]],[[`글머리 기호`,`format:ul`],[`글머리 번호`,`format:ol`]],[[`위 첨자`,`format:sup`],[`아래 첨자`,`format:sub`]]],misc:[[[`웹 접근성 체크`,`misc:a11y`],[`개인정보 체크`,`misc:privacy`]],[[`폼 컨트롤`,`misc:form`],[`계산식`,`misc:calc`],[`템플릿`,`misc:template`]]],help:[[[`단축키`,`help:shortcuts`],[`사용자 가이드`,`help:guide`],[`제품 정보`,`help:about`]]]},go=new Set([`view:design`,`view:html`,`view:preview`,`view:text`,`view:page`]),_o=`
:host {
  display: block;
  --icon-color:         #374151;
...
</div>`;let n=this.shadow.getElementById(`paper-select`),r=this.shadow.getElementById(`zoom-select`),i=this.shadow.getElementById(`zoom-in`),a=this.shadow.getElementById(`zoom-out`);n.value=`A4`,r.value=`100`,n.addEventListener(`change`,()=>{this.dispatchAction(`paper:size`,n.value)}),r.addEventListener(`change`,()=>{this.dispatchAction(`paper:zoom`,r.value)}),i.addEventListener(`click`,()=>this.dispatchAction(`paper:zoom-in`)),a.addEventListener(`click`,()=>this.dispatchAction(`paper:zoom-out`))}update(e){let t=this.shadow.getElementById(`char-count`),n=this.shadow.getElementById(`word-count`);if(!t||!n)return;let r=new DOMParser().parseFromString(e,`text/html`).body.textContent??``,i=[...r.replace(/\s/g,``)].length,a=r.trim()===``?0:r.trim().split(/\s+/).length;t.textContent=`${i}자`,n.textContent=`${a}단어`}syncPaper(e){let{size:t,zoom:n}=e.detail,r=this.shadow.getElementById(`paper-select`),i=this.shadow.getElementById(`zoom-select`);r&&(r.value=t.id),i&&(i.value=String(n))}dispatchAction(e,t){this.dispatchEvent(new CustomEvent(`poa-action`,{bubbles:!0,composed:!0,detail:{type:e,...t===void 0?{}:{value:t}}}))}},Do=`
:host { display: none; }
:host([open]) { display: block; }
.backdrop {
  position: fixed; inset: 0;
...
`;function Oo(e){return new Date(e).toLocaleString(`ko-KR`,{month:`2-digit`,day:`2-digit`,hour:`2-digit`,minute:`2-digit`,second:`2-digit`})}var ko=class extends HTMLElement{shadow;autoSave=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Do}</style>
  ...
  <div class="dialog" role="dialog" aria-modal="true" aria-label="설정">
    ...
    <div class="body">
      ...
      <div class="section">
        ...
        <span class="history-time">${Oo(n.savedAt)}</span>
        <button class="restore-btn" data-id="${String(n.id)}">복원</button>`,t.querySelector(`.restore-btn`).addEventListener(`click`,()=>{this.handleRestore(n)}),e.appendChild(t)}}handleRestore(e){this.dispatchEvent(new CustomEvent(`poa-autosave-restore`,{bubbles:!0,composed:!0,detail:{html:e.html}})),this.close()}},Ao=`
/* ── 호스트 슬라이드 애니메이션 ── */
:host {
  display: block;
  overflow: hidden;
...
`,Po=class extends HTMLElement{shadow;editor=new si;originalSrc=``;currentDataUrl=``;busy=!1;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${No}</style>
<div class="backdrop" id="backdrop">
...
`;function Io(e){return e<1024*1024?`${(e/1024).toFixed(1)}KB`:`${(e/1024/1024).toFixed(1)}MB`}var Lo=class extends HTMLElement{shadow;uploader=new ui;uploadConfig=null;selectedFiles=[];busy=!1;onErrorFn=null;setOnError(e){this.onErrorFn=e}constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Fo}</style>
<div class="backdrop" id="backdrop">
...
.grid-picker {
  display: grid;
  grid-template-columns: repeat(${Ro}, ${Bo}px);
  gap: ${Vo}px;
  cursor: crosshair;
}
.gc {
  width: ${Bo}px; height: ${Bo}px;
...
.btn:not(.btn-primary):hover { background: #f5f5f5; }
`,Uo=class e extends HTMLElement{shadow;selectedPreset=`border-all`;gridCells=[];constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Ho}</style>
<div class="overlay" id="overlay">
  <div class="dialog" role="dialog" aria-modal="true">
    <div class="dlg-hd">
...
</div>`,t.querySelector(`#pp-al`).value=s,t.querySelector(`#pp-cancel`).addEventListener(`click`,()=>this.close()),t.querySelector(`#pp-ok`).addEventListener(`click`,()=>{let e={width:t.querySelector(`#pp-w`).value.trim(),borderColor:t.querySelector(`#pp-bc`).value,bgColor:t.querySelector(`#pp-bg`).value,align:t.querySelector(`#pp-al`).value};this.dispatchEvent(new CustomEvent(`poa-table-update`,{bubbles:!0,composed:!0,detail:{options:e,table:n}})),this.close()})}static rgbToHex(e){let t=e.match(/\d+/g);return!t||t.length<3?``:`#`+t.slice(0,3).map(e=>parseInt(e).toString(16).padStart(2,`0`)).join(``)}static colorToHex(t){return e.rgbToHex(t)||`#ffffff`}},Wo=[{label:`날짜 (2025-05-05)`,fn:()=>new Date().toISOString().slice(0,10)},{label:`시간 (14:30:05)`,fn:()=>new Intl.DateTimeFormat(`ko-KR`,{hour:`2-digit`,minute:`2-digit`,second:`2-digit`,hour12:!1}).format(new Date)},{label:`날짜·시간 (2025-05-05 14:30)`,fn:()=>{let e=new Date;return`${e.toISOString().slice(0,10)} ${new Intl.DateTimeFormat(`ko-KR`,{hour:`2-digit`,minute:`2-digit`,hour12:!1}).format(e)}`}},{label:`한국어 날짜 (2025년 5월 5일)`,fn:()=>new Intl.DateTimeFormat(`ko-KR`,{year:`numeric`,month:`long`,day:`numeric`}).format(new Date)},{label:`한국어 날짜·시간 (2025년 5월 5일 월요일 오후 2:30)`,fn:()=>new Intl.DateTimeFormat(`ko-KR`,{year:`numeric`,month:`long`,day:`numeric`,weekday:`long`,hour:`numeric`,minute:`2-digit`}).format(new Date)}],Go=`
:host { display: none; }
:host([open]) { display: block; }
.backdrop {
  position: fixed; inset: 0; background: rgba(0,0,0,.45);
...
`,Ko=class extends HTMLElement{shadow;bookmarks=[];editingId=null;editingAnchor=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Go}</style>
<div class="backdrop" id="backdrop">
  <div class="dialog" role="dialog" aria-modal="true" aria-label="링크 삽입">
    ...
    `).join(``),this.refreshDatetimePreviews(),e.addEventListener(`click`,e=>{let t=e.target.closest(`[data-preset]`);if(!t)return;let n=Wo[parseInt(t.dataset.preset??`0`)]?.fn()??``;this.dispatch(`poa-datetime-insert`,{text:n}),this.close()}))}refreshDatetimePreviews(){Wo.forEach((e,t)=>{let n=this.shadow.getElementById(`dt-prev-${t}`);n&&(n.textContent=e.fn())})}dispatch(e,t){this.dispatchEvent(new CustomEvent(e,{bubbles:!0,composed:!0,detail:t}))}},qo=`
:host { display: none; }
:host([open]) {
  display: flex; align-items: center; gap: 6px;
...
`,Jo=class extends HTMLElement{shadow;img=null;aspectLocked=!1;naturalW=0;naturalH=0;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${qo}</style>
<label>너비</label>
...
`,Zo=class extends HTMLElement{shadow;resolvePromise=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Yo}</style>${Xo}`,this.bindEvents()}show(e,t=`확인`){let n=this.shadow.getElementById(`dlg-title`),r=this.shadow.getElementById(`dlg-message`);return n&&(n.textContent=t),r&&(r.textContent=e),this.setAttribute(`open`,``),new Promise(e=>{this.resolvePromise=e})}close(e){this.removeAttribute(`open`),this.resolvePromise?.(e),this.resolvePromise=null}bindEvents(){this.shadow.getElementById(`btn-ok`)?.addEventListener(`click`,()=>this.close(!0)),this.shadow.getElementById(`btn-cancel`)?.addEventListener(`click`,()=>this.close(!1)),this.shadow.getElementById(`overlay`)?.addEventListener(`click`,e=>{e.target.id===`overlay`&&this.close(!1)}),this.addEventListener(`keydown`,e=>{e.key===`Escape`&&(e.preventDefault(),this.close(!1))})}},Qo={error:`●`,warning:`▲`,info:`ℹ`},$o={error:`#DC2626`,warning:`#D97706`,info:`#2563EB`},es=`
:host { display: none; }
:host([open]) { display: block; }
...
`,ts=class extends HTMLElement{shadow;issues=[];contentEl=null;rerunFn=null;currentFilter=`all`;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${es}</style>
<div class="overlay" id="overlay">
  <div class="dialog">
    <div class="header">
      <h2>웹 접근성 검사 결과</h2>
...
<div class="card" data-level="${e.riskLevel}" data-id="${e.id}">
  <div class="card-header" data-toggle="${e.id}">
    <span class="card-icon">${rs[e.riskLevel]}</span>
    <span class="card-label">${e.label}</span>
    <span class="card-risk">${is[e.riskLevel]}</span>
    <span class="card-toggle">∨</span>
  </div>
  <div class="card-body" id="body-${e.id}">
    <div class="raw-text">"${ss(e.raw)}"</div>
...
`,ls=class extends HTMLElement{shadow;currentTable=null;currentRow=1;currentCol=1;pickBar=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${cs}</style>`}open(e){this.currentTable=e.table,this.currentRow=e.cellRow,this.currentCol=e.cellCol,this.setAttribute(`open`,``),this.render(e.existingFormula,e.initialRange)}close(){this.removeAttribute(`open`),this.exitPickMode(),this.currentTable=null}applyRange(e,t,n,r){this.exitPickMode();let i=e=>this.shadow.getElementById(e),a=(e,t)=>{let n=i(e);n&&(n.value=String(t))};a(`sr`,e),a(`sc`,t),a(`er`,n),a(`ec`,r)}render(e,t){let n=e?.fn??`SUM`,r=e?.format??`integer`,i=e?.decimalPlaces??2,[a,o,s,c]=e?.range??t??[1,1,1,1],l=e?.targetRow??this.currentRow,u=e?.targetCol??this.currentCol,d=!e||e.targetRow===this.currentRow&&e.targetCol===this.currentCol,f=e?.style?.backgroundColor??`#ffffff`,p=e?.style?.color??`#000000`,m=(e,t)=>`<label><input type="radio" name="fn" value="${e}"${n===e?` checked`:``}> ${e} (${t})</label>`,h=(e,t)=>`<label><input type="radio" name="format" value="${e}"${r===e?` checked`:``}> ${t}</label>`;this.shadow.innerHTML=`<style>${cs}</style>
<div class="overlay" id="overlay">
  <div class="dialog" id="dialog">
    <div class="header">
      <h2>계산식 설정</h2>
    ...
    <div class="footer">
      ...
      ">취소</button>`,document.body.appendChild(e),this.pickBar=e,e.querySelector(`#cancel-pick`)?.addEventListener(`click`,()=>this.exitPickMode()),this.dispatchEvent(new CustomEvent(`poa-formula-start-pick`,{bubbles:!0}))}exitPickMode(){this.shadow.getElementById(`overlay`)?.classList.remove(`pick-mode`),this.pickBar?.remove(),this.pickBar=null}};customElements.get(`poa-menubar`)||customElements.define(`poa-menubar`,qi),customElements.get(`poa-context-toolbar`)||customElements.define(`poa-context-toolbar`,vo),customElements.get(`poa-toolbar`)||customElements.define(`poa-toolbar`,To),customElements.get(`poa-status-bar`)||customElements.define(`poa-status-bar`,Eo),customElements.get(`poa-settings-dialog`)||customElements.define(`poa-settings-dialog`,ko),customElements.get(`poa-find-replace-dialog`)||customElements.define(`poa-find-replace-dialog`,Mo),customElements.get(`poa-image-edit-dialog`)||customElements.define(`poa-image-edit-dialog`,Po),customElements.get(`poa-image-dialog`)||customElements.define(`poa-image-dialog`,Lo),customElements.get(`poa-table-dialog`)||customElements.define(`poa-table-dialog`,Uo),customElements.get(`poa-cell-split-dialog`)||customElements.define(`poa-cell-split-dialog`,fi),customElements.get(`poa-link-dialog`)||customElements.define(`poa-link-dialog`,Ko),customElements.get(`poa-image-toolbar`)||customElements.define(`poa-image-toolbar`,Jo),customElements.get(`poa-confirm-dialog`)||customElements.define(`poa-confirm-dialog`,Zo),customElements.get(`poa-accessibility-dialog`)||customElements.define(`poa-accessibility-dialog`,ts),customElements.get(`poa-privacy-dialog`)||customElements.define(`poa-privacy-dialog`,os),customElements.get(`poa-formula-dialog`)||customElements.define(`poa-formula-dialog`,ls),customElements.get(`poa-video-dialog`)||customElements.define(`poa-video-dialog`,mi),customElements.get(`poa-form-control-dialog`)||customElements.define(`poa-form-control-dialog`,bi),customElements.get(`poa-template-tree`)||customElements.define(`poa-template-tree`,Ei),customElements.get(`poa-template-dialog`)||customElements.define(`poa-template-dialog`,ki),customElements.get(`poa-signature-dialog`)||customElements.define(`poa-signature-dialog`,Ri),customElements.get(`poa-emoji-dialog`)||customElements.define(`poa-emoji-dialog`,Bi),customElements.get(`poa-tooltip-dialog`)||customElements.define(`poa-tooltip-dialog`,Hi),customElements.get(`poa-input-property-dialog`)||customElements.define(`poa-input-property-dialog`,Wi),customElements.get(`poa-editor`)||customElements.define(`poa-editor`,oi);var $=document.getElementById(`editor`),us=document.getElementById(`output`),ds=`<h2>poa-editor v1.0.0</h2>
...
<p>표 삽입 → 삽입 메뉴 → 표를 사용해보세요.</p>`;document.getElementById(`btn-get-html`).addEventListener(`click`,()=>{us.style.display=`block`,us.textContent=$.getHTML()}),document.getElementById(`btn-set-html`).addEventListener(`click`,()=>{$.setHTML(ds),us.style.display=`none`}),document.getElementById(`btn-clear`).addEventListener(`click`,()=>{$.setHTML(``),us.style.display=`none`}),document.getElementById(`btn-find`).addEventListener(`click`,()=>{$.querySelector(`.poa-editor-content`)?.focus(),$.dispatchEvent(new KeyboardEvent(`keydown`,{key:`f`,ctrlKey:!0,bubbles:!0}))}),document.getElementById(`btn-replace`).addEventListener(`click`,()=>{$.querySelector(`.poa-editor-content`)?.focus(),$.dispatchEvent(new KeyboardEvent(`keydown`,{key:`h`,ctrlKey:!0,bubbles:!0}))}),document.getElementById(`btn-undo`).addEventListener(`click`,()=>{$.querySelector(`.poa-editor-content`)?.focus(),document.execCommand(`undo`)}),document.getElementById(`btn-redo`).addEventListener(`click`,()=>{$.querySelector(`.poa-editor-content`)?.focus(),document.execCommand(`redo`)}),setTimeout(()=>$.setHTML(ds),100);