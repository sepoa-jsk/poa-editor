...
`,ji=class extends HTMLElement{shadow;targetCell=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Ai}</style>
...
`,Ni=class extends HTMLElement{shadow;activeTab=`video`;constructor(){super(),this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${Mi}</style>`}open(e=`video`){this.activeTab=e,this.setAttribute(`open`,``),this.render()}close(){this.removeAttribute(`open`)}render(){this.shadow.innerHTML=`<style>${Mi}</style>
<div class="overlay" id="overlay">
  ...
  </svg>`},Fi={text:`텍스트`,textarea:`여러 줄`,checkbox:`체크박스`,radio:`라디오`,select:`목록 선택`,button:`버튼`,date:`날짜 선택`},Ii=[`text`,`textarea`,`checkbox`,`radio`,`select`,`button`,`date`],Li=`
...
`;function Y(e,t,n=``){return`<div class="field">
  ...
  </div>`}function X(e,t=``,n=`text`){return`<input type="${n}" id="${e}" placeholder="${t}">`}function Ri(e,t){return`<select id="${e}">${t.map(([e,t])=>`<option value="${e}">${t}</option>`).join(``)}</select>`}var zi=class extends HTMLElement{shadow;activeType=`text`;connectedCallback(){this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${Li}</style>${this._tpl()}`,this._buildTypeCards(),this._bindEvents(),this._setType(`text`)}open(e){let t=this.shadow.getElementById(`dlg-title`),n=this.shadow.getElementById(`btn-confirm`);t.textContent=e?`입력 요소 편집`:`입력 요소 추가`,n.textContent=e?`수정하기`:`추가하기`,e?(this._setType(e.type),this._fillFields(e)):(this._setType(`text`),this._clearFields()),this.setAttribute(`open`,``),this.shadow.getElementById(`f-label`)?.focus()}close(){this.removeAttribute(`open`)}_tpl(){return`<div class="dlg">
  <div class="hdr">
    ...
    <div class="adv-body" data-adv-body>${e}</div>`}_secText(){return`<div class="specific-section" id="sec-text">
      ...
      ${this._adv(`
        <div class="field">
          ...
          ${Ri(`f-btn-style`,[[`default`,`기본 (회색)`],[`primary`,`강조 (파랑)`],[`danger`,`경고 (빨강)`]])}
        </div>
...
`,Wi=class extends HTMLElement{shadow;mgr;tree;selected=null;_getEditorHTML=null;connectedCallback(){this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${Hi}</style>${Ui}`,this.mgr=new xi,this.tree=this.shadow.querySelector(`poa-template-tree`),this.tree.setManager(this.mgr),this._bind()}setup(e){this._getEditorHTML=e}open(){this.selected=null,this._refreshPreview(),this._fillFolderSelect(),this.tree.render(),this.setAttribute(`open`,``)}close(){this.removeAttribute(`open`)}_bind(){this.shadow.querySelector(`.x-btn`).addEventListener(`click`,()=>this.close()),this.addEventListener(`click`,e=>{e.composedPath()[0]===this&&this.close()}),this.addEventListener(`keydown`,e=>{e.key===`Escape`&&this.close()}),this.shadow.getElementById(`btn-add-folder`).addEventListener(`click`,()=>{let e=this.tree.getSelected(),t=e?.type===`folder`?e.id:null;this.tree.addFolder(t)}),this.shadow.getElementById(`btn-save-tmpl`).addEventListener(`click`,()=>{this._fillFolderSelect(),this.shadow.getElementById(`save-name`).value=``,this.shadow.getElementById(`save-overlay`).classList.remove(`hidden`),setTimeout(()=>this.shadow.getElementById(`save-name`).focus(),50)}),this.shadow.getElementById(`save-cancel`).addEventListener(`click`,()=>{this.shadow.getElementById(`save-overlay`).classList.add(`hidden`)}),this.shadow.getElementById(`save-ok`).addEventListener(`click`,()=>this._doSave()),this.shadow.getElementById(`save-name`).addEventListener(`keydown`,e=>{e.key===`Enter`&&this._doSave()}),this.shadow.getElementById(`btn-append`).addEventListener(`click`,()=>this._apply(`append`)),this.shadow.getElementById(`btn-replace`).addEventListener(`click`,()=>this._apply(`replace`)),this.shadow.addEventListener(`poa-tmpl-copy-link`,()=>{this._showToast(`링크가 복사되었습니다.`)}),this.shadow.addEventListener(`poa-tmpl-select`,e=>{let{node:t}=e.detail;this.selected=t.type===`template`?t:null,this._refreshPreview()}),this.shadow.addEventListener(`poa-tmpl-dblclick`,e=>{let{node:t}=e.detail;this.selected=t,this._apply(`replace`)})}_refreshPreview(){let e=this.shadow.getElementById(`preview-frame`),t=this.shadow.getElementById(`preview-empty`),n=this.shadow.getElementById(`btn-append`),r=this.shadow.getElementById(`btn-replace`),i=this.shadow.getElementById(`selected-name`);if(!this.selected||this.selected.type!==`template`){e.classList.add(`hidden`),t.classList.remove(`hidden`),n.disabled=!0,r.disabled=!0,i.textContent=``,i.classList.remove(`visible`);return}i.textContent=this.selected.name,i.classList.add(`visible`),e.srcdoc=`<!doctype html><html><head><style>
       ...
       </style></head><body>${String(M.sanitize(this.selected.content??``,{USE_PROFILES:{html:!0}}))}</body></html>`,e.classList.remove(`hidden`),t.classList.add(`hidden`),n.disabled=!1,r.disabled=!1}_apply(e){if(!this.selected?.content)return;let t=String(M.sanitize(this.selected.content,{USE_PROFILES:{html:!0}}));this.dispatchEvent(new CustomEvent(`poa-tmpl-insert`,{bubbles:!0,composed:!0,detail:{html:t,mode:e}})),this.close()}_doSave(){let e=this.shadow.getElementById(`save-name`).value.trim();if(!e){this.shadow.getElementById(`save-name`).focus();return}let t=this.shadow.getElementById(`save-folder`).value||null,n=this.shadow.getElementById(`save-public`).checked,r=this._getEditorHTML?.()??``;this.mgr.addTemplate(e,r,t,n),this.tree.render(),this.shadow.getElementById(`save-overlay`).classList.add(`hidden`)}_showToast(e){let t=document.createElement(`div`);t.className=`tmpl-toast`,t.textContent=e,this.shadow.querySelector(`.dlg`).appendChild(t),setTimeout(()=>t.remove(),2200)}_fillFolderSelect(){let e=this.shadow.getElementById(`save-folder`);e.innerHTML=``;let t=this.mgr.getFolders().find(e=>!e.isPublic);for(let n of this.mgr.getFolders()){let r=document.createElement(`option`);r.value=n.id,r.textContent=n.name,n===t&&(r.selected=!0),e.appendChild(r)}}},Gi=`poa-signatures`,Ki=10;function qi(){return`${Date.now().toString(36)}-${Math.random().toString(36).slice(2,7)}`}function Z(e){return e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}function Ji(e){let{layout:t,fields:n,logo:r,headerColor:i=`#2563eb`}=e,a=n.displayName?`<strong style="font-size:15px;color:#111;">${Z(n.displayName)}</strong>`:``,o=[n.title,n.department].filter(Boolean).map(Z).join(` | `),s=n.company?`<span style="color:#555;">${Z(n.company)}</span>`:``,c=n.phone?`<span style="color:#888;font-size:12px;">T.</span>&nbsp;${Z(n.phone)}`:``,l=n.email?`<span style="color:#888;font-size:12px;">E.</span>&nbsp;${Z(n.email)}`:``,u=n.website?`<span style="color:#888;font-size:12px;">W.</span>&nbsp;${Z(n.website)}`:``,d=[o?`<span style="color:#555;">${o}</span>`:``,s,c,l,u].filter(Boolean),f=r?`<img src="${Z(r)}" alt="" style="max-width:80px;max-height:60px;display:block;">`:``,p=`font-size:14px;line-height:1.6;border-collapse:collapse;font-family:Arial,Helvetica,sans-serif;`,m=()=>[a,...d].filter(Boolean).join(`<br>`);switch(t){case 1:return`<table class="poa-signature" style="${p}"><tbody><tr><td style="padding:8px 0;vertical-align:top;">${m()}</td></tr></tbody></table>`;case 2:return r?`<table class="poa-signature" style="${p}"><tbody><tr><td style="padding:0 14px 0 0;vertical-align:middle;border-right:2px solid #e5e7eb;">${f}</td><td style="padding:0 0 0 14px;vertical-align:middle;">${m()}</td></tr></tbody></table>`:Ji({...e,layout:1});case 3:return r?`<table class="poa-signature" style="${p}"><tbody><tr><td style="text-align:center;padding-bottom:6px;">${f}</td></tr><tr><td style="padding:4px 0;">${m()}</td></tr></tbody></table>`:Ji({...e,layout:1});case 4:return`<table class="poa-signature" style="${p}"><tbody><tr><td style="padding:8px 0;vertical-align:top;">${a?a+`<br>`:``}<hr style="border:none;border-top:1px solid #aaa;margin:4px 0;">${d.join(`<br>`)}</td></tr></tbody></table>`;case 5:{let e=[a,n.department?`<span style="color:#555;">${Z(n.department)}</span>`:``].filter(Boolean).join(`<br>`),t=[n.title?`<span style="color:#555;">${Z(n.title)}</span>`:``,s].filter(Boolean).join(`<br>`),r=[c,l].filter(Boolean).join(`&nbsp;|&nbsp;`);return`<table class="poa-signature" style="${p}"><tbody><tr><td style="padding:4px 14px 4px 0;border-right:1px solid #ddd;vertical-align:top;">${e||`&nbsp;`}</td><td style="padding:4px 0 4px 14px;vertical-align:top;">${t||`&nbsp;`}</td></tr>`+(r?`<tr><td colspan="2" style="padding:6px 0 4px;border-top:1px solid #eee;font-size:13px;">${r}</td></tr>`:``)+`</tbody></table>`}case 6:{let e=[n.displayName?`<strong style="color:#fff;">${Z(n.displayName)}</strong>`:``,n.title?`<span style="color:rgba(255,255,255,.85);">${Z(n.title)}</span>`:``].filter(Boolean).join(`&nbsp;&nbsp;`),t=[n.department?`<span style="color:#555;">${Z(n.department)}</span>`:``,s,c,l,u].filter(Boolean);return`<table class="poa-signature" style="${p}border:1px solid ${Z(i)};"><tbody><tr><td style="background:${Z(i)};padding:8px 16px;">${e||`&nbsp;`}</td></tr><tr><td style="padding:8px 16px;">${t.join(`<br>`)||`&nbsp;`}</td></tr></tbody></table>`}default:return``}}var Yi=class{sigs=[];constructor(){this._load()}_load(){try{let e=localStorage.getItem(Gi);this.sigs=e?JSON.parse(e):[]}catch{this.sigs=[]}}_persist(){try{localStorage.setItem(Gi,JSON.stringify(this.sigs))}catch{}}getAll(){return[...this.sigs]}getById(e){return this.sigs.find(t=>t.id===e)??null}add(e){if(this.sigs.length>=Ki)throw Error(`최대 ${Ki}개까지 저장할 수 있습니다.`);let t={...e,id:qi(),createdAt:Date.now(),updatedAt:Date.now()};return this.sigs.push(t),this._persist(),t}update(e,t){let n=this.sigs.find(t=>t.id===e);return n?(Object.assign(n,t,{updatedAt:Date.now()}),this._persist(),!0):!1}delete(e){let t=this.sigs.findIndex(t=>t.id===e);return t<0?!1:(this.sigs.splice(t,1),this._persist(),!0)}isFull(){return this.sigs.length>=Ki}},Xi={1:`기본형`,2:`로고(좌)`,3:`로고(상)`,4:`구분선형`,5:`2단형`,6:`컬러헤더`},Zi=`
*, *::before, *::after { box-sizing: border-box; }
:host { display: none; position: fixed; inset: 0; z-index: 9999; align-items: center; justify-content: center; }
:host([open]) { display: flex; background: rgba(0,0,0,.45); }
...
`,ta=class extends HTMLElement{shadow;inserter=new Nr;activeCatId=Ar[0].id;connectedCallback(){this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${ea}</style>
<div class="dlg">
...
.hidden { display: none !important; }
`,ra=class extends HTMLElement{shadow;mode=`add`;editId=null;entries=[];connectedCallback(){this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${na}</style>
<div class="dlg">
  <div class="hdr">
...
`,aa=class extends HTMLElement{shadow;targetInput=null;connectedCallback(){this.shadow||(this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${ia}</style>${this._tpl()}`,this._bindEvents())}open(e){this.shadow||this.connectedCallback(),this.targetInput=e,this._populate(e),this.setAttribute(`open`,``)}close(){this.removeAttribute(`open`)}_tpl(){return`<div class="dlg">
  <div class="hdr">
    <h3>입력 요소 속성</h3>
    <button class="x-btn" aria-label="닫기">✕</button>
  </div>
...
`,ca=new Set([`edit`,`insert`,`table`,`format`,`misc`]),la=class extends HTMLElement{shadow;_activeTab=`edit`;_userMode=!1;busHandler=({tab:e})=>{this._activeTab=e,this.updateActive()};constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){let e=oa.map(e=>`<button class="tab${e.id===this._activeTab?` active`:``}" data-tab="${e.id}">${e.label}</button>`).join(``);this.shadow.innerHTML=`<style>${sa}</style><div class="menubar">${e}</div>`,this.shadow.querySelector(`.menubar`).addEventListener(`mousedown`,e=>{e.preventDefault();let t=e.target.dataset.tab;t&&P.emit(F.MENUBAR_CHANGE,{tab:t})}),P.on(F.MENUBAR_CHANGE,this.busHandler)}disconnectedCallback(){P.off(F.MENUBAR_CHANGE,this.busHandler)}applyUserMode(){this._userMode=!0,ca.has(this._activeTab)&&P.emit(F.MENUBAR_CHANGE,{tab:`file`}),this._applyUserModeStyles()}_applyUserModeStyles(){if(!this._userMode)return;this.shadow.querySelectorAll(`.tab`).forEach(e=>{let t=e.dataset.tab;ca.has(t)&&(e.dataset.userDisabled=`true`)});let e=this.shadow.querySelector(`.menubar`);if(!e.querySelector(`.user-mode-badge`)){let t=document.createElement(`span`);t.className=`user-mode-badge`,t.textContent=`사용자 모드`,e.appendChild(t)}}updateActive(){this.shadow.querySelectorAll(`.tab`).forEach(e=>{e.classList.toggle(`active`,e.dataset.tab===this._activeTab)})}},ua=`
<svg
...
`;function rs(e){return new Date(e).toLocaleString(`ko-KR`,{month:`2-digit`,day:`2-digit`,hour:`2-digit`,minute:`2-digit`,second:`2-digit`})}var is=class extends HTMLElement{shadow;autoSave=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${ns}</style>
  ...
  <div class="dialog" role="dialog" aria-modal="true" aria-label="설정">
    ...
    <div class="body">
      ...
      <div class="section">
        ...
        <span class="history-time">${rs(n.savedAt)}</span>
        <button class="restore-btn" data-id="${String(n.id)}">복원</button>`,t.querySelector(`.restore-btn`).addEventListener(`click`,()=>{this.handleRestore(n)}),e.appendChild(t)}}handleRestore(e){this.dispatchEvent(new CustomEvent(`poa-autosave-restore`,{bubbles:!0,composed:!0,detail:{html:e.html}})),this.close()}},as=`
/* ── 호스트 슬라이드 애니메이션 ── */
:host {
  display: block;
  overflow: hidden;
...
`,ls=class extends HTMLElement{shadow;editor=new Ei;originalSrc=``;currentDataUrl=``;busy=!1;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${cs}</style>
<div class="backdrop" id="backdrop">
...
`;function ds(e){return e<1024*1024?`${(e/1024).toFixed(1)}KB`:`${(e/1024/1024).toFixed(1)}MB`}var fs=class extends HTMLElement{shadow;uploader=new ki;uploadConfig=null;selectedFiles=[];busy=!1;onErrorFn=null;setOnError(e){this.onErrorFn=e}constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${us}</style>
<div class="backdrop" id="backdrop">
...
.grid-picker {
  display: grid;
  grid-template-columns: repeat(${ps}, ${hs}px);
  gap: ${gs}px;
  cursor: crosshair;
}
.gc {
...
`,vs=class e extends HTMLElement{shadow;selectedPreset=`border-all`;gridCells=[];constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${_s}</style>
<div class="overlay" id="overlay">
  <div class="dialog" role="dialog" aria-modal="true">
    <div class="dlg-hd">
...
`,xs=class extends HTMLElement{shadow;bookmarks=[];editingId=null;editingAnchor=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${bs}</style>
<div class="backdrop" id="backdrop">
  <div class="dialog" role="dialog" aria-modal="true" aria-label="링크 삽입">
    ...
    `).join(``),this.refreshDatetimePreviews(),e.addEventListener(`click`,e=>{let t=e.target.closest(`[data-preset]`);if(!t)return;let n=ys[parseInt(t.dataset.preset??`0`)]?.fn()??``;this.dispatch(`poa-datetime-insert`,{text:n}),this.close()}))}refreshDatetimePreviews(){ys.forEach((e,t)=>{let n=this.shadow.getElementById(`dt-prev-${t}`);n&&(n.textContent=e.fn())})}dispatch(e,t){this.dispatchEvent(new CustomEvent(e,{bubbles:!0,composed:!0,detail:t}))}},Ss=`
:host { display: none; }
:host([open]) {
  display: flex; align-items: center; gap: 6px;
...
`,Cs=class extends HTMLElement{shadow;img=null;aspectLocked=!1;naturalW=0;naturalH=0;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Ss}</style>
<label>너비</label>
...
`,Es=class extends HTMLElement{shadow;resolvePromise=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${ws}</style>${Ts}`,this.bindEvents()}show(e,t=`확인`){let n=this.shadow.getElementById(`dlg-title`),r=this.shadow.getElementById(`dlg-message`);return n&&(n.textContent=t),r&&(r.textContent=e),this.setAttribute(`open`,``),new Promise(e=>{this.resolvePromise=e})}close(e){this.removeAttribute(`open`),this.resolvePromise?.(e),this.resolvePromise=null}bindEvents(){this.shadow.getElementById(`btn-ok`)?.addEventListener(`click`,()=>this.close(!0)),this.shadow.getElementById(`btn-cancel`)?.addEventListener(`click`,()=>this.close(!1)),this.shadow.getElementById(`overlay`)?.addEventListener(`click`,e=>{e.target.id===`overlay`&&this.close(!1)}),this.addEventListener(`keydown`,e=>{e.key===`Escape`&&(e.preventDefault(),this.close(!1))})}},Ds={error:`●`,warning:`▲`,info:`ℹ`},Os={error:`#DC2626`,warning:`#D97706`,info:`#2563EB`},ks=`
:host { display: none; }
:host([open]) { display: block; }
...
`,As=class extends HTMLElement{shadow;issues=[];contentEl=null;rerunFn=null;currentFilter=`all`;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${ks}</style>
<div class="overlay" id="overlay">
  <div class="dialog">
    <div class="header">
      <h2>웹 접근성 검사 결과</h2>
...
`,Fs=class extends HTMLElement{shadow;items=[];filteredType=`all`;onModified=null;onConfirmFn=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${Ps}</style><slot></slot>`}setup(e,t){this.onModified=e,this.onConfirmFn=t}startLoading(){this.setAttribute(`open`,``),this.shadow.innerHTML=`<style>${Ps}</style>
<div class="overlay">
  <div class="dialog">
    <div class="header">
      <h2>개인정보 검사</h2>
...
<div class="card" data-level="${e.riskLevel}" data-id="${e.id}">
  <div class="card-header" data-toggle="${e.id}">
    <span class="card-icon">${Ms[e.riskLevel]}</span>
    <span class="card-label">${e.label}</span>
    <span class="card-risk">${Ns[e.riskLevel]}</span>
    <span class="card-toggle">∨</span>
  </div>
  <div class="card-body" id="body-${e.id}">
    <div class="raw-text">"${Is(e.raw)}"</div>
...
`,Rs=class extends HTMLElement{shadow;currentTable=null;currentRow=1;currentCol=1;pickBar=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${Ls}</style>`}open(e){this.currentTable=e.table,this.currentRow=e.cellRow,this.currentCol=e.cellCol,this.setAttribute(`open`,``),this.render(e.existingFormula,e.initialRange)}close(){this.removeAttribute(`open`),this.exitPickMode(),this.currentTable=null}applyRange(e,t,n,r){this.exitPickMode();let i=e=>this.shadow.getElementById(e),a=(e,t)=>{let n=i(e);n&&(n.value=String(t))};a(`sr`,e),a(`sc`,t),a(`er`,n),a(`ec`,r)}render(e,t){let n=e?.fn??`SUM`,r=e?.format??`integer`,i=e?.decimalPlaces??2,[a,o,s,c]=e?.range??t??[1,1,1,1],l=e?.targetRow??this.currentRow,u=e?.targetCol??this.currentCol,d=!e||e.targetRow===this.currentRow&&e.targetCol===this.currentCol,f=e?.style?.backgroundColor??`#ffffff`,p=e?.style?.color??`#000000`,m=(e,t)=>`<label><input type="radio" name="fn" value="${e}"${n===e?` checked`:``}> ${e} (${t})</label>`,h=(e,t)=>`<label><input type="radio" name="format" value="${e}"${r===e?` checked`:``}> ${t}</label>`;this.shadow.innerHTML=`<style>${Ls}</style>
<div class="overlay" id="overlay">
  <div class="dialog" id="dialog">
    <div class="header">
      <h2>계산식 설정</h2>
    ...
    <div class="footer">
      ...
      ">취소</button>`,document.body.appendChild(e),this.pickBar=e,e.querySelector(`#cancel-pick`)?.addEventListener(`click`,()=>this.exitPickMode()),this.dispatchEvent(new CustomEvent(`poa-formula-start-pick`,{bubbles:!0}))}exitPickMode(){this.shadow.getElementById(`overlay`)?.classList.remove(`pick-mode`),this.pickBar?.remove(),this.pickBar=null}};customElements.get(`poa-menubar`)||customElements.define(`poa-menubar`,la),customElements.get(`poa-context-toolbar`)||customElements.define(`poa-context-toolbar`,qo),customElements.get(`poa-toolbar`)||customElements.define(`poa-toolbar`,es),customElements.get(`poa-status-bar`)||customElements.define(`poa-status-bar`,ts),customElements.get(`poa-settings-dialog`)||customElements.define(`poa-settings-dialog`,is),customElements.get(`poa-find-replace-dialog`)||customElements.define(`poa-find-replace-dialog`,ss),customElements.get(`poa-image-edit-dialog`)||customElements.define(`poa-image-edit-dialog`,ls),customElements.get(`poa-image-dialog`)||customElements.define(`poa-image-dialog`,fs),customElements.get(`poa-table-dialog`)||customElements.define(`poa-table-dialog`,vs),customElements.get(`poa-cell-split-dialog`)||customElements.define(`poa-cell-split-dialog`,ji),customElements.get(`poa-link-dialog`)||customElements.define(`poa-link-dialog`,xs),customElements.get(`poa-image-toolbar`)||customElements.define(`poa-image-toolbar`,Cs),customElements.get(`poa-confirm-dialog`)||customElements.define(`poa-confirm-dialog`,Es),customElements.get(`poa-accessibility-dialog`)||customElements.define(`poa-accessibility-dialog`,As),customElements.get(`poa-privacy-dialog`)||customElements.define(`poa-privacy-dialog`,Fs),customElements.get(`poa-formula-dialog`)||customElements.define(`poa-formula-dialog`,Rs),customElements.get(`poa-video-dialog`)||customElements.define(`poa-video-dialog`,Ni),customElements.get(`poa-form-control-dialog`)||customElements.define(`poa-form-control-dialog`,zi),customElements.get(`poa-template-tree`)||customElements.define(`poa-template-tree`,Vi),customElements.get(`poa-template-dialog`)||customElements.define(`poa-template-dialog`,Wi),customElements.get(`poa-signature-dialog`)||customElements.define(`poa-signature-dialog`,$i),customElements.get(`poa-emoji-dialog`)||customElements.define(`poa-emoji-dialog`,ta),customElements.get(`poa-tooltip-dialog`)||customElements.define(`poa-tooltip-dialog`,ra),customElements.get(`poa-input-property-dialog`)||customElements.define(`poa-input-property-dialog`,aa),customElements.get(`poa-editor`)||customElements.define(`poa-editor`,Ti);var zs=class{mgr;constructor(e){this.container=e,this.mgr=new xi}renderTemplateSelector(){this.container.innerHTML=``;let e=document.createElement(`h2`);e.className=`user-mode-selector-title`,e.textContent=`양식 선택`,this.container.appendChild(e);let t=document.createElement(`div`);t.className=`user-mode-card-grid`;let n=this.mgr.getAll().filter(e=>e.type===`template`&&!e.isTemp);if(n.length===0){let e=document.createElement(`p`);e.style.cssText=`color:#9ca3af;font-size:14px;`,e.textContent=`저장된 템플릿이 없습니다.`,t.appendChild(e)}else for(let e of n)t.appendChild(this._buildCard(e));this.container.appendChild(t)}_buildCard(e){let t=document.createElement(`div`);t.className=`user-mode-card`;let n=document.createElement(`div`);n.className=`user-mode-card-icon`,n.textContent=`📄`;let r=document.createElement(`div`);return r.className=`user-mode-card-name`,r.textContent=e.name,t.appendChild(n),t.appendChild(r),t.addEventListener(`click`,()=>{window.location.href=ui(e.id)}),t}};if(si()===`user`){let e=ci();if(e){let t=new xi().getById(e),n=document.getElementById(`editor`);t?.content&&n.setHTML(t.content),n.enterUserMode(t?.name??`문서`)}else document.getElementById(`editor-root`).classList.add(`hidden`),document.body.classList.add(`user-mode`),new zs(document.getElementById(`user-mode-root`)).renderTemplateSelector()}var Bs=document.getElementById(`editor`),Vs=document.getElementById(`output`),Hs=`<h2>poa-editor v1.0.0</h2>
...
<p>표 삽입 → 삽입 메뉴 → 표를 사용해보세요.</p>`;document.getElementById(`btn-get-html`).addEventListener(`click`,()=>{Vs.style.display=`block`,Vs.textContent=Bs.getHTML()}),document.getElementById(`btn-set-html`).addEventListener(`click`,()=>{Bs.setHTML(Hs),Vs.style.display=`none`}),document.getElementById(`btn-clear`).addEventListener(`click`,()=>{Bs.setHTML(``),Vs.style.display=`none`}),document.getElementById(`btn-find`).addEventListener(`click`,()=>{Bs.querySelector(`.poa-editor-content`)?.focus(),Bs.dispatchEvent(new KeyboardEvent(`keydown`,{key:`f`,ctrlKey:!0,bubbles:!0}))}),document.getElementById(`btn-replace`).addEventListener(`click`,()=>{Bs.querySelector(`.poa-editor-content`)?.focus(),Bs.dispatchEvent(new KeyboardEvent(`keydown`,{key:`h`,ctrlKey:!0,bubbles:!0}))}),document.getElementById(`btn-undo`).addEventListener(`click`,()=>{Bs.querySelector(`.poa-editor-content`)?.focus(),document.execCommand(`undo`)}),document.getElementById(`btn-redo`).addEventListener(`click`,()=>{Bs.querySelector(`.poa-editor-content`)?.focus(),document.execCommand(`redo`)}),si()!==`user`&&setTimeout(()=>Bs.setHTML(Hs),100);