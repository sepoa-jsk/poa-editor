...
</div>`,T=n.defaultView,E=T?T.innerWidth:1920,ce=T?T.innerHeight:1080,D=r.left,le=r.bottom+6;le+340>ce&&(le=r.top-340-6),D+360>E&&(D=r.right-360),D=Math.max(4,D),le=Math.max(4,le);let O=n.createElement(`div`);O.className=`poa-field-popup`,O.style.cssText=[`position:fixed`,`left:${Math.round(D)}px`,`top:${Math.round(le)}px`,`width:360px`,`background:#fff`,`border:1px solid #E2E8F0`,`border-radius:12px`,`box-shadow:0 10px 40px rgba(0,0,0,.15),0 2px 8px rgba(0,0,0,.08)`,`z-index:99999`,`font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif`,`font-size:13px`,`overflow:hidden`].join(`;`),O.innerHTML=`
<div id="pf-header" style="padding:10px 12px 10px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #F1F5F9;gap:8px;cursor:move;">
  <div style="display:flex;align-items:center;gap:8px;min-width:0;">
    <span style="display:inline-flex;align-items:center;justify-content:center;width:26px;height:26px;background:linear-gradient(135deg,#3B82F6,#2563EB);border-radius:7px;color:#fff;font-size:11px;font-weight:700;flex-shrink:0;">${Rr[u]??`T`}</span>
...
`,mi=class extends HTMLElement{shadow;targetCell=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${pi}</style>
...
`,gi=class extends HTMLElement{shadow;activeTab=`video`;constructor(){super(),this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${hi}</style>`}open(e=`video`){this.activeTab=e,this.setAttribute(`open`,``),this.render()}close(){this.removeAttribute(`open`)}render(){this.shadow.innerHTML=`<style>${hi}</style>
<div class="overlay" id="overlay">
  ...
  </svg>`},vi={text:`텍스트`,textarea:`여러 줄`,checkbox:`체크박스`,radio:`라디오`,select:`목록 선택`,button:`버튼`,date:`날짜 선택`},yi=[`text`,`textarea`,`checkbox`,`radio`,`select`,`button`,`date`],bi=`
...
`;function J(e,t,n=``){return`<div class="field">
  ...
  <div class="ftr">
    ...
    <div class="adv-body" data-adv-body>${e}</div>`}_secText(){return`<div class="specific-section" id="sec-text">
      ${J(`항목 이름`,Y(`f-label`,`예) 이름, 이메일 주소…`),`사용자에게 보여지는 항목 설명입니다`)}
      ${J(`안내 문구`,Y(`f-text-ph`,`예) 홍길동`),`입력창 안에 표시되는 도움말입니다`)}
      ${this._adv(`
        <div class="chk-row" style="margin-bottom:10px">
          <input type="checkbox" id="f-aria-req"><label for="f-aria-req">필수 입력 항목으로 표시</label>
        </div>
        ${J(`최대 글자 수`,Y(`f-text-ml`,`제한 없음`,`number`))}
        ${J(`기본값`,Y(`f-text-val`,`미리 채울 내용`))}
        ${J(`name 속성`,Y(`f-name`,`자동 생성`))}
      `)}
    </div>`}_secTextarea(){return`<div class="specific-section" id="sec-textarea">
      ${J(`항목 이름`,Y(`f-ta-label`,`예) 내용, 자기소개…`))}
      ${J(`안내 문구`,Y(`f-ta-ph`,`예) 간단하게 소개해 주세요`))}
      ${J(`줄 수`,`<input type="number" id="f-ta-rows" value="4" min="2" max="20" style="width:80px">`,`입력창의 기본 높이를 줄 수로 조정합니다`)}
      ${this._adv(`
        <div class="chk-row" style="margin-bottom:10px">
          <input type="checkbox" id="f-ta-req"><label for="f-ta-req">필수 입력 항목으로 표시</label>
        </div>
        ${J(`name 속성`,Y(`f-ta-name`,`자동 생성`))}
      `)}
    </div>`}_secCheckbox(){return`<div class="specific-section" id="sec-checkbox">
      ${J(`체크박스 텍스트`,Y(`f-cb-lbl`,`예) 개인정보 수집에 동의합니다`),`체크박스 옆에 표시되는 설명입니다`)}
      <div class="field">
        <div class="chk-row">
      ...
      ${this._adv(`
        ...
        ${J(`name 속성`,Y(`f-cb-name`,`자동 생성`))}
      `)}
    </div>`}_secRadio(){return`<div class="specific-section" id="sec-radio">
      ${J(`그룹 제목`,Y(`f-radio-label`,`예) 성별, 선호 연락 방법…`),`라디오 버튼 그룹 위에 표시되는 제목입니다`)}
      <div class="field">
        <label>선택지 목록</label>
      ...
      ${this._adv(`
        <div class="field">
          ...
          ${xi(`f-btn-style`,[[`default`,`기본 (회색)`],[`primary`,`강조 (파랑)`],[`danger`,`경고 (빨강)`]])}
        </div>
        ${J(`name 속성`,Y(`f-btn-name`,`자동 생성`))}
      `)}
    </div>`}_secDate(){return`<div class="specific-section" id="sec-date">
      ${J(`항목 이름`,Y(`f-date-label`,`예) 생년월일, 예약일…`))}
      <div class="field">
        <div class="chk-row">
...
`,ji=class extends HTMLElement{shadow;mgr;tree;selected=null;_getEditorHTML=null;connectedCallback(){this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${ki}</style>${Ai}`,this.mgr=new Ei,this.tree=this.shadow.querySelector(`poa-template-tree`),this.tree.setManager(this.mgr),this._bind()}setup(e){this._getEditorHTML=e}open(){this.selected=null,this._refreshPreview(),this._fillFolderSelect(),this.tree.render(),this.setAttribute(`open`,``)}close(){this.removeAttribute(`open`)}_bind(){this.shadow.querySelector(`.x-btn`).addEventListener(`click`,()=>this.close()),this.addEventListener(`click`,e=>{e.composedPath()[0]===this&&this.close()}),this.shadow.getElementById(`btn-add-folder`).addEventListener(`click`,()=>{let e=this.tree.getSelected(),t=e?.type===`folder`?e.id:null;this.tree.addFolder(t)}),this.shadow.getElementById(`btn-save-tmpl`).addEventListener(`click`,()=>{this._fillFolderSelect(),this.shadow.getElementById(`save-name`).value=``,this.shadow.getElementById(`save-overlay`).classList.remove(`hidden`),setTimeout(()=>this.shadow.getElementById(`save-name`).focus(),50)}),this.shadow.getElementById(`save-cancel`).addEventListener(`click`,()=>{this.shadow.getElementById(`save-overlay`).classList.add(`hidden`)}),this.shadow.getElementById(`save-ok`).addEventListener(`click`,()=>this._doSave()),this.shadow.getElementById(`save-name`).addEventListener(`keydown`,e=>{e.key===`Enter`&&this._doSave()}),this.shadow.getElementById(`btn-append`).addEventListener(`click`,()=>this._apply(`append`)),this.shadow.getElementById(`btn-replace`).addEventListener(`click`,()=>this._apply(`replace`)),this.shadow.addEventListener(`poa-tmpl-select`,e=>{let{node:t}=e.detail;this.selected=t.type===`template`?t:null,this._refreshPreview()}),this.shadow.addEventListener(`poa-tmpl-dblclick`,e=>{let{node:t}=e.detail;this.selected=t,this._apply(`replace`)})}_refreshPreview(){let e=this.shadow.getElementById(`preview-frame`),t=this.shadow.getElementById(`preview-empty`),n=this.shadow.getElementById(`btn-append`),r=this.shadow.getElementById(`btn-replace`);if(!this.selected||this.selected.type!==`template`){e.classList.add(`hidden`),t.classList.remove(`hidden`),n.disabled=!0,r.disabled=!0;return}e.srcdoc=`<!doctype html><html><head><style>
       ...
       </style></head><body>${String(j.sanitize(this.selected.content??``,{USE_PROFILES:{html:!0}}))}</body></html>`,e.classList.remove(`hidden`),t.classList.add(`hidden`),n.disabled=!1,r.disabled=!1}_apply(e){if(!this.selected?.content)return;let t=String(j.sanitize(this.selected.content,{USE_PROFILES:{html:!0}}));this.dispatchEvent(new CustomEvent(`poa-tmpl-insert`,{bubbles:!0,composed:!0,detail:{html:t,mode:e}})),e===`append`&&this.close()}_doSave(){let e=this.shadow.getElementById(`save-name`).value.trim();if(!e){this.shadow.getElementById(`save-name`).focus();return}let t=this.shadow.getElementById(`save-folder`).value||null,n=this.shadow.getElementById(`save-public`).checked,r=this._getEditorHTML?.()??``;this.mgr.addTemplate(e,r,t,n),this.tree.render(),this.shadow.getElementById(`save-overlay`).classList.add(`hidden`)}_fillFolderSelect(){let e=this.shadow.getElementById(`save-folder`);e.innerHTML=``;let t=this.mgr.getFolders().find(e=>!e.isPublic);for(let n of this.mgr.getFolders()){let r=document.createElement(`option`);r.value=n.id,r.textContent=n.name,n===t&&(r.selected=!0),e.appendChild(r)}}},Mi=`poa-signatures`,Ni=10;function Pi(){return`${Date.now().toString(36)}-${Math.random().toString(36).slice(2,7)}`}function X(e){return e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}function Fi(e){let{layout:t,fields:n,logo:r,headerColor:i=`#2563eb`}=e,a=n.displayName?`<strong style="font-size:15px;color:#111;">${X(n.displayName)}</strong>`:``,o=[n.title,n.department].filter(Boolean).map(X).join(` | `),s=n.company?`<span style="color:#555;">${X(n.company)}</span>`:``,c=n.phone?`<span style="color:#888;font-size:12px;">T.</span>&nbsp;${X(n.phone)}`:``,l=n.email?`<span style="color:#888;font-size:12px;">E.</span>&nbsp;${X(n.email)}`:``,u=n.website?`<span style="color:#888;font-size:12px;">W.</span>&nbsp;${X(n.website)}`:``,d=[o?`<span style="color:#555;">${o}</span>`:``,s,c,l,u].filter(Boolean),f=r?`<img src="${X(r)}" alt="" style="max-width:80px;max-height:60px;display:block;">`:``,p=`font-size:14px;line-height:1.6;border-collapse:collapse;font-family:Arial,Helvetica,sans-serif;`,m=()=>[a,...d].filter(Boolean).join(`<br>`);switch(t){case 1:return`<table class="poa-signature" style="${p}"><tbody><tr><td style="padding:8px 0;vertical-align:top;">${m()}</td></tr></tbody></table>`;case 2:return r?`<table class="poa-signature" style="${p}"><tbody><tr><td style="padding:0 14px 0 0;vertical-align:middle;border-right:2px solid #e5e7eb;">${f}</td><td style="padding:0 0 0 14px;vertical-align:middle;">${m()}</td></tr></tbody></table>`:Fi({...e,layout:1});case 3:return r?`<table class="poa-signature" style="${p}"><tbody><tr><td style="text-align:center;padding-bottom:6px;">${f}</td></tr><tr><td style="padding:4px 0;">${m()}</td></tr></tbody></table>`:Fi({...e,layout:1});case 4:return`<table class="poa-signature" style="${p}"><tbody><tr><td style="padding:8px 0;vertical-align:top;">${a?a+`<br>`:``}<hr style="border:none;border-top:1px solid #aaa;margin:4px 0;">${d.join(`<br>`)}</td></tr></tbody></table>`;case 5:{let e=[a,n.department?`<span style="color:#555;">${X(n.department)}</span>`:``].filter(Boolean).join(`<br>`),t=[n.title?`<span style="color:#555;">${X(n.title)}</span>`:``,s].filter(Boolean).join(`<br>`),r=[c,l].filter(Boolean).join(`&nbsp;|&nbsp;`);return`<table class="poa-signature" style="${p}"><tbody><tr><td style="padding:4px 14px 4px 0;border-right:1px solid #ddd;vertical-align:top;">${e||`&nbsp;`}</td><td style="padding:4px 0 4px 14px;vertical-align:top;">${t||`&nbsp;`}</td></tr>`+(r?`<tr><td colspan="2" style="padding:6px 0 4px;border-top:1px solid #eee;font-size:13px;">${r}</td></tr>`:``)+`</tbody></table>`}case 6:{let e=[n.displayName?`<strong style="color:#fff;">${X(n.displayName)}</strong>`:``,n.title?`<span style="color:rgba(255,255,255,.85);">${X(n.title)}</span>`:``].filter(Boolean).join(`&nbsp;&nbsp;`),t=[n.department?`<span style="color:#555;">${X(n.department)}</span>`:``,s,c,l,u].filter(Boolean);return`<table class="poa-signature" style="${p}border:1px solid ${X(i)};"><tbody><tr><td style="background:${X(i)};padding:8px 16px;">${e||`&nbsp;`}</td></tr><tr><td style="padding:8px 16px;">${t.join(`<br>`)||`&nbsp;`}</td></tr></tbody></table>`}default:return``}}var Ii=class{sigs=[];constructor(){this._load()}_load(){try{let e=localStorage.getItem(Mi);this.sigs=e?JSON.parse(e):[]}catch{this.sigs=[]}}_persist(){try{localStorage.setItem(Mi,JSON.stringify(this.sigs))}catch{}}getAll(){return[...this.sigs]}getById(e){return this.sigs.find(t=>t.id===e)??null}add(e){if(this.sigs.length>=Ni)throw Error(`최대 ${Ni}개까지 저장할 수 있습니다.`);let t={...e,id:Pi(),createdAt:Date.now(),updatedAt:Date.now()};return this.sigs.push(t),this._persist(),t}update(e,t){let n=this.sigs.find(t=>t.id===e);return n?(Object.assign(n,t,{updatedAt:Date.now()}),this._persist(),!0):!1}delete(e){let t=this.sigs.findIndex(t=>t.id===e);return t<0?!1:(this.sigs.splice(t,1),this._persist(),!0)}isFull(){return this.sigs.length>=Ni}},Li={1:`기본형`,2:`로고(좌)`,3:`로고(상)`,4:`구분선형`,5:`2단형`,6:`컬러헤더`},Ri=`
*, *::before, *::after { box-sizing: border-box; }
:host { display: none; position: fixed; inset: 0; z-index: 9999; align-items: center; justify-content: center; }
:host([open]) { display: flex; background: rgba(0,0,0,.45); }
...
`,Hi=class extends HTMLElement{shadow;inserter=new jr;activeCatId=Or[0].id;connectedCallback(){this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${Vi}</style>
<div class="dlg">
...
</div>`,this._buildCatBar(),this._bind()}open(){this._renderCategory(this.activeCatId),this.setAttribute(`open`,``),setTimeout(()=>this.shadow.getElementById(`search`).focus(),50)}close(){this.removeAttribute(`open`)}_buildCatBar(){let e=this.shadow.getElementById(`cat-bar`);e.innerHTML=``;for(let t of Or){let n=document.createElement(`button`);n.className=`cat-btn${t.id===this.activeCatId?` active`:``}`,n.title=t.label,n.textContent=t.icon,n.dataset.catId=t.id,n.addEventListener(`click`,()=>{this.activeCatId=t.id,this._syncCatBar(),this._renderCategory(t.id),this.shadow.getElementById(`search`).value=``}),e.appendChild(n)}}_syncCatBar(){this.shadow.querySelectorAll(`.cat-btn`).forEach(e=>{e.classList.toggle(`active`,e.dataset.catId===this.activeCatId)})}_renderCategory(e){let t=this.shadow.getElementById(`emoji-area`);t.innerHTML=``;let n=this.inserter.getRecent();if(n.length>0){this._appendSection(t,`최근 사용`,n);let e=document.createElement(`hr`);e.className=`divider`,t.appendChild(e)}let r=Or.find(t=>t.id===e);r&&this._appendSection(t,r.label,r.emojis)}_renderSearch(e){let t=this.shadow.getElementById(`emoji-area`);t.innerHTML=``;let n=Ar(e);if(n.length===0){let n=document.createElement(`div`);n.className=`no-result`,n.textContent=`"${e}"에 대한 결과가 없습니다`,t.appendChild(n)}else this._appendSection(t,`검색 결과 (${n.length})`,n)}_appendSection(e,t,n){let r=document.createElement(`div`);r.className=`section-label`,r.textContent=t,e.appendChild(r);let i=document.createElement(`div`);i.className=`emoji-grid`;for(let e of n){let t=document.createElement(`button`);t.className=`emoji-btn`,t.textContent=e,t.title=e,t.addEventListener(`click`,()=>this._pickEmoji(e)),i.appendChild(t)}e.appendChild(i)}_bind(){this.shadow.querySelector(`.x-btn`).addEventListener(`click`,()=>this.close()),this.addEventListener(`click`,e=>{e.composedPath()[0]===this&&this.close()});let e=this.shadow.getElementById(`search`);e.addEventListener(`input`,()=>{let t=e.value.trim();t?this._renderSearch(t):this._renderCategory(this.activeCatId)}),e.addEventListener(`keydown`,e=>{e.key===`Escape`&&(e.stopPropagation(),this.close())})}_pickEmoji(e){this.dispatchEvent(new CustomEvent(`poa-emoji-insert`,{bubbles:!0,composed:!0,detail:{emoji:e}})),this.shadow.getElementById(`search`).value.trim()||this._renderCategory(this.activeCatId)}},Ui=`
*, *::before, *::after { box-sizing: border-box; }
:host { display: none; position: fixed; inset: 0; z-index: 9999; align-items: center; justify-content: center; }
:host([open]) { display: flex; background: rgba(0,0,0,.45); }
...
.hidden { display: none !important; }
`,Wi=class extends HTMLElement{shadow;mode=`add`;editId=null;entries=[];connectedCallback(){this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${Ui}</style>
<div class="dlg">
  <div class="hdr">
...
`,Ki=class extends HTMLElement{shadow;targetInput=null;connectedCallback(){this.shadow||(this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${Gi}</style>${this._tpl()}`,this._bindEvents())}open(e){this.shadow||this.connectedCallback(),this.targetInput=e,this._populate(e),this.setAttribute(`open`,``)}close(){this.removeAttribute(`open`)}_tpl(){return`<div class="dlg">
  <div class="hdr">
    <h3>입력 요소 속성</h3>
    <button class="x-btn" aria-label="닫기">✕</button>
  </div>
...
`,Yi=class extends HTMLElement{shadow;_activeTab=`edit`;busHandler=({tab:e})=>{this._activeTab=e,this.updateActive()};constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){let e=qi.map(e=>`<button class="tab${e.id===this._activeTab?` active`:``}" data-tab="${e.id}">${e.label}</button>`).join(``);this.shadow.innerHTML=`<style>${Ji}</style><div class="menubar">${e}</div>`,this.shadow.querySelector(`.menubar`).addEventListener(`mousedown`,e=>{e.preventDefault();let t=e.target.dataset.tab;t&&N.emit(P.MENUBAR_CHANGE,{tab:t})}),N.on(P.MENUBAR_CHANGE,this.busHandler)}disconnectedCallback(){N.off(P.MENUBAR_CHANGE,this.busHandler)}updateActive(){this.shadow.querySelectorAll(`.tab`).forEach(e=>{e.classList.toggle(`active`,e.dataset.tab===this._activeTab)})}},Xi=`
<svg
...
`;function Z(e){return e.replace(/width="24"/,`width="16"`).replace(/height="24"/,`height="16"`)}var Q={bold:Z(ra),italic:Z(Sa),underline:Z(fo),strike:Z(Qa),undo:Z(po),redo:Z(Ba),alignLeft:Z(co),alignCenter:Z(ao),alignRight:Z(oo),alignJustify:Z(so),indent:Z(ka),outdent:Z(Oa),foreColor:Z(ta),backColor:Z(va),lineHeight:Z(na),letterSpacing:Z(Fa),fileNew:Z(ma),fileOpen:Z(ga),save:Z(Wa),print:Z(za),settings:Z(qa),clock:Z(la),cut:Z(Ga),copy:Z(fa),paste:Z(ca),pastePlain:Z(sa),selectAll:Z(Za),search:Z(Ka),image:Z(ya),images:Z(ba),link:Z(Da),linkOff:Z(Ea),bookmark:Z(aa),calendar:Z(oa),hr:Z(Na),symbol:Z(Ia),video:Z(mo),youtube:Z(Pa),viewDesign:Z(wa),viewHtml:Z(ua),viewPreview:Z(pa),viewText:Z(uo),viewPage:Z(ha),fullscreen:Z(Ma),ruler:Z(Ua),grid:Z(_a),hiddenBorder:Z(Za),table:Z(io),tableProps:Z(ro),cellProps:Z(Ta),mergeCells:Z(to),splitCell:Z(no),rowAbove:Z(ea),rowBelow:Z(Zi),colLeft:Z(Qi),colRight:Z($i),rowDelete:Z(Ha),colDelete:Z(da),tableDelete:Z(lo),painterCopy:Z(La),painterPaste:Z(Ra),formatClear:Z(Va),ul:Z(ja),ol:Z(Aa),sup:Z(eo),sub:Z($a),a11y:Z(Xi),privacy:Z(Ja),form:Z(Xa),calc:Z(Ya),shortcuts:Z(Ca),guide:Z(ia),about:Z(xa)},ho={"file:new":`fileNew`,"file:open":`fileOpen`,"file:save":`save`,"file:saveas":`save`,"file:print":`print`,settings:`settings`,"edit:cut":`cut`,"edit:copy":`copy`,"edit:paste":`paste`,"edit:paste-plain":`pastePlain`,"edit:select-all":`selectAll`,"find-replace":`search`,"edit:image-edit":`image`,image:`image`,"insert:multi-image":`images`,"insert:link":`link`,"insert:bookmark":`bookmark`,"insert:datetime":`calendar`,"insert:hr":`hr`,"insert:symbol":`symbol`,"view:design":`viewDesign`,"view:html":`viewHtml`,"view:preview":`viewPreview`,"view:text":`viewText`,"view:page":`viewPage`,"view:fullscreen":`fullscreen`,"view:ruler":`ruler`,"view:grid":`grid`,"view:hidden-border":`hiddenBorder`,table:`table`,"table:table-props":`tableProps`,"table:cell-props":`cellProps`,"table:merge":`mergeCells`,"table:split-cell":`splitCell`,"table:row-above":`rowAbove`,"table:row-below":`rowBelow`,"table:col-left":`colLeft`,"table:col-right":`colRight`,"table:row-delete":`rowDelete`,"table:col-delete":`colDelete`,"table:delete":`tableDelete`,"format:painter-copy":`painterCopy`,"format:painter-paste":`painterPaste`,"format:clear":`formatClear`,"format:ul":`ul`,"format:ol":`ol`,"format:sup":`sup`,"format:sub":`sub`,"misc:a11y":`a11y`,"misc:privacy":`privacy`,"misc:form":`form`,"misc:calc":`calc`,"help:shortcuts":`shortcuts`,"help:guide":`guide`,"help:about":`about`},go={text:`T`,textarea:`☰`,number:`#`,date:`📅`},_o={file:[[[`새 문서`,`file:new`],[`열기`,`file:open`],[`저장`,`file:save`],[`다른 이름으로 저장`,`file:saveas`]],[[`인쇄`,`file:print`],[`환경설정`,`settings`]]],edit:[[[`잘라내기`,`edit:cut`],[`복사`,`edit:copy`],[`붙여넣기`,`edit:paste`],[`서식 없이 붙여넣기`,`edit:paste-plain`]],[[`찾기·바꾸기`,`find-replace`],[`이미지 편집`,`edit:image-edit`],[`전체 선택`,`edit:select-all`]]],insert:[[[`이미지`,`image`]],[[`하이퍼링크`,`insert:link`],[`책갈피`,`insert:bookmark`]],[[`서명`,`insert:signature`],[`이모지`,`insert:emoji`]],[[`툴팁`,`insert:tooltip`],[`툴팁 관리`,`insert:tooltip-list`]],[[`날짜·시간`,`insert:datetime`],[`가로줄`,`insert:hr`],[`기호`,`insert:symbol`]],[{dropdown:!0,id:`doc-field`,label:`양식 필드`,items:$r.map(e=>[e.label,`insert:field`,e.id,go[e.type]])}]],view:[[[`디자인`,`view:design`],[`HTML`,`view:html`],[`미리보기`,`view:preview`],[`텍스트`,`view:text`],[`페이지`,`view:page`]],[[`전체화면`,`view:fullscreen`],[`눈금자`,`view:ruler`],[`그리드`,`view:grid`],[`숨김 테두리`,`view:hidden-border`]]],table:[[[`표 삽입`,`table`],[`표 속성`,`table:table-props`],[`셀 속성`,`table:cell-props`]],[[`셀 병합`,`table:merge`],[`셀 나누기`,`table:split-cell`]],[[`위에 행 삽입`,`table:row-above`],[`아래에 행 삽입`,`table:row-below`],[`왼쪽에 열 삽입`,`table:col-left`],[`오른쪽에 열 삽입`,`table:col-right`]],[[`행 삭제`,`table:row-delete`],[`열 삭제`,`table:col-delete`],[`표 삭제`,`table:delete`]]],format:[[[`서식 복사`,`format:painter-copy`],[`서식 붙여넣기`,`format:painter-paste`],[`서식 제거`,`format:clear`]],[[`글머리 기호`,`format:ul`],[`글머리 번호`,`format:ol`]],[[`위 첨자`,`format:sup`],[`아래 첨자`,`format:sub`]]],misc:[[[`웹 접근성 체크`,`misc:a11y`],[`개인정보 체크`,`misc:privacy`]],[[`폼 컨트롤`,`misc:form`],[`계산식`,`misc:calc`],[`템플릿`,`misc:template`]]],help:[[[`단축키`,`help:shortcuts`],[`사용자 가이드`,`help:guide`],[`제품 정보`,`help:about`]]]},vo=new Set([`view:design`,`view:html`,`view:preview`,`view:text`,`view:page`]),yo=`
:host {
  display: block;
  --icon-color:         #374151;
...
`,bo=class extends HTMLElement{shadow;activeTab=`edit`;activeViewMode=`design`;outsideHandler=null;busHandler=({tab:e})=>{this.activeTab=e,this.render()};viewHandler=({mode:e})=>{this.activeViewMode=e,this.activeTab===`view`&&this.render()};constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.render(),N.on(P.MENUBAR_CHANGE,this.busHandler),N.on(P.VIEW_CHANGE,this.viewHandler)}disconnectedCallback(){N.off(P.MENUBAR_CHANGE,this.busHandler),N.off(P.VIEW_CHANGE,this.viewHandler),this.closeAllDropdowns()}render(){this.closeAllDropdowns();let e=_o[this.activeTab]??[],t=[];for(let n=0;n<e.length;n++){n>0&&t.push(`<div class="sep"></div>`),t.push(`<div class="group">`);for(let r of e[n])if(r===null)t.push(`<div class="sep" style="margin:0 2px;"></div>`);else if(`dropdown`in r&&r.dropdown){let e=r.items.map(([e,t,n,r])=>`<div class="drop-item" data-action="${t}"${n?` data-value="${n}"`:``}>${r?`<span class="type-icon">${r}</span>`:``}${e}</div>`).join(``);t.push(`<div class="dropdown-wrap" id="dd-${r.id}"><button class="btn dropdown-toggle" data-dropdown-id="${r.id}">${r.label}</button><div class="dropdown-menu" id="menu-${r.id}">${e}</div></div>`)}else{let[e,n,i,a]=r,o=i?` data-value="${i}"`:``,s=vo.has(n)&&n===`view:${this.activeViewMode}`?` active`:``,c=a??e,l=ho[n],u=l?Q[l]:``,d=u?`${u}<span>${e}</span>`:e;t.push(`<button class="btn${s}" data-action="${n}"${o} data-tip="${c}">${d}</button>`)}t.push(`</div>`)}this.shadow.innerHTML=`<style>${yo}</style><div class="ctx-bar">${t.join(``)}</div>`,this.shadow.querySelector(`.ctx-bar`).addEventListener(`mousedown`,e=>{let t=e.target,n=t.closest(`.dropdown-toggle`);if(n){let t=n.dataset.dropdownId;if(!t)return;e.preventDefault();let r=this.shadow.getElementById(`menu-${t}`);if(!r)return;let i=r.classList.contains(`open`);this.closeAllDropdowns(),i||(r.classList.add(`open`),this.setupOutsideClick(t));return}let r=t.closest(`.drop-item`);if(r){let t=r.dataset.action,n=r.dataset.value;if(!t)return;e.preventDefault(),this.closeAllDropdowns(),this.dispatchEvent(new CustomEvent(`poa-action`,{bubbles:!0,composed:!0,detail:{type:t,value:n}}));return}let i=t.closest(`.btn`);if(!i||i.disabled)return;let a=i.dataset.action;if(!a)return;e.preventDefault();let o=i.dataset.value;this.dispatchEvent(new CustomEvent(`poa-action`,{bubbles:!0,composed:!0,detail:{type:a,value:o}}))})}closeAllDropdowns(){this.shadow.querySelectorAll(`.dropdown-menu.open`).forEach(e=>e.classList.remove(`open`)),this.outsideHandler&&=(document.removeEventListener(`mousedown`,this.outsideHandler),null)}setupOutsideClick(e){this.outsideHandler=t=>{let n=this.shadow.getElementById(`dd-${e}`);n&&(t.composedPath().includes(n)||this.closeAllDropdowns())},document.addEventListener(`mousedown`,this.outsideHandler)}},xo=[{label:`기본`,value:`inherit`},{label:`맑은 고딕`,value:`맑은 고딕, sans-serif`},{label:`나눔고딕`,value:`나눔고딕, sans-serif`},{label:`나눔명조`,value:`나눔명조, serif`},{label:`굴림`,value:`굴림, sans-serif`},{label:`돋움`,value:`돋움, sans-serif`},{label:`바탕`,value:`바탕, serif`},{label:`궁서`,value:`궁서, serif`},{label:`Arial`,value:`Arial, sans-serif`},{label:`Times New Roman`,value:`Times New Roman, serif`},{label:`Courier New`,value:`Courier New, monospace`},{label:`Georgia`,value:`Georgia, serif`},{label:`Verdana`,value:`Verdana, sans-serif`},{label:`Tahoma`,value:`Tahoma, sans-serif`}],So=[`8`,`9`,`10`,`11`,`12`,`14`,`16`,`18`,`20`,`24`,`28`,`32`,`36`,`48`,`72`],Co=[`1`,`1.2`,`1.4`,`1.5`,`1.6`,`1.8`,`2.0`,`2.5`],wo=[`0px`,`0.5px`,`1px`,`1.5px`,`2px`,`3px`,`4px`],To=`
:host {
  display: block;
  --icon-color:         #374151;
  --icon-hover-bg:      #F3F4F6;
...
`;function Ao(e){return new Date(e).toLocaleString(`ko-KR`,{month:`2-digit`,day:`2-digit`,hour:`2-digit`,minute:`2-digit`,second:`2-digit`})}var jo=class extends HTMLElement{shadow;autoSave=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${ko}</style>
  ...
  <div class="dialog" role="dialog" aria-modal="true" aria-label="설정">
    ...
    <div class="body">
      ...
      <div class="section">
        ...
        <span class="history-time">${Ao(n.savedAt)}</span>
        <button class="restore-btn" data-id="${String(n.id)}">복원</button>`,t.querySelector(`.restore-btn`).addEventListener(`click`,()=>{this.handleRestore(n)}),e.appendChild(t)}}handleRestore(e){this.dispatchEvent(new CustomEvent(`poa-autosave-restore`,{bubbles:!0,composed:!0,detail:{html:e.html}})),this.close()}},Mo=`
/* ── 호스트 슬라이드 애니메이션 ── */
:host {
  display: block;
  overflow: hidden;
...
`,Io=class extends HTMLElement{shadow;editor=new li;originalSrc=``;currentDataUrl=``;busy=!1;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Fo}</style>
<div class="backdrop" id="backdrop">
...
`;function Ro(e){return e<1024*1024?`${(e/1024).toFixed(1)}KB`:`${(e/1024/1024).toFixed(1)}MB`}var zo=class extends HTMLElement{shadow;uploader=new fi;uploadConfig=null;selectedFiles=[];busy=!1;onErrorFn=null;setOnError(e){this.onErrorFn=e}constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Lo}</style>
<div class="backdrop" id="backdrop">
...
.grid-picker {
  display: grid;
  grid-template-columns: repeat(${Bo}, ${Ho}px);
  gap: ${Uo}px;
  cursor: crosshair;
}
.gc {
  width: ${Ho}px; height: ${Ho}px;
...
.btn:not(.btn-primary):hover { background: #f5f5f5; }
`,Go=class e extends HTMLElement{shadow;selectedPreset=`border-all`;gridCells=[];constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Wo}</style>
<div class="overlay" id="overlay">
  <div class="dialog" role="dialog" aria-modal="true">
    <div class="dlg-hd">
...
</div>`,this.shadow.getElementById(`btn-close`).addEventListener(`click`,()=>this.close()),this.shadow.getElementById(`overlay`).addEventListener(`click`,e=>{e.target.id===`overlay`&&this.close()})}open(e){this.classList.add(`open`);let t=this.shadow.getElementById(`dlg-body`),n=this.shadow.getElementById(`dlg-title`);e?(n.textContent=`표 속성`,this.renderProps(t,e)):(n.textContent=`표`,this.renderInsert(t))}close(){this.classList.remove(`open`),this.gridCells=[]}renderInsert(e){let t=e.ownerDocument;e.innerHTML=``;let n=t.createElement(`div`);n.className=`preset-grid`;for(let e of tn){let r=t.createElement(`div`);r.className=`preset-item`+(e.id===this.selectedPreset?` active`:``),r.dataset.pid=e.id,r.innerHTML=`${e.icon}<span class="preset-lbl">${e.label}</span>`,r.addEventListener(`click`,()=>this.selectPreset(e.id)),n.appendChild(r)}e.appendChild(n);let r=t.createElement(`hr`);r.className=`sep`,e.appendChild(r);let i=t.createElement(`div`);i.className=`grid-wrap`;let a=t.createElement(`div`);a.className=`grid-picker`,this.gridCells=[];for(let e=0;e<Vo;e++)for(let n=0;n<Bo;n++){let r=t.createElement(`div`);r.className=`gc`,r.dataset.r=String(e),r.dataset.c=String(n),a.appendChild(r),this.gridCells.push(r)}let o=t.createElement(`div`);o.className=`grid-size`,o.textContent=``,a.addEventListener(`mousemove`,e=>{if(!e.target.closest(`.grid-picker`))return;let t=a.getBoundingClientRect(),n=Ho+Uo,r=Math.min(Math.floor((e.clientX-t.left)/n),Bo-1),i=Math.min(Math.floor((e.clientY-t.top)/n),Vo-1);this.highlightGrid(i,r),o.textContent=`${r+1} × ${i+1}`}),a.addEventListener(`mouseleave`,()=>{this.highlightGrid(-1,-1),o.textContent=``}),a.addEventListener(`click`,e=>{let t=a.getBoundingClientRect(),n=Ho+Uo,r=Math.min(Math.floor((e.clientX-t.left)/n),Bo-1),i=Math.min(Math.floor((e.clientY-t.top)/n),Vo-1);this.insertTable(i+1,r+1)}),i.appendChild(a),i.appendChild(o),e.appendChild(i)}selectPreset(e){this.selectedPreset=e,this.shadow.querySelectorAll(`.preset-item`).forEach(t=>{t.classList.toggle(`active`,t.dataset.pid===e)})}highlightGrid(e,t){for(let n of this.gridCells){let r=parseInt(n.dataset.r??`0`),i=parseInt(n.dataset.c??`0`);n.classList.toggle(`hl`,r<=e&&i<=t)}}insertTable(e,t){let n=tn.find(e=>e.id===this.selectedPreset)??tn[0],r={rows:e,cols:t,width:`100%`,...n.baseOptions};this.dispatchEvent(new CustomEvent(`poa-table-insert`,{bubbles:!0,composed:!0,detail:{options:r,presetId:n.id}})),this.close()}renderProps(t,n){let r=n.style.width||`100%`,i=e.rgbToHex(n.style.backgroundColor)||`#ffffff`,a=n.querySelector(`td,th`)?.style.border.match(/solid\s+(#[\da-fA-F]{3,6}|[a-z]+)/i),o=a?a[1]:`#000000`,s=`left`;n.style.marginLeft===`auto`&&n.style.marginRight===`auto`?s=`center`:n.style.marginLeft===`auto`&&(s=`right`),t.innerHTML=`
...
</div>`,t.querySelector(`#pp-al`).value=s,t.querySelector(`#pp-cancel`).addEventListener(`click`,()=>this.close()),t.querySelector(`#pp-ok`).addEventListener(`click`,()=>{let e={width:t.querySelector(`#pp-w`).value.trim(),borderColor:t.querySelector(`#pp-bc`).value,bgColor:t.querySelector(`#pp-bg`).value,align:t.querySelector(`#pp-al`).value};this.dispatchEvent(new CustomEvent(`poa-table-update`,{bubbles:!0,composed:!0,detail:{options:e,table:n}})),this.close()})}static rgbToHex(e){let t=e.match(/\d+/g);return!t||t.length<3?``:`#`+t.slice(0,3).map(e=>parseInt(e).toString(16).padStart(2,`0`)).join(``)}static colorToHex(t){return e.rgbToHex(t)||`#ffffff`}},Ko=[{label:`날짜 (2025-05-05)`,fn:()=>new Date().toISOString().slice(0,10)},{label:`시간 (14:30:05)`,fn:()=>new Intl.DateTimeFormat(`ko-KR`,{hour:`2-digit`,minute:`2-digit`,second:`2-digit`,hour12:!1}).format(new Date)},{label:`날짜·시간 (2025-05-05 14:30)`,fn:()=>{let e=new Date;return`${e.toISOString().slice(0,10)} ${new Intl.DateTimeFormat(`ko-KR`,{hour:`2-digit`,minute:`2-digit`,hour12:!1}).format(e)}`}},{label:`한국어 날짜 (2025년 5월 5일)`,fn:()=>new Intl.DateTimeFormat(`ko-KR`,{year:`numeric`,month:`long`,day:`numeric`}).format(new Date)},{label:`한국어 날짜·시간 (2025년 5월 5일 월요일 오후 2:30)`,fn:()=>new Intl.DateTimeFormat(`ko-KR`,{year:`numeric`,month:`long`,day:`numeric`,weekday:`long`,hour:`numeric`,minute:`2-digit`}).format(new Date)}],qo=`
:host { display: none; }
:host([open]) { display: block; }
.backdrop {
  position: fixed; inset: 0; background: rgba(0,0,0,.45);
...
`,Jo=class extends HTMLElement{shadow;bookmarks=[];editingId=null;editingAnchor=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${qo}</style>
<div class="backdrop" id="backdrop">
  <div class="dialog" role="dialog" aria-modal="true" aria-label="링크 삽입">
    ...
    `).join(``),this.refreshDatetimePreviews(),e.addEventListener(`click`,e=>{let t=e.target.closest(`[data-preset]`);if(!t)return;let n=Ko[parseInt(t.dataset.preset??`0`)]?.fn()??``;this.dispatch(`poa-datetime-insert`,{text:n}),this.close()}))}refreshDatetimePreviews(){Ko.forEach((e,t)=>{let n=this.shadow.getElementById(`dt-prev-${t}`);n&&(n.textContent=e.fn())})}dispatch(e,t){this.dispatchEvent(new CustomEvent(e,{bubbles:!0,composed:!0,detail:t}))}},Yo=`
:host { display: none; }
:host([open]) {
  display: flex; align-items: center; gap: 6px;
...
`,Xo=class extends HTMLElement{shadow;img=null;aspectLocked=!1;naturalW=0;naturalH=0;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Yo}</style>
<label>너비</label>
...
`,$o=class extends HTMLElement{shadow;resolvePromise=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Zo}</style>${Qo}`,this.bindEvents()}show(e,t=`확인`){let n=this.shadow.getElementById(`dlg-title`),r=this.shadow.getElementById(`dlg-message`);return n&&(n.textContent=t),r&&(r.textContent=e),this.setAttribute(`open`,``),new Promise(e=>{this.resolvePromise=e})}close(e){this.removeAttribute(`open`),this.resolvePromise?.(e),this.resolvePromise=null}bindEvents(){this.shadow.getElementById(`btn-ok`)?.addEventListener(`click`,()=>this.close(!0)),this.shadow.getElementById(`btn-cancel`)?.addEventListener(`click`,()=>this.close(!1)),this.shadow.getElementById(`overlay`)?.addEventListener(`click`,e=>{e.target.id===`overlay`&&this.close(!1)}),this.addEventListener(`keydown`,e=>{e.key===`Escape`&&(e.preventDefault(),this.close(!1))})}},es={error:`●`,warning:`▲`,info:`ℹ`},ts={error:`#DC2626`,warning:`#D97706`,info:`#2563EB`},ns=`
:host { display: none; }
:host([open]) { display: block; }
...
`,rs=class extends HTMLElement{shadow;issues=[];contentEl=null;rerunFn=null;currentFilter=`all`;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${ns}</style>
<div class="overlay" id="overlay">
  <div class="dialog">
    <div class="header">
      <h2>웹 접근성 검사 결과</h2>
...
`,cs=class extends HTMLElement{shadow;items=[];filteredType=`all`;onModified=null;onConfirmFn=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${ss}</style><slot></slot>`}setup(e,t){this.onModified=e,this.onConfirmFn=t}startLoading(){this.setAttribute(`open`,``),this.shadow.innerHTML=`<style>${ss}</style>
<div class="overlay">
  <div class="dialog">
    <div class="header">
      <h2>개인정보 검사</h2>
...
<div class="card" data-level="${e.riskLevel}" data-id="${e.id}">
  <div class="card-header" data-toggle="${e.id}">
    <span class="card-icon">${as[e.riskLevel]}</span>
    <span class="card-label">${e.label}</span>
    <span class="card-risk">${os[e.riskLevel]}</span>
    <span class="card-toggle">∨</span>
  </div>
  <div class="card-body" id="body-${e.id}">
    <div class="raw-text">"${ls(e.raw)}"</div>
...
`,ds=class extends HTMLElement{shadow;currentTable=null;currentRow=1;currentCol=1;pickBar=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${us}</style>`}open(e){this.currentTable=e.table,this.currentRow=e.cellRow,this.currentCol=e.cellCol,this.setAttribute(`open`,``),this.render(e.existingFormula,e.initialRange)}close(){this.removeAttribute(`open`),this.exitPickMode(),this.currentTable=null}applyRange(e,t,n,r){this.exitPickMode();let i=e=>this.shadow.getElementById(e),a=(e,t)=>{let n=i(e);n&&(n.value=String(t))};a(`sr`,e),a(`sc`,t),a(`er`,n),a(`ec`,r)}render(e,t){let n=e?.fn??`SUM`,r=e?.format??`integer`,i=e?.decimalPlaces??2,[a,o,s,c]=e?.range??t??[1,1,1,1],l=e?.targetRow??this.currentRow,u=e?.targetCol??this.currentCol,d=!e||e.targetRow===this.currentRow&&e.targetCol===this.currentCol,f=e?.style?.backgroundColor??`#ffffff`,p=e?.style?.color??`#000000`,m=(e,t)=>`<label><input type="radio" name="fn" value="${e}"${n===e?` checked`:``}> ${e} (${t})</label>`,h=(e,t)=>`<label><input type="radio" name="format" value="${e}"${r===e?` checked`:``}> ${t}</label>`;this.shadow.innerHTML=`<style>${us}</style>
<div class="overlay" id="overlay">
  <div class="dialog" id="dialog">
    <div class="header">
      <h2>계산식 설정</h2>
    ...
    <div class="footer">
      ...
      ">취소</button>`,document.body.appendChild(e),this.pickBar=e,e.querySelector(`#cancel-pick`)?.addEventListener(`click`,()=>this.exitPickMode()),this.dispatchEvent(new CustomEvent(`poa-formula-start-pick`,{bubbles:!0}))}exitPickMode(){this.shadow.getElementById(`overlay`)?.classList.remove(`pick-mode`),this.pickBar?.remove(),this.pickBar=null}};customElements.get(`poa-menubar`)||customElements.define(`poa-menubar`,Yi),customElements.get(`poa-context-toolbar`)||customElements.define(`poa-context-toolbar`,bo),customElements.get(`poa-toolbar`)||customElements.define(`poa-toolbar`,Do),customElements.get(`poa-status-bar`)||customElements.define(`poa-status-bar`,Oo),customElements.get(`poa-settings-dialog`)||customElements.define(`poa-settings-dialog`,jo),customElements.get(`poa-find-replace-dialog`)||customElements.define(`poa-find-replace-dialog`,Po),customElements.get(`poa-image-edit-dialog`)||customElements.define(`poa-image-edit-dialog`,Io),customElements.get(`poa-image-dialog`)||customElements.define(`poa-image-dialog`,zo),customElements.get(`poa-table-dialog`)||customElements.define(`poa-table-dialog`,Go),customElements.get(`poa-cell-split-dialog`)||customElements.define(`poa-cell-split-dialog`,mi),customElements.get(`poa-link-dialog`)||customElements.define(`poa-link-dialog`,Jo),customElements.get(`poa-image-toolbar`)||customElements.define(`poa-image-toolbar`,Xo),customElements.get(`poa-confirm-dialog`)||customElements.define(`poa-confirm-dialog`,$o),customElements.get(`poa-accessibility-dialog`)||customElements.define(`poa-accessibility-dialog`,rs),customElements.get(`poa-privacy-dialog`)||customElements.define(`poa-privacy-dialog`,cs),customElements.get(`poa-formula-dialog`)||customElements.define(`poa-formula-dialog`,ds),customElements.get(`poa-video-dialog`)||customElements.define(`poa-video-dialog`,gi),customElements.get(`poa-form-control-dialog`)||customElements.define(`poa-form-control-dialog`,Si),customElements.get(`poa-template-tree`)||customElements.define(`poa-template-tree`,Oi),customElements.get(`poa-template-dialog`)||customElements.define(`poa-template-dialog`,ji),customElements.get(`poa-signature-dialog`)||customElements.define(`poa-signature-dialog`,Bi),customElements.get(`poa-emoji-dialog`)||customElements.define(`poa-emoji-dialog`,Hi),customElements.get(`poa-tooltip-dialog`)||customElements.define(`poa-tooltip-dialog`,Wi),customElements.get(`poa-input-property-dialog`)||customElements.define(`poa-input-property-dialog`,Ki),customElements.get(`poa-editor`)||customElements.define(`poa-editor`,ci);var $=document.getElementById(`editor`),fs=document.getElementById(`output`),ps=`<h2>poa-editor v1.0.0</h2>
...
<p>표 삽입 → 삽입 메뉴 → 표를 사용해보세요.</p>`;document.getElementById(`btn-get-html`).addEventListener(`click`,()=>{fs.style.display=`block`,fs.textContent=$.getHTML()}),document.getElementById(`btn-set-html`).addEventListener(`click`,()=>{$.setHTML(ps),fs.style.display=`none`}),document.getElementById(`btn-clear`).addEventListener(`click`,()=>{$.setHTML(``),fs.style.display=`none`}),document.getElementById(`btn-find`).addEventListener(`click`,()=>{$.querySelector(`.poa-editor-content`)?.focus(),$.dispatchEvent(new KeyboardEvent(`keydown`,{key:`f`,ctrlKey:!0,bubbles:!0}))}),document.getElementById(`btn-replace`).addEventListener(`click`,()=>{$.querySelector(`.poa-editor-content`)?.focus(),$.dispatchEvent(new KeyboardEvent(`keydown`,{key:`h`,ctrlKey:!0,bubbles:!0}))}),document.getElementById(`btn-undo`).addEventListener(`click`,()=>{$.querySelector(`.poa-editor-content`)?.focus(),document.execCommand(`undo`)}),document.getElementById(`btn-redo`).addEventListener(`click`,()=>{$.querySelector(`.poa-editor-content`)?.focus(),document.execCommand(`redo`)}),setTimeout(()=>$.setHTML(ps),100);