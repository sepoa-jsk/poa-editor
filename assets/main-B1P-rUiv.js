...
${o}
${s}
</head><body>${e}</body></html>`),a.close();let c=()=>{r.remove()};i.onafterprint=c,setTimeout(()=>{r.parentNode&&c()},2e3),i.focus(),i.print()}htmlToPlainText(e){return at(e)}toHtml(e,t){if(e.endsWith(`.txt`))return M(t);let n=j(t);return this.normalizeTableCells(n)}normalizeTableCells(e){let t=new DOMParser().parseFromString(e,`text/html`);return t.querySelectorAll(`td, th`).forEach(e=>{e.style.verticalAlign||(e.style.verticalAlign=`middle`)}),t.body.innerHTML}},jt=(e,t)=>t.some(t=>e instanceof t),Mt,Nt;function P(){return Mt||=[IDBDatabase,IDBObjectStore,IDBIndex,IDBCursor,IDBTransaction]}function Pt(){return Nt||=[IDBCursor.prototype.advance,IDBCursor.prototype.continue,IDBCursor.prototype.continuePrimaryKey]}var Ft=new WeakMap,It=new WeakMap,Lt=new WeakMap;function Rt(e){let t=new Promise((t,n)=>{let r=()=>{e.removeEventListener(`success`,i),e.removeEventListener(`error`,a)},i=()=>{t(I(e.result)),r()},a=()=>{n(e.error),r()};e.addEventListener(`success`,i),e.addEventListener(`error`,a)});return Lt.set(t,e),t}function zt(e){if(Ft.has(e))return;let t=new Promise((t,n)=>{let r=()=>{e.removeEventListener(`complete`,i),e.removeEventListener(`error`,a),e.removeEventListener(`abort`,a)},i=()=>{t(),r()},a=()=>{n(e.error||new DOMException(`AbortError`,`AbortError`)),r()};e.addEventListener(`complete`,i),e.addEventListener(`error`,a),e.addEventListener(`abort`,a)});Ft.set(e,t)}var Bt={get(e,t,n){if(e instanceof IDBTransaction){if(t===`done`)return Ft.get(e);if(t===`store`)return n.objectStoreNames[1]?void 0:n.objectStore(n.objectStoreNames[0])}return I(e[t])},set(e,t,n){return e[t]=n,!0},has(e,t){return e instanceof IDBTransaction&&(t===`done`||t===`store`)?!0:t in e}};function Vt(e){Bt=e(Bt)}function Ht(e){return Pt().includes(e)?function(...t){return e.apply(Ut(this),t),I(this.request)}:function(...t){return I(e.apply(Ut(this),t))}}function F(e){return typeof e==`function`?Ht(e):(e instanceof IDBTransaction&&zt(e),jt(e,P())?new Proxy(e,Bt):e)}function I(e){if(e instanceof IDBRequest)return Rt(e);if(It.has(e))return It.get(e);let t=F(e);return t!==e&&(It.set(e,t),Lt.set(t,e)),t}var Ut=e=>Lt.get(e);function Wt(e,t,{blocked:n,upgrade:r,blocking:i,terminated:a}={}){let o=indexedDB.open(e,t),s=I(o);return r&&o.addEventListener(`upgradeneeded`,e=>{r(I(o.result),e.oldVersion,e.newVersion,I(o.transaction),e)}),n&&o.addEventListener(`blocked`,e=>n(e.oldVersion,e.newVersion,e)),s.then(e=>{a&&e.addEventListener(`close`,()=>a()),i&&e.addEventListener(`versionchange`,e=>i(e.oldVersion,e.newVersion,e))}).catch(()=>{}),s}var Gt=[`get`,`getKey`,`getAll`,`getAllKeys`,`count`],Kt=[`put`,`add`,`delete`,`clear`],qt=new Map;function Jt(e,t){if(!(e instanceof IDBDatabase&&!(t in e)&&typeof t==`string`))return;if(qt.get(t))return qt.get(t);let n=t.replace(/FromIndex$/,``),r=t!==n,i=Kt.includes(n);if(!(n in(r?IDBIndex:IDBObjectStore).prototype)||!(i||Gt.includes(n)))return;let a=async function(e,...t){let a=this.transaction(e,i?`readwrite`:`readonly`),o=a.store;return r&&(o=o.index(t.shift())),(await Promise.all([o[n](...t),i&&a.done]))[0]};return qt.set(t,a),a}Vt(e=>({...e,get:(t,n,r)=>Jt(t,n)||e.get(t,n,r),has:(t,n)=>!!Jt(t,n)||e.has(t,n)}));var Yt=[`continue`,`continuePrimaryKey`,`advance`],Xt={},Zt=new WeakMap,Qt=new WeakMap,$t={get(e,t){if(!Yt.includes(t))return e[t];let n=Xt[t];return n||=Xt[t]=function(...e){Zt.set(this,Qt.get(this)[t](...e))},n}};async function*en(...e){let t=this;if(t instanceof IDBCursor||(t=await t.openCursor(...e)),!t)return;t=t;let n=new Proxy(t,$t);for(Qt.set(n,t),Lt.set(n,Ut(t));t;)yield n,t=await(Zt.get(n)||t.continue()),Zt.delete(n)}function L(e,t){return t===Symbol.asyncIterator&&jt(e,[IDBIndex,IDBObjectStore,IDBCursor])||t===`iterate`&&jt(e,[IDBIndex,IDBObjectStore])}Vt(e=>({...e,get(t,n,r){return L(t,n)?en:e.get(t,n,r)},has(t,n){return L(t,n)||e.has(t,n)}}));var tn=`poa-editor-autosave`,nn=`snapshots`,rn=10,an=300*1e3,on=class{timer=null;intervalMs;constructor(e=an){this.intervalMs=e}start(e){this.stop(),this.timer=setInterval(()=>{this.saveNow(e())},this.intervalMs)}stop(){this.timer!==null&&(clearInterval(this.timer),this.timer=null)}isRunning(){return this.timer!==null}async saveNow(e){let t=(await this.openDB()).transaction(nn,`readwrite`);await t.store.put({html:e,savedAt:Date.now()});let n=await t.store.getAllKeys();n.length>rn&&await t.store.delete(n[0]),await t.done,k.emit(A.AUTOSAVE_SAVED,{savedAt:Date.now()})}async listSnapshots(){let e=await this.openDB(),[t,n]=await Promise.all([e.getAllKeys(nn),e.getAll(nn)]);return n.map((e,n)=>({id:t[n],html:e.html,savedAt:e.savedAt})).reverse()}async restoreSnapshot(e){return(await(await this.openDB()).get(nn,e))?.html??null}async clearAll(){await(await this.openDB()).clear(nn)}async openDB(){return Wt(tn,1,{upgrade(e){e.objectStoreNames.contains(nn)||e.createObjectStore(nn,{autoIncrement:!0})}})}},sn=`poa-settings`,R={fontFamily:`맑은 고딕`,fontSize:11,lineHeight:1.5,autoSaveEnabled:!0,autoSaveInterval:5,spellCheck:!1,changeWarning:!0};function cn(){try{let e=localStorage.getItem(sn);return e?{...R,...JSON.parse(e)}:{...R}}catch{return{...R}}}function ln(e){try{localStorage.setItem(sn,JSON.stringify(e))}catch{}}var un=[`맑은 고딕`,`나눔고딕`,`나눔명조`,`굴림`,`돋움`,`바탕`,`궁서`,`Arial`,`Georgia`,`Times New Roman`,`Verdana`],dn=[8,9,10,11,12,14,16,18,20,24,28,32],fn=[1,1.2,1.4,1.5,1.6,1.8,2,2.5],pn=[1,2,3,5,10,15,30],mn=`
:host { display: none; }
...
`;function hn(e,t,n){return`<option value="${e}"${n?` selected`:``}>${t}</option>`}var gn=class extends HTMLElement{shadow;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${mn}</style>
    ...
    <div class="body">
      <div class="section">
        ...
        <div class="row">
          ...
          <div class="row-ctrl">
            <select id="sel-font-family">${un.map(e=>hn(e,e,!1)).join(``)}</select>
          </div>
        </div>
        <div class="row">
          <span class="row-label">글자 크기</span>
          <div class="row-ctrl">
            <select id="sel-font-size">${dn.map(e=>hn(String(e),`${e}pt`,!1)).join(``)}</select>
          </div>
        </div>
        <div class="row">
          <span class="row-label">줄 간격</span>
          <div class="row-ctrl">
            <select id="sel-line-height">${fn.map(e=>hn(String(e),String(e),!1)).join(``)}</select>
          </div>
        </div>
      </div>
      <div class="section">
        ...
        <div class="row" id="row-interval">
          ...
          <div class="row-ctrl">
            <select id="sel-interval">${pn.map(e=>hn(String(e),`${e}분`,!1)).join(``)}</select>
          </div>
        </div>
      </div>
      <div class="section">
    ...
    <div class="pf-row2">
      <div class="pf-field" style="margin-bottom:0;">
        ...
        <div style="display:flex;align-items:center;gap:4px;">
          <input id="pf-width" type="number" value="${ue}" min="60" max="800" step="1"
      ...
      <div class="pf-field" style="margin-bottom:0;">
        ...
        <div style="display:flex;align-items:center;gap:4px;">
          <input id="pf-height" type="number" value="${de}" min="28" max="600" step="1"
            style="width:70px;" class="pf-input">
          <span style="font-size:12px;color:#6B7280;">px</span>
        </div>
      </div>`:`<div></div>`}
...
</div>`,b=n.defaultView,pe=b?b.innerWidth:1920,x=b?b.innerHeight:1080,me=r.left,S=r.bottom+6;S+340>x&&(S=r.top-340-6),me+320>pe&&(me=r.right-320),me=Math.max(4,me),S=Math.max(4,S);let C=n.createElement(`div`);C.className=`poa-field-popup`,C.style.cssText=[`position:fixed`,`left:${Math.round(me)}px`,`top:${Math.round(S)}px`,`width:320px`,`background:#fff`,`border:1px solid #E2E8F0`,`border-radius:12px`,`box-shadow:0 10px 40px rgba(0,0,0,.15),0 2px 8px rgba(0,0,0,.08)`,`z-index:99999`,`font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif`,`font-size:13px`,`overflow:hidden`].join(`;`),C.innerHTML=`
  ...
  <div id="pf-body-font" class="pf-sec-body">
    <div class="pf-row2">
      <div class="pf-field">
        ...
        <select id="pf-fontsize" class="pf-select">${y}${re}</select>
      </div>
      <div class="pf-field">
