...
`,Ci=class extends HTMLElement{shadow;targetCell=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Si}</style>
...
`,Ti=class extends HTMLElement{shadow;activeTab=`video`;constructor(){super(),this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${wi}</style>`}open(e=`video`){this.activeTab=e,this.setAttribute(`open`,``),this.render()}close(){this.removeAttribute(`open`)}render(){this.shadow.innerHTML=`<style>${wi}</style>
<div class="overlay" id="overlay">
  ...
  </svg>`},Di={text:`텍스트`,textarea:`여러 줄`,checkbox:`체크박스`,radio:`라디오`,select:`목록 선택`,button:`버튼`,date:`날짜 선택`},Oi=[`text`,`textarea`,`checkbox`,`radio`,`select`,`button`,`date`],ki=`
    ...
    <div class="adv-body" data-adv-body>${e}</div>`}_secText(){return`<div class="specific-section" id="sec-text">
      ...
      ${this._adv(`
        <div class="field">
          ...
          ${Ai(`f-btn-style`,[[`default`,`기본 (회색)`],[`primary`,`강조 (파랑)`],[`danger`,`경고 (빨강)`]])}
        </div>
...
`,qi=class extends HTMLElement{shadow;inserter=new Mr;activeCatId=kr[0].id;connectedCallback(){this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${Ki}</style>
<div class="dlg">
...
.hidden { display: none !important; }
`,Yi=class extends HTMLElement{shadow;mode=`add`;editId=null;entries=[];connectedCallback(){this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${Ji}</style>
<div class="dlg">
  <div class="hdr">
...
`,Zi=class extends HTMLElement{shadow;targetInput=null;connectedCallback(){this.shadow||(this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${Xi}</style>${this._tpl()}`,this._bindEvents())}open(e){this.shadow||this.connectedCallback(),this.targetInput=e,this._populate(e),this.setAttribute(`open`,``)}close(){this.removeAttribute(`open`)}_tpl(){return`<div class="dlg">
  <div class="hdr">
    <h3>입력 요소 속성</h3>
    <button class="x-btn" aria-label="닫기">✕</button>
  </div>
...
`,ea=new Set([`edit`,`insert`,`table`,`format`,`misc`]),ta=class extends HTMLElement{shadow;_activeTab=`edit`;_userMode=!1;busHandler=({tab:e})=>{this._activeTab=e,this.updateActive()};constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){let e=Qi.map(e=>`<button class="tab${e.id===this._activeTab?` active`:``}" data-tab="${e.id}">${e.label}</button>`).join(``);this.shadow.innerHTML=`<style>${$i}</style><div class="menubar">${e}</div>`,this.shadow.querySelector(`.menubar`).addEventListener(`mousedown`,e=>{e.preventDefault();let t=e.target.dataset.tab;t&&P.emit(F.MENUBAR_CHANGE,{tab:t})}),P.on(F.MENUBAR_CHANGE,this.busHandler)}disconnectedCallback(){P.off(F.MENUBAR_CHANGE,this.busHandler)}applyUserMode(){this._userMode=!0,ea.has(this._activeTab)&&P.emit(F.MENUBAR_CHANGE,{tab:`file`}),this._applyUserModeStyles()}_applyUserModeStyles(){if(!this._userMode)return;this.shadow.querySelectorAll(`.tab`).forEach(e=>{let t=e.dataset.tab;ea.has(t)&&(e.dataset.userDisabled=`true`)});let e=this.shadow.querySelector(`.menubar`);if(!e.querySelector(`.user-mode-badge`)){let t=document.createElement(`span`);t.className=`user-mode-badge`,t.textContent=`사용자 모드`,e.appendChild(t)}}updateActive(){this.shadow.querySelectorAll(`.tab`).forEach(e=>{e.classList.toggle(`active`,e.dataset.tab===this._activeTab)})}},na=`
<svg
...
`;function Q(e){return e.replace(/width="24"/,`width="16"`).replace(/height="24"/,`height="16"`)}var $={bold:Q(la),italic:Q(Ma),underline:Q(ko),strike:Q(go),undo:Q(Ao),redo:Q(to),alignLeft:Q(Eo),alignCenter:Q(Co),alignRight:Q(wo),alignJustify:Q(To),indent:Q(Ba),outdent:Q(za),foreColor:Q(sa),backColor:Q(Da),lineHeight:Q(ca),letterSpacing:Q(Ja),fileNew:Q(Sa),fileOpen:Q(Ta),save:Q(ao),fileSaveAs:Q(xa),print:Q($a),settings:Q(lo),clock:Q(ga),cut:Q(oo),copy:Q(ya),paste:Q(ha),pastePlain:Q(ma),selectAll:Q(ho),search:Q(so),imageEdit:Q(Oa),image:Q(ka),images:Q(Aa),videoTag:Q(wa),embedVideo:Q(pa),link:Q(Ra),linkOff:Q(La),bookmark:Q(da),signature:Q(Qa),emoji:Q(po),tooltip:Q(Wa),tooltipList:Q(Ga),calendar:Q(fa),hr:Q(Ka),symbol:Q(Ya),youtube:Q(qa),pageBreak:Q(co),template:Q(Ia),formField:Q(eo),viewDesign:Q(Pa),viewHtml:Q(_a),viewPreview:Q(ba),viewText:Q(Oo),viewPage:Q(Ca),fullscreen:Q(Ua),ruler:Q(io),grid:Q(Ea),hiddenBorder:Q(ho),table:Q(So),tableProps:Q(xo),cellProps:Q(Fa),mergeCells:Q(yo),splitCell:Q(bo),rowAbove:Q(oa),rowBelow:Q(ra),colLeft:Q(ia),colRight:Q(aa),rowDelete:Q(ro),colDelete:Q(va),tableDelete:Q(Do),painterCopy:Q(Xa),painterPaste:Q(Za),formatClear:Q(no),ul:Q(Ha),ol:Q(Va),sup:Q(vo),sub:Q(_o),a11y:Q(na),privacy:Q(uo),form:Q(mo),calc:Q(fo),userMode:Q(jo),shortcuts:Q(Na),guide:Q(ua),about:Q(ja)},Mo={"file:new":`fileNew`,"file:open":`fileOpen`,"file:save":`save`,"file:saveas":`fileSaveAs`,"file:print":`print`,settings:`settings`,"edit:cut":`cut`,"edit:copy":`copy`,"edit:paste":`paste`,"edit:paste-plain":`pastePlain`,"edit:select-all":`selectAll`,"find-replace":`search`,"edit:image-edit":`imageEdit`,image:`image`,"insert:multi-image":`images`,"insert:video":`videoTag`,"insert:embed":`embedVideo`,"insert:link":`link`,"insert:link-remove":`linkOff`,"insert:bookmark":`bookmark`,"insert:signature":`signature`,"insert:emoji":`emoji`,"insert:tooltip":`tooltip`,"insert:tooltip-list":`tooltipList`,"insert:datetime":`calendar`,"insert:hr":`hr`,"insert:symbol":`symbol`,"insert:pagebreak":`pageBreak`,"misc:template":`template`,"view:design":`viewDesign`,"view:html":`viewHtml`,"view:preview":`viewPreview`,"view:text":`viewText`,"view:page":`viewPage`,"view:fullscreen":`fullscreen`,"view:ruler":`ruler`,"view:grid":`grid`,"view:hidden-border":`hiddenBorder`,table:`table`,"table:table-props":`tableProps`,"table:cell-props":`cellProps`,"table:merge":`mergeCells`,"table:split-cell":`splitCell`,"table:row-above":`rowAbove`,"table:row-below":`rowBelow`,"table:col-left":`colLeft`,"table:col-right":`colRight`,"table:row-delete":`rowDelete`,"table:col-delete":`colDelete`,"table:delete":`tableDelete`,"table:align-left":`alignLeft`,"table:align-center":`alignCenter`,"table:align-right":`alignRight`,"format:painter-copy":`painterCopy`,"format:painter-paste":`painterPaste`,"format:clear":`formatClear`,"format:ul":`ul`,"format:ol":`ol`,"format:sup":`sup`,"format:sub":`sub`,"misc:a11y":`a11y`,"misc:privacy":`privacy`,"misc:form":`form`,"misc:calc":`calc`,"misc:user-mode":`userMode`,"help:shortcuts":`shortcuts`,"help:guide":`guide`,"help:about":`about`},No={text:`T`,number:`#`,date:`📅`};function Po(){let e=[`기본`,`계약`,`금액`,`보증`,`기타`],t=[],n=!0;for(let r of e){let e=ei.filter(e=>e.category===r);if(e.length!==0){n||t.push([``,`__sep__`]),t.push([r,`__cat__`]),n=!1;for(let n of e)t.push([n.label,`insert:field`,n.id,No[n.type]])}}return t}var Fo={file:[[[`새 문서`,`file:new`],[`열기`,`file:open`],[`저장`,`file:save`],[`다른 이름으로 저장`,`file:saveas`]],[[`인쇄`,`file:print`],[`환경설정`,`settings`]]],edit:[[[`잘라내기`,`edit:cut`],[`복사`,`edit:copy`],[`붙여넣기`,`edit:paste`],[`서식 없이 붙여넣기`,`edit:paste-plain`]],[[`찾기·바꾸기`,`find-replace`],[`이미지 편집`,`edit:image-edit`],[`전체 선택`,`edit:select-all`]]],insert:[[[`이미지`,`image`],[`다중 이미지`,`insert:multi-image`]],[[`비디오 태그`,`insert:video`],[`외부 동영상`,`insert:embed`]],[[`하이퍼링크`,`insert:link`],[`책갈피`,`insert:bookmark`]],[[`서명`,`insert:signature`],[`이모지`,`insert:emoji`]],[[`툴팁`,`insert:tooltip`],[`툴팁 관리`,`insert:tooltip-list`]],[[`날짜·시간`,`insert:datetime`],[`가로줄`,`insert:hr`],[`기호`,`insert:symbol`],[`페이지 구분선`,`insert:pagebreak`],[`템플릿`,`misc:template`]],[{dropdown:!0,id:`doc-field`,label:`양식 필드`,iconKey:`formField`,items:Po()}]],view:[[[`디자인`,`view:design`],[`HTML`,`view:html`],[`미리보기`,`view:preview`],[`텍스트`,`view:text`],[`페이지`,`view:page`]],[[`전체화면`,`view:fullscreen`],[`눈금자`,`view:ruler`],[`그리드`,`view:grid`],[`숨김 테두리`,`view:hidden-border`]]],table:[[[`표 삽입`,`table`],[`표 속성`,`table:table-props`],[`셀 속성`,`table:cell-props`]],[[`셀 병합`,`table:merge`],[`셀 나누기`,`table:split-cell`]],[[`위에 행 삽입`,`table:row-above`],[`아래에 행 삽입`,`table:row-below`],[`왼쪽에 열 삽입`,`table:col-left`],[`오른쪽에 열 삽입`,`table:col-right`]],[[`행 삭제`,`table:row-delete`],[`열 삭제`,`table:col-delete`],[`표 삭제`,`table:delete`]],[[`표 왼쪽 정렬`,`table:align-left`],[`표 가운데 정렬`,`table:align-center`],[`표 오른쪽 정렬`,`table:align-right`]]],format:[[[`서식 복사`,`format:painter-copy`],[`서식 붙여넣기`,`format:painter-paste`],[`서식 제거`,`format:clear`]],[[`글머리 기호`,`format:ul`],[`글머리 번호`,`format:ol`]],[[`위 첨자`,`format:sup`],[`아래 첨자`,`format:sub`]]],misc:[[[`웹 접근성 체크`,`misc:a11y`],[`개인정보 체크`,`misc:privacy`]],[[`폼 컨트롤`,`misc:form`],[`계산식`,`misc:calc`],[`템플릿`,`misc:template`]],[[`사용자 모드로 보기`,`misc:user-mode`]]],help:[[[`단축키`,`help:shortcuts`],[`사용자 가이드`,`help:guide`],[`제품 정보`,`help:about`]]]},Io=new Set([`view:design`,`view:html`,`view:preview`,`view:text`,`view:page`]),Lo=`
:host {
  display: block;
  --icon-color:         #374151;
...
`;function Yo(e){return new Date(e).toLocaleString(`ko-KR`,{month:`2-digit`,day:`2-digit`,hour:`2-digit`,minute:`2-digit`,second:`2-digit`})}var Xo=class extends HTMLElement{shadow;autoSave=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Jo}</style>
  ...
  <div class="dialog" role="dialog" aria-modal="true" aria-label="설정">
    ...
    <div class="body">
      ...
      <div class="section">
        ...
        <span class="history-time">${Yo(n.savedAt)}</span>
...
`,ts=class extends HTMLElement{shadow;editor=new vi;originalSrc=``;currentDataUrl=``;busy=!1;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${es}</style>
<div class="backdrop" id="backdrop">
...
`;function rs(e){return e<1024*1024?`${(e/1024).toFixed(1)}KB`:`${(e/1024/1024).toFixed(1)}MB`}var is=class extends HTMLElement{shadow;uploader=new xi;uploadConfig=null;selectedFiles=[];busy=!1;onErrorFn=null;setOnError(e){this.onErrorFn=e}constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${ns}</style>
<div class="backdrop" id="backdrop">
...
.grid-picker {
  display: grid;
  grid-template-columns: repeat(${as}, ${ss}px);
  gap: ${cs}px;
  cursor: crosshair;
}
.gc {
...
`,us=class e extends HTMLElement{shadow;selectedPreset=`border-all`;gridCells=[];constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${ls}</style>
<div class="overlay" id="overlay">
  <div class="dialog" role="dialog" aria-modal="true">
    <div class="dlg-hd">
...
</div>`;let l=s;t.querySelector(`#pp-align-btns`).addEventListener(`click`,e=>{let n=e.target.closest(`.align-btn`);n&&(l=n.dataset.align,t.querySelectorAll(`.align-btn`).forEach(e=>{e.classList.toggle(`active`,e.dataset.align===l)}))}),t.querySelector(`#pp-cancel`).addEventListener(`click`,()=>this.close()),t.querySelector(`#pp-ok`).addEventListener(`click`,()=>{let e={width:t.querySelector(`#pp-w`).value.trim(),borderColor:t.querySelector(`#pp-bc`).value,bgColor:t.querySelector(`#pp-bg`).value,align:l};this.dispatchEvent(new CustomEvent(`poa-table-update`,{bubbles:!0,composed:!0,detail:{options:e,table:n}})),this.close()})}static rgbToHex(e){let t=e.match(/\d+/g);return!t||t.length<3?``:`#`+t.slice(0,3).map(e=>parseInt(e).toString(16).padStart(2,`0`)).join(``)}static colorToHex(t){return e.rgbToHex(t)||`#ffffff`}},ds=[{label:`날짜 (2025-05-05)`,fn:()=>new Date().toISOString().slice(0,10)},{label:`시간 (14:30:05)`,fn:()=>new Intl.DateTimeFormat(`ko-KR`,{hour:`2-digit`,minute:`2-digit`,second:`2-digit`,hour12:!1}).format(new Date)},{label:`날짜·시간 (2025-05-05 14:30)`,fn:()=>{let e=new Date;return`${e.toISOString().slice(0,10)} ${new Intl.DateTimeFormat(`ko-KR`,{hour:`2-digit`,minute:`2-digit`,hour12:!1}).format(e)}`}},{label:`한국어 날짜 (2025년 5월 5일)`,fn:()=>new Intl.DateTimeFormat(`ko-KR`,{year:`numeric`,month:`long`,day:`numeric`}).format(new Date)},{label:`한국어 날짜·시간 (2025년 5월 5일 월요일 오후 2:30)`,fn:()=>new Intl.DateTimeFormat(`ko-KR`,{year:`numeric`,month:`long`,day:`numeric`,weekday:`long`,hour:`numeric`,minute:`2-digit`}).format(new Date)}],fs=`
:host { display: none; }
:host([open]) { display: block; }
.backdrop {
  position: fixed; inset: 0; background: rgba(0,0,0,.45);
...
`,ps=class extends HTMLElement{shadow;bookmarks=[];editingId=null;editingAnchor=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${fs}</style>
<div class="backdrop" id="backdrop">
  <div class="dialog" role="dialog" aria-modal="true" aria-label="링크 삽입">
    ...
    `).join(``),this.refreshDatetimePreviews(),e.addEventListener(`click`,e=>{let t=e.target.closest(`[data-preset]`);if(!t)return;let n=ds[parseInt(t.dataset.preset??`0`)]?.fn()??``;this.dispatch(`poa-datetime-insert`,{text:n}),this.close()}))}refreshDatetimePreviews(){ds.forEach((e,t)=>{let n=this.shadow.getElementById(`dt-prev-${t}`);n&&(n.textContent=e.fn())})}dispatch(e,t){this.dispatchEvent(new CustomEvent(e,{bubbles:!0,composed:!0,detail:t}))}},ms=`
:host { display: none; }
:host([open]) {
  display: flex; align-items: center; gap: 6px;
...
`,hs=class extends HTMLElement{shadow;img=null;aspectLocked=!1;naturalW=0;naturalH=0;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${ms}</style>
<label>너비</label>
...
`,vs=class extends HTMLElement{shadow;resolvePromise=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${gs}</style>${_s}`,this.bindEvents()}show(e,t=`확인`){let n=this.shadow.getElementById(`dlg-title`),r=this.shadow.getElementById(`dlg-message`);return n&&(n.textContent=t),r&&(r.textContent=e),this.setAttribute(`open`,``),new Promise(e=>{this.resolvePromise=e})}close(e){this.removeAttribute(`open`),this.resolvePromise?.(e),this.resolvePromise=null}bindEvents(){this.shadow.getElementById(`btn-ok`)?.addEventListener(`click`,()=>this.close(!0)),this.shadow.getElementById(`btn-cancel`)?.addEventListener(`click`,()=>this.close(!1)),this.shadow.getElementById(`overlay`)?.addEventListener(`click`,e=>{e.target.id===`overlay`&&this.close(!1)}),this.addEventListener(`keydown`,e=>{e.key===`Escape`&&(e.preventDefault(),this.close(!1))})}},ys={error:`●`,warning:`▲`,info:`ℹ`},bs={error:`#DC2626`,warning:`#D97706`,info:`#2563EB`},xs=`
:host { display: none; }
:host([open]) { display: block; }
...
`,Ss=class extends HTMLElement{shadow;issues=[];contentEl=null;rerunFn=null;currentFilter=`all`;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${xs}</style>
<div class="overlay" id="overlay">
  <div class="dialog">
    <div class="header">
      <h2>웹 접근성 검사 결과</h2>
    ...
    <div class="issue-list" id="issue-list"></div>
      ...
      </div>`;return}e.innerHTML=this.issues.map((e,t)=>this.renderCard(e,t)).join(``),this.applyFilter(),this.bindCardEvents()}renderCard(e,t){let n=bs[e.level],r=ys[e.level],i=this.buildFixPanelHTML(e,t);return`
<div class="issue-card" data-level="${e.level}" data-idx="${t}">
  <div class="issue-header">
    <span class="issue-icon" style="color:${n}">${r}</span>
    <span class="issue-title">${Cs(e.title)}</span>
    <span class="issue-arrow">∨</span>
  </div>
  <div class="issue-body">
    <p class="issue-message">${Cs(e.message)}</p>
...
<div class="card" data-level="${e.riskLevel}" data-id="${e.id}">
  <div class="card-header" data-toggle="${e.id}">
    <span class="card-icon">${ws[e.riskLevel]}</span>
    <span class="card-label">${e.label}</span>
    <span class="card-risk">${Ts[e.riskLevel]}</span>
    <span class="card-toggle">∨</span>
  </div>
  <div class="card-body" id="body-${e.id}">
    <div class="raw-text">"${Os(e.raw)}"</div>
...
`,As=class extends HTMLElement{shadow;currentTable=null;currentRow=1;currentCol=1;pickBar=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${ks}</style>`}open(e){this.currentTable=e.table,this.currentRow=e.cellRow,this.currentCol=e.cellCol,this.setAttribute(`open`,``),this.render(e.existingFormula,e.initialRange)}close(){this.removeAttribute(`open`),this.exitPickMode(),this.currentTable=null}applyRange(e,t,n,r){this.exitPickMode();let i=e=>this.shadow.getElementById(e),a=(e,t)=>{let n=i(e);n&&(n.value=String(t))};a(`sr`,e),a(`sc`,t),a(`er`,n),a(`ec`,r)}render(e,t){let n=e?.fn??`SUM`,r=e?.format??`integer`,i=e?.decimalPlaces??2,[a,o,s,c]=e?.range??t??[1,1,1,1],l=e?.targetRow??this.currentRow,u=e?.targetCol??this.currentCol,d=!e||e.targetRow===this.currentRow&&e.targetCol===this.currentCol,f=e?.style?.backgroundColor??`#ffffff`,p=e?.style?.color??`#000000`,m=(e,t)=>`<label><input type="radio" name="fn" value="${e}"${n===e?` checked`:``}> ${e} (${t})</label>`,h=(e,t)=>`<label><input type="radio" name="format" value="${e}"${r===e?` checked`:``}> ${t}</label>`;this.shadow.innerHTML=`<style>${ks}</style>
<div class="overlay" id="overlay">
  <div class="dialog" id="dialog">
    <div class="header">
      <h2>계산식 설정</h2>
    ...
    <div class="footer">
      ...
      ">취소</button>`,document.body.appendChild(e),this.pickBar=e,e.querySelector(`#cancel-pick`)?.addEventListener(`click`,()=>this.exitPickMode()),this.dispatchEvent(new CustomEvent(`poa-formula-start-pick`,{bubbles:!0}))}exitPickMode(){this.shadow.getElementById(`overlay`)?.classList.remove(`pick-mode`),this.pickBar?.remove(),this.pickBar=null}};customElements.get(`poa-menubar`)||customElements.define(`poa-menubar`,ta),customElements.get(`poa-context-toolbar`)||customElements.define(`poa-context-toolbar`,zo),customElements.get(`poa-toolbar`)||customElements.define(`poa-toolbar`,Ko),customElements.get(`poa-status-bar`)||customElements.define(`poa-status-bar`,qo),customElements.get(`poa-settings-dialog`)||customElements.define(`poa-settings-dialog`,Xo),customElements.get(`poa-find-replace-dialog`)||customElements.define(`poa-find-replace-dialog`,$o),customElements.get(`poa-image-edit-dialog`)||customElements.define(`poa-image-edit-dialog`,ts),customElements.get(`poa-image-dialog`)||customElements.define(`poa-image-dialog`,is),customElements.get(`poa-table-dialog`)||customElements.define(`poa-table-dialog`,us),customElements.get(`poa-cell-split-dialog`)||customElements.define(`poa-cell-split-dialog`,Ci),customElements.get(`poa-link-dialog`)||customElements.define(`poa-link-dialog`,ps),customElements.get(`poa-image-toolbar`)||customElements.define(`poa-image-toolbar`,hs),customElements.get(`poa-confirm-dialog`)||customElements.define(`poa-confirm-dialog`,vs),customElements.get(`poa-accessibility-dialog`)||customElements.define(`poa-accessibility-dialog`,Ss),customElements.get(`poa-privacy-dialog`)||customElements.define(`poa-privacy-dialog`,Ds),customElements.get(`poa-formula-dialog`)||customElements.define(`poa-formula-dialog`,As),customElements.get(`poa-video-dialog`)||customElements.define(`poa-video-dialog`,Ti),customElements.get(`poa-form-control-dialog`)||customElements.define(`poa-form-control-dialog`,ji),customElements.get(`poa-template-tree`)||customElements.define(`poa-template-tree`,Ni),customElements.get(`poa-template-dialog`)||customElements.define(`poa-template-dialog`,Ii),customElements.get(`poa-signature-dialog`)||customElements.define(`poa-signature-dialog`,Gi),customElements.get(`poa-emoji-dialog`)||customElements.define(`poa-emoji-dialog`,qi),customElements.get(`poa-tooltip-dialog`)||customElements.define(`poa-tooltip-dialog`,Yi),customElements.get(`poa-input-property-dialog`)||customElements.define(`poa-input-property-dialog`,Zi),customElements.get(`poa-editor`)||customElements.define(`poa-editor`,_i);var js=class{mgr;constructor(e){this.container=e,this.mgr=new pi}renderTemplateSelector(){this.container.innerHTML=``;let e=document.createElement(`h2`);e.className=`user-mode-selector-title`,e.textContent=`양식 선택`,this.container.appendChild(e);let t=document.createElement(`div`);t.className=`user-mode-card-grid`;let n=this.mgr.getAll().filter(e=>e.type===`template`&&!e.isTemp);if(n.length===0){let e=document.createElement(`p`);e.style.cssText=`color:#9ca3af;font-size:14px;`,e.textContent=`저장된 템플릿이 없습니다.`,t.appendChild(e)}else for(let e of n)t.appendChild(this._buildCard(e));this.container.appendChild(t)}_buildCard(e){let t=document.createElement(`div`);t.className=`user-mode-card`;let n=document.createElement(`div`);n.className=`user-mode-card-icon`,n.textContent=`📄`;let r=document.createElement(`div`);return r.className=`user-mode-card-name`,r.textContent=e.name,t.appendChild(n),t.appendChild(r),t.addEventListener(`click`,()=>{window.location.href=li(e.id)}),t}};if(oi()===`user`){let e=si();if(e){let t=new pi().getById(e),n=document.getElementById(`editor`);t?.content&&n.setHTML(t.content),n.enterUserMode(t?.name??`문서`)}else document.getElementById(`editor-root`).classList.add(`hidden`),document.body.classList.add(`user-mode`),new js(document.getElementById(`user-mode-root`)).renderTemplateSelector()}var Ms=document.getElementById(`editor`),Ns=document.getElementById(`output`),Ps=`<h2>poa-editor v1.0.0</h2>
...
<p>표 삽입 → 삽입 메뉴 → 표를 사용해보세요.</p>`;document.getElementById(`btn-get-html`).addEventListener(`click`,()=>{Ns.style.display=`block`,Ns.textContent=Ms.getHTML()}),document.getElementById(`btn-set-html`).addEventListener(`click`,()=>{Ms.setHTML(Ps),Ns.style.display=`none`}),document.getElementById(`btn-clear`).addEventListener(`click`,()=>{Ms.setHTML(``),Ns.style.display=`none`}),document.getElementById(`btn-find`).addEventListener(`click`,()=>{Ms.querySelector(`.poa-editor-content`)?.focus(),Ms.dispatchEvent(new KeyboardEvent(`keydown`,{key:`f`,ctrlKey:!0,bubbles:!0}))}),document.getElementById(`btn-replace`).addEventListener(`click`,()=>{Ms.querySelector(`.poa-editor-content`)?.focus(),Ms.dispatchEvent(new KeyboardEvent(`keydown`,{key:`h`,ctrlKey:!0,bubbles:!0}))}),document.getElementById(`btn-undo`).addEventListener(`click`,()=>{Ms.querySelector(`.poa-editor-content`)?.focus(),document.execCommand(`undo`)}),document.getElementById(`btn-redo`).addEventListener(`click`,()=>{Ms.querySelector(`.poa-editor-content`)?.focus(),document.execCommand(`redo`)}),oi()!==`user`&&setTimeout(()=>Ms.setHTML(Ps),100);