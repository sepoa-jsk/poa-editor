...
function buildFieldDropdownItems() {
...
const VIEW_MODE_ACTIONS = new Set(['view:design', 'view:html', 'view:preview', 'view:text', 'view:page']);
const CSS = `
:host {
  display: block;
  --icon-color:         #374151;
...
export class PoaContextToolbar extends HTMLElement {
    ...
    connectedCallback() {
        this.render();
        eventBus.on(BusEvent.MENUBAR_CHANGE, this.busHandler);
        eventBus.on(BusEvent.VIEW_CHANGE, this.viewHandler);
    }
    disconnectedCallback() {
        eventBus.off(BusEvent.MENUBAR_CHANGE, this.busHandler);
        eventBus.off(BusEvent.VIEW_CHANGE, this.viewHandler);
    ...
    render() {
        ...
        if (isWrite && this.activeTab === 'insert') {
            rawGroups = rawGroups.map(group => group.filter(item => {
                if (item === null)
                    return true;
                if ('dropdown' in item && item.dropdown)
                    return item.id !== 'doc-field';
                return true;
            ...
            for (const item of groups[gi]) {
                if (item === null) {
                    ...
                    const menuItems = dynItems.map(([label, action, value, typeIcon]) => {
                        if (action === '__sep__')
                            return `<hr class="drop-sep">`;
                        if (action === '__cat__')
                            return `<div class="drop-cat">${label}</div>`;
                    ...
                    const ddIconSvg = item.iconKey ? Icons[item.iconKey] ?? '' : '';
                    const ddBtnContent = ddIconSvg ? `${ddIconSvg}<span>${item.label}</span>` : item.label;
                    parts.push(`<div class="dropdown-wrap" id="dd-${item.id}">` +
                    ...
                    const [label, action, value, title] = item;
                    const da = value ? ` data-value="${value}"` : '';
                    ...
                    parts.push(`<button class="btn${cls}" data-action="${action}"${da} data-tip="${tip}">${content}</button>`);
                }
            }
            parts.push('</div>');
        }
        ...
        this.shadow.querySelector('.ctx-bar').addEventListener('mousedown', (e) => {
            ...
            // 드롭다운 메뉴 항목
            const dropItem = target.closest('.drop-item');
            if (dropItem) {
    ...
    _applyUserModeButtons() {
        this.shadow.querySelectorAll('.btn[data-action]').forEach(btn => {
            const action = btn.dataset.action ?? '';
