...
`;
const USER_DISABLED_TABS = new Set(['edit', 'insert', 'table', 'format', 'misc']);
export class PoaMenuBar extends HTMLElement {
    ...
    connectedCallback() {
        ...
        const tabsHtml = TABS.map((t) => `<button class="tab${t.id === this._activeTab ? ' active' : ''}" data-tab="${t.id}">${t.label}</button>`).join('');
        ...
        this.shadow.querySelectorAll('.tab').forEach(btn => {
            btn.addEventListener('mousedown', (e) => {
                e.preventDefault();
                eventBus.emit(BusEvent.MENUBAR_CHANGE, { tab: btn.dataset.tab });
            });
        });
        this._bindUserArea();
    ...
    _bindUserArea() {
        ...
        const input = this.shadow.getElementById('user-name-input');
    ...
    applyUserMode() {
        ...
        if (USER_DISABLED_TABS.has(this._activeTab)) {
            eventBus.emit(BusEvent.MENUBAR_CHANGE, { tab: 'file' });
        }
    ...
    _applyWriteMode() {
        ...
        if (this._activeTab === 'misc') {
            eventBus.emit(BusEvent.MENUBAR_CHANGE, { tab: 'edit' });
        }
    }
    updateActive() {
