...
export class PoaStatusBar extends HTMLElement {
    ...
    connectedCallback() {
        const paperOptions = PAPER_SIZES.map(p => `<option value="${p.id}">${p.id}</option>`).join('');
        const zoomOptions = [50, 60, 70, 80, 90, 100, 110, 120, 130, 140, 150, 175, 200].map(v => `<option value="${v}">${v}%</option>`).join('');
...
<div class="bar">
  ...
  <button id="zoom-in" class="poa-zoom-btn" title="확대 (Ctrl+=)">+</button>
        ...
        const paperSel = this.shadow.getElementById('paper-select');
        const zoomSel = this.shadow.getElementById('zoom-select');
        const zoomInBtn = this.shadow.getElementById('zoom-in');
        const zoomOutBtn = this.shadow.getElementById('zoom-out');
        const marginBtn = this.shadow.getElementById('margin-btn');
        const popup = this.shadow.getElementById('margin-popup');
        const mTop = this.shadow.getElementById('m-top');
        const mBottom = this.shadow.getElementById('m-bottom');
        const mLeft = this.shadow.getElementById('m-left');
        const mRight = this.shadow.getElementById('m-right');
        const applyBtn = this.shadow.getElementById('m-apply');
        // 기본값 설정
        paperSel.value = 'A4';
        zoomSel.value = '100';
        ...
        // 팝오버 외부 클릭 시 닫기
        this.docClickHandler = () => { popup.classList.add('hidden'); };
    ...
    updatePage(current, total) {
        const el = this.shadow.getElementById('page-info');
        if (el)
            el.textContent = `${current}/${total} 페이지`;
    }
    /** PaperSizeManager 가 발행하는 paper-change 이벤트를 수신해 UI 동기화 */
    syncPaper(e) {
        const { size, zoom, margin } = e.detail;
        const paperSel = this.shadow.getElementById('paper-select');
        const zoomSel = this.shadow.getElementById('zoom-select');
        if (paperSel)
            paperSel.value = size.id;
        if (zoomSel)
        ...
        if (margin) {
            ...
            const mRight = this.shadow.getElementById('m-right');
            if (mTop)
                mTop.value = String(margin.top);
            if (mBottom)
                mBottom.value = String(margin.bottom);
            if (mLeft)
                mLeft.value = String(margin.left);
            if (mRight)
                mRight.value = String(margin.right);
        }
    }
