 ...
 */
function normalizeFontFamilyCss(value) {
...
export class PoaEditor extends HTMLElement {
     ...
     */
    pendingStyles = new Map();
    pendingStylesJustSet = false;
    selectionHandler = () => {
...
<poa-user-guide-dialog></poa-user-guide-dialog>`;
        ...
        this.contentEl = this.querySelector('.poa-editor-content')
        ...
        this.toolbar = this.shadow.querySelector('poa-toolbar');
        this.statusBar = this.shadow.querySelector('poa-status-bar');
        this.findDialog = this.shadow.querySelector('poa-find-replace-dialog');
        this.imageDialog = this.shadow.querySelector('poa-image-edit-dialog');
        this.imageInsertDialog = this.shadow.querySelector('poa-image-dialog');
        this.settingsDialog = this.shadow.querySelector('poa-settings-dialog');
        this.fileManagerDialog = this.shadow.querySelector('poa-file-manager-dialog');
        this.tableDialog = this.shadow.querySelector('poa-table-dialog');
        this.cellSplitDialog = this.shadow.querySelector('poa-cell-split-dialog');
        this.linkDialog = this.shadow.querySelector('poa-link-dialog');
        this.imageToolbar = this.shadow.querySelector('poa-image-toolbar');
        this.confirmDialog = this.shadow.querySelector('poa-confirm-dialog');
        this.accessibilityDialog = this.shadow.querySelector('poa-accessibility-dialog');
        this.accessibilityDialog.setup(this.contentEl, () => this.runAccessibilityCheck());
        this.privacyDialog = this.shadow.querySelector('poa-privacy-dialog');
        this.privacyDialog.setup(() => { void this.core.captureHistory('privacyEdit'); this.statusBar.update(this.contentEl.innerHTML); }, (msg) => this.confirmDialog.show(msg));
        this.formulaManager = new TableFormulaManager();
        this.formulaDialog = this.shadow.querySelector('poa-formula-dialog');
        this.videoInserter = new VideoInserter(this.contentEl);
        this.videoDialog = this.shadow.querySelector('poa-video-dialog');
        ...
        this.formControlDialog = this.shadow.querySelector('poa-form-control-dialog');
        this.templateDialog = this.shadow.querySelector('poa-template-dialog');
        this.templateDialog.setup(() => this.getHTML());
        this.signatureDialog = this.shadow.querySelector('poa-signature-dialog');
        this.emojiDialog = this.shadow.querySelector('poa-emoji-dialog');
        this.tooltipDialog = this.shadow.querySelector('poa-tooltip-dialog');
        this.emojiInserter = new EmojiInserter();
        this.inputPropertyDialog = this.shadow.querySelector('poa-input-property-dialog');
        this.documentListDialog = this.shadow.querySelector('poa-document-list-dialog');
        this.docTitleDialog = this.shadow.querySelector('poa-doc-title-dialog');
        this.shortcutsDialog = this.shadow.querySelector('poa-shortcuts-dialog');
        this.userGuideDialog = this.shadow.querySelector('poa-user-guide-dialog');
        this.documentListDialog.setup((doc) => {
        ...
        if (placeholder)
            this.contentEl.dataset.placeholder = placeholder;
        ...
        const navCallbacks = {
            onMerge: () => {
                ...
                if (!cells.length || !tbl)
                    return { success: false, message: '선택된 셀이 없습니다.' };
        ...
        this.shadow.addEventListener('poa-formula-apply', (e) => {
            const { formula, table } = e.detail;
            const result = this.formulaManager.applyFormula(table, formula);
            if (result === 'circular')
                this.toast.show('순환 참조가 발견됐습니다. (#REF!)', 'error');
            else if (result === 'invalid')
                this.toast.show('대상 셀을 찾을 수 없습니다.', 'error');
            else {
        ...
        this.shadow.addEventListener('poa-tooltip-insert', (e) => {
            ...
            if (!this.savedRange || !this.contentEl.contains(this.savedRange.startContainer))
                return;
        ...
        this.contentEl.addEventListener('poa-form-contextmenu', (e) => {
            ...
            if (cfg)
                this.formControlDialog.open(cfg);
        });
        ...
        this.shadow.addEventListener('poa-bookmark-link-insert', (e) => {
            const { bookmarkId, text } = e.detail;
            try {
        ...
        this.shadow.addEventListener('poa-image-edit-confirm', (e) => {
            const { original, edited, attrs } = e.detail;
        ...
        this.contentEl.addEventListener('mousedown', (e) => {
            ...
            const table = this.findTableNode(e.target);
            // ── 표 선택 테두리 + 이미지 교차 해제 ─────────────────────────
            if (table) {
                this.selectTable(table);
                if (this.imageResizer.getActiveImage())
            ...
            if (inTable && !this.inTableContext) {
                ...
                eventBus.emit(BusEvent.MENUBAR_CHANGE, { tab: 'table' });
            }
        ...
        this.contentEl.addEventListener('dblclick', (e) => {
            ...
            if (target.tagName === 'IMG' && !target.dataset.dir) {
                ...
                void this.imageDialog.open(img.src, {
                    ...
                    align: img.style.float || undefined,
                    id: img.id || undefined,
        ...
        this.contentEl.addEventListener('click', (e) => {
            ...
            const pb = e.target.closest('.poa-page-break');
            if (pb) {
                e.preventDefault();
                const ownerDoc = this.contentEl.ownerDocument;
            ...
            const anchor = e.target.closest('a[href]:not(.poa-bookmark)');
            if (!anchor)
                return;
            e.preventDefault();
        ...
        eventBus.on(BusEvent.MENUBAR_CHANGE, ({ tab }) => {
            if (tab !== 'table')
                this.previousMenuTab = tab;
        });
        this.statusBar.update(this.contentEl.innerHTML);
        this.syncToolbar();
    ...
    initResponsiveLayout() {
        const applyAutoZoom = () => {
            if (!this.paperSizeManager)
                return;
            const w = this.offsetWidth;
            if (w <= 0 || w >= 900)
                return;
            // 사용자가 기본 zoom(100%)을 유지 중일 때만 자동 축소
            if (this.paperSizeManager.getZoom() !== 100)
                return;
            const zoom = Math.max(PaperSizeManager.ZOOM_MIN, Math.round(((w - 40) / 794) * 10) * 10);
            if (zoom < 100)
                this.paperSizeManager.setZoom(zoom);
        };
        if (typeof ResizeObserver !== 'undefined') {
            this._resizeObs = new ResizeObserver(applyAutoZoom);
            this._resizeObs.observe(this);
        }
        this._fullscreenHandler = () => { setTimeout(applyAutoZoom, 100); };
    ...
    initResizeHandle() {
        ...
        const handle = this.shadow.querySelector('.poa-resize-handle');
        if (!handle)
            return;
        handle.addEventListener('mousedown', (e) => {
            ...
            const onMove = (ev) => {
                const newH = Math.min(window.innerHeight - 100, Math.max(300, startH + (ev.clientY - startY)));
            ...
            const onUp = () => {
                ...
                try {
                    localStorage.setItem(STORAGE_KEY, String(this.offsetHeight));
                }
                catch { /* ignore */ }
            };
    ...
    getExportHTML() {
        ...
        this.contentEl.querySelectorAll('input.poa-field-input').forEach((input) => {
            if (input.value)
                input.setAttribute('value', input.value);
        });
        const clone = this.contentEl.cloneNode(true);
        // textarea IDL value는 cloneNode에 복사되지 않으므로 라이브 값을 클론에 반영
        const liveTAs = Array.from(this.contentEl.querySelectorAll('textarea.poa-field-input'));
        const cloneTAs = Array.from(clone.querySelectorAll('textarea.poa-field-input'));
        liveTAs.forEach((ta, i) => { if (cloneTAs[i])
    ...
    setHTML(html) {
        ...
        this.contentEl.querySelectorAll('td, th').forEach((cell) => {
    ...
    enterUserMode(templateName = '문서') {
        ...
        this.shadow.querySelector('poa-menubar').applyUserMode();
        this.shadow.querySelector('poa-context-toolbar').applyUserMode();
    ...
    _activateUserModeFields() {
        this.contentEl.querySelectorAll('.poa-field input.poa-field-input, .poa-field textarea.poa-field-input').forEach(el => {
            el.removeAttribute('readonly');
            el.removeAttribute('disabled');
        });
        this.contentEl.querySelectorAll('input:not(.poa-field-input), textarea:not(.poa-field-input), select').forEach(el => el.removeAttribute('disabled'));
    }
    _pdfExport(templateName) {
        // 1. 안내 토스트
        ...
        clone.querySelectorAll('.poa-field').forEach(field => {
            const inp = field.querySelector('.poa-field-input');
        ...
        clone.querySelectorAll('.poa-page-break').forEach(br => {
...
<body>${clone.outerHTML}</body>
    ...
    restoreSelection() {
        ...
        if (existing && this.contentEl.contains(existing.startContainer))
            return;
        if (!this.savedRange) {
    ...
    async handleAction(e) {
        ...
        switch (type) {
            case 'format': {
                const tag = value ? FORMAT_TAG_MAP[value] : undefined;
            ...
            case 'align': {
                ...
                if (activeImg) {
                    ...
                    selCellInput.style.textAlign = align === 'left' ? '' : align;
                }
            ...
            case 'file:saveas':
                ...
                return;
            case 'file:print':
                this.fileManager.printDocument(this.getExportHTML(), this.paperSizeManager?.getMargin());
                return;
            // ── 편집 탭 액션 ─────────────────────────────────────────────
            case 'edit:cut': {
                const ownerDoc = this.contentEl.ownerDocument;
            ...
            case 'table:split-v': {
                ...
                if (cell)
                    this.cellSplitDialog.open(cell);
                return;
            }
            ...
            case 'table:align-right': {
                ...
                if (!t)
                    return;
                this.tableNavigator.applyTableAlign(t, type.replace('table:align-', ''));
            ...
            case 'table:delete': {
                ...
                const tbl = cell?.closest('table');
                if (cell && tbl)
                    this.tableNavigator.executeAction(type, cell, tbl);
                return;
            }
            // ── 서식 탭 액션 ─────────────────────────────────────────────
            case 'format:clear':
            ...
            case 'paper:zoom-out':
                this.paperSizeManager?.zoomOut();
                return;
            case 'paper:zoom-reset':
                this.paperSizeManager?.resetZoom();
                return;
            case 'paper:size':
                this.paperSizeManager?.setPaperSize(value ?? '');
                return;
            case 'paper:zoom':
                if (value)
                    this.paperSizeManager?.setZoom(parseInt(value, 10));
                return;
            case 'paper:margin':
                if (value) {
                    try {
            ...
            case 'insert:field': {
                ...
                if (!field) {
                    this.toast.show('알 수 없는 양식 필드입니다.', 'error');
                    return;
                }
    ...
    applyInlineStyle(cssProperty, value) {
        const ownerDoc = this.contentEl.ownerDocument;
        const sel = ownerDoc.getSelection();
        if (!sel || sel.rangeCount === 0)
            return;
    ...
    runPrivacyCheck() {
        ...
        setTimeout(() => {
            ...
            if (matches.length > 0)
                PrivacyChecker.highlight(matches);
    ...
    openFormulaDialog() {
        ...
        const pos = cell.dataset.formula
            ? (() => {
                try {
                    return JSON.parse(cell.dataset.formula);
                }
                catch {
                    return null;
                }
    ...
    findBlockAncestor(node) {
        ...
        while (cur && cur !== this.contentEl) {
            if (cur.nodeType === 1 && BLOCK_TAGS.has(cur.tagName.toLowerCase())) {
                return cur;
            }
            cur = cur.parentNode;
        }
    ...
    syncToolbar() {
        ...
        const state = {
            ...
            align: (() => {
                const activeImg = this.imageResizer.getActiveImage();
                if (activeImg)
                    return getImageAlign(activeImg);
                if (this.selectedTable)
                    return getTableAlign(this.selectedTable);
                return (this.getInlineStyle(anchor, 'text-align') || 'left');
            })(),
            canUndo,
            canRedo,
    ...
    hasAncestorTag(node, tag) {
        let cur = node;
        while (cur && cur !== this.contentEl) {
            if (cur.nodeType === 1 && cur.tagName.toLowerCase() === tag)
                return true;
            cur = cur.parentNode;
        }
        return false;
    }
    getInlineStyle(node, cssProperty) {
        let cur = node;
        while (cur && cur !== this.contentEl) {
            if (cur.nodeType === 1) {
                const val = cur.style.getPropertyValue(cssProperty);
                if (val)
                    return val;
            }
            cur = cur.parentNode;
    ...
    getComputedStyle(node, cssProperty) {
    ...
    getFocusedTable() {
        return this.getFocusedCell()?.closest('table') ?? null;
    }
    ...
    static injectContentStyles() {
        if (PoaEditor._stylesInjected)
            return;
    ...
    // ── 링크 컨텍스트 메뉴 ──────────────────────────────────────────────────
    showLinkContextMenu(anchor, x, y) {
        this.hideLinkContextMenu();
        const menu = document.createElement('div');
    ...
    async _doFileSave() {
        try {
            const result = await this.fileManager.saveDocument(this.getHTML(), () => this.docTitleDialog.showTitleInput(this.fileManager.getCurrentDocKey()
            ...
            this.toast.show(`저장 실패: ${e.message}`, 'error');
        }
    }
    async _doFileSaveAs() {
        try {
            const result = await this.fileManager.saveAsDocument(this.getHTML(), () => this.docTitleDialog.showTitleInput(this._getDefaultDocTitle()));
            if (result) {
                this.toast.show('저장되었습니다.', 'success');
                this.embedBridge.notifySaved(result.docKey, result.title ?? '');
            }
