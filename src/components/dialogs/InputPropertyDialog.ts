...
export class PoaInputPropertyDialog extends HTMLElement {
  ...
  open(input: HTMLElement): void {
    if (!this.shadow) this.connectedCallback();
  ...
  private _populate(input: HTMLElement): void {
    const s = this.shadow;
    const w = input.style.width;

    // 너비 모드 감지
    const radios = s.querySelectorAll<HTMLInputElement>('input[name="w-mode"]');
    const wpx  = s.getElementById('w-px')  as HTMLInputElement;
    const wpct = s.getElementById('w-pct') as HTMLInputElement;
    if (!w || w === '100%') {
    ...
    const alignSel = s.getElementById('align') as HTMLSelectElement;
    if (ml === 'auto' && mr === 'auto') alignSel.value = 'center';
    else if (ml === 'auto')             alignSel.value = 'right';
    else                                alignSel.value = 'left';

    // 텍스트 정렬
    (s.getElementById('text-align') as HTMLSelectElement).value = input.style.textAlign || '';

    // placeholder
    const ph = s.getElementById('placeholder') as HTMLInputElement;
    ph.value = (input as HTMLInputElement).placeholder ?? '';

    // required / disabled
    (s.getElementById('required') as HTMLInputElement).checked = (input as HTMLInputElement).required ?? false;
    (s.getElementById('disabled') as HTMLInputElement).checked = (input as HTMLInputElement).disabled ?? false;
  }
  ...
  private _bindEvents(): void {
    ...
    this.shadow.addEventListener('change', (e) => {
      ...
      if ((t as HTMLInputElement).name === 'w-mode') {
        const v = (t as HTMLInputElement).value;
        (this.shadow.getElementById('w-px')  as HTMLInputElement).disabled = (v !== 'px');
        (this.shadow.getElementById('w-pct') as HTMLInputElement).disabled = (v !== 'pct');
      }
    });
  ...
  private _apply(): void {
    ...
    if (!input) { this.close(); return; }

    const s = this.shadow;

    // 너비
    const wMode = (s.querySelector<HTMLInputElement>('input[name="w-mode"]:checked'))?.value ?? 'full';
    if (wMode === 'full') {
      input.style.width = '100%';
      input.style.maxWidth = '100%';
    } else if (wMode === 'px') {
      const v = parseFloat((s.getElementById('w-px') as HTMLInputElement).value);
      if (v > 0) input.style.width = `${v}px`;
    } else {
      const v = parseFloat((s.getElementById('w-pct') as HTMLInputElement).value);
      if (v > 0) input.style.width = `${v}%`;
    }

    // 정렬 (셀 내 위치)
    const align = (s.getElementById('align') as HTMLSelectElement).value;
    input.style.display      = 'block';
    input.style.marginLeft   = align === 'center' || align === 'right'  ? 'auto' : '';
    input.style.marginRight  = align === 'center' || align === 'left'   ? 'auto' : '';
    if (align === 'left') { input.style.marginLeft = ''; input.style.marginRight = 'auto'; }
    if (align === 'right') { input.style.marginLeft = 'auto'; input.style.marginRight = ''; }

    // 텍스트 정렬
    input.style.textAlign = (s.getElementById('text-align') as HTMLSelectElement).value;

    // placeholder
    (input as HTMLInputElement).placeholder =
      (s.getElementById('placeholder') as HTMLInputElement).value;

    // required / disabled
    (input as HTMLInputElement).required =
      (s.getElementById('required') as HTMLInputElement).checked;
    (input as HTMLInputElement).disabled =
      (s.getElementById('disabled') as HTMLInputElement).checked;
