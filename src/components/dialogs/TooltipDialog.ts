import type { TooltipEntry } from '../../modules/insert/TooltipManager.js';
...
type DialogMode = 'add' | 'edit' | 'list';

export class PoaTooltipDialog extends HTMLElement {
  private shadow!: ShadowRoot;
  private mode: DialogMode = 'add';
  private editId: string | null = null;
  private entries: TooltipEntry[] = [];
...
<div class="dlg">
  ...
  openAdd(selectedText: string): void {
    this.mode   = 'add';
    this.editId = null;
    this._val('f-title',   '');
    this._val('f-content', '');
    ...
    setTimeout(() => (this.shadow.getElementById('f-title') as HTMLInputElement).focus(), 50);
  }

  openEdit(entry: TooltipEntry): void {
    this.mode   = 'edit';
    ...
    this._updatePreview();
    this.setAttribute('open', '');
    setTimeout(() => (this.shadow.getElementById('f-content') as HTMLTextAreaElement).focus(), 50);
  }

  openList(entries: TooltipEntry[]): void {
    this.mode    = 'list';
  ...
  private _showEditPanel(title: string, okLabel: string): void {
  ...
  private _updatePreview(): void {
    const title   = (this.shadow.getElementById('f-title')   as HTMLInputElement).value.trim();
    const content = (this.shadow.getElementById('f-content') as HTMLTextAreaElement).value.trim();
  ...
  private _bind(): void {
    ...
    const liveUpdate = (): void => this._updatePreview();
    ...
    this.shadow.getElementById('f-content')!.addEventListener('keydown', (e) => {
      if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') this._doOk();
  ...
  private _doOk(): void {
    ...
    const content = (this.shadow.getElementById('f-content') as HTMLTextAreaElement).value.trim();
    if (!content) {
      (this.shadow.getElementById('f-content') as HTMLTextAreaElement).focus();
      return;
    }
  ...
  private _val(id: string, value: string): void {
    (this.shadow.getElementById(id) as HTMLInputElement | HTMLTextAreaElement).value = value;
  }
}
