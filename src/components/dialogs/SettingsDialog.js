...
<div class="backdrop" id="backdrop">
  <div class="dialog" role="dialog" aria-modal="true" aria-label="환경설정">
    ...
    readForm() {
        const sh = this.shadow;
        const getSelect = (id) => sh.getElementById(id)?.value ?? '';
        const getCheck = (id) => sh.getElementById(id)?.checked ?? false;
        return {
    ...
    toggleIntervalRow(enabled) {
        const row = this.shadow.getElementById('row-interval');
        if (row)
    ...
    bindEvents() {
        ...
        sh.getElementById('backdrop')?.addEventListener('click', (e) => {
            if (e.target === sh.getElementById('backdrop'))
                this.close();
        });
        ...
        sh.getElementById('chk-autosave')
            ?.addEventListener('change', (e) => {
            this.toggleIntervalRow(e.target.checked);
        });
    }
    handleSave() {
