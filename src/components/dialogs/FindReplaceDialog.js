...
export class PoaFindReplaceDialog extends HTMLElement {
    ...
    updateResult(count, current) {
        const label = this.shadow.getElementById('count-label');
        const findInp = this.shadow.getElementById('inp-find');
        if (!label)
            return;
    ...
    bindEvents() {
        const s = this.shadow;
        const findInput = s.getElementById('inp-find');
        const replaceInput = s.getElementById('inp-replace');
        const caseChk = s.getElementById('chk-case');
        const wordChk = s.getElementById('chk-word');
        const getOpts = () => ({
        ...
        // 바꾸기 버튼
        s.getElementById('btn-replace')?.addEventListener('click', () => this.dispatch('find-replace', { replacement: replaceInput.value }));
        s.getElementById('btn-replace-all')?.addEventListener('click', () => this.dispatch('find-replace-all', { ...getOpts(), replacement: replaceInput.value }));
    }
