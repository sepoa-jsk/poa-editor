...
import type { BookmarkEntry } from '../../modules/insert/BookmarkManager.js';

type DialogTab = 'link' | 'bookmark' | 'datetime';

/** Intl.DateTimeFormat 프리셋 5종 */
const DATETIME_PRESETS: ReadonlyArray<{ label: string; fn: () => string }> = [
  {
    label: '날짜 (2025-05-05)',
...
<div class="backdrop" id="backdrop">
  ...
  setBookmarks(entries: BookmarkEntry[]): void {
    ...
    if (this.isConnected) this.renderBookmarkList();
  }

  open(tab: DialogTab = 'link', anchor?: HTMLAnchorElement): void {
    this.reset();
    ...
    if (tab === 'link') {
      if (anchor) {
        ...
        const s = this.shadow;
        (s.getElementById('inp-href')  as HTMLInputElement).value  = anchor.getAttribute('href') ?? '';
        (s.getElementById('inp-text')  as HTMLInputElement).value  = anchor.textContent ?? '';
        (s.getElementById('inp-title') as HTMLInputElement).value  = anchor.title ?? '';
        const target = anchor.target === '_blank' ? '_blank' : '';
        (s.getElementById('sel-target') as HTMLSelectElement).value = target;
        s.getElementById('rel-hint')?.classList.toggle('show', target === '_blank');
        s.getElementById('dialog-title')!.textContent = '링크 수정';
        (s.getElementById('btn-confirm') as HTMLButtonElement).textContent = '수정';
      }
      (this.shadow.getElementById('inp-href') as HTMLInputElement)?.focus();
    } else if (tab === 'bookmark') {
      (this.shadow.getElementById('inp-bm-label') as HTMLInputElement)?.focus();
    }
  }
  ...
  private reset(): void {
    const s = this.shadow;
    (s.getElementById('inp-href')  as HTMLInputElement).value  = '';
    (s.getElementById('inp-text')  as HTMLInputElement).value  = '';
    (s.getElementById('inp-title') as HTMLInputElement).value  = '';
    (s.getElementById('sel-target') as HTMLSelectElement).value = '';
    (s.getElementById('inp-bm-label') as HTMLInputElement).value = '';
    ...
    (s.getElementById('btn-confirm') as HTMLButtonElement).textContent = '삽입';
    this.editingId = null;
    this.editingAnchor = null;
  ...
  private bindEvents(): void {
    ...
    s.querySelectorAll('.tab-btn').forEach((btn) => {
      btn.addEventListener('click', () =>
        this.switchTab((btn as HTMLElement).dataset.tab as DialogTab));
    });

    // 새 탭 선택 시 rel 힌트 표시
    s.getElementById('sel-target')?.addEventListener('change', () => {
      const blank = (s.getElementById('sel-target') as HTMLSelectElement).value === '_blank';
    ...
    s.getElementById('inp-bm-label')?.addEventListener('keydown', (e) => {
      if ((e as KeyboardEvent).key === 'Enter') this.onBookmarkCreate();
    });
  }
  ...
  private switchTab(tab: DialogTab): void {
    ...
    const titles: Record<DialogTab, string> = {
      link: '링크 삽입',
      bookmark: '책갈피 관리',
      datetime: '날짜·시간 삽입',
    };
  ...
  private onConfirm(): void {
    ...
    if (activePanel?.id === 'panel-link') this.confirmLink();
  }

  private confirmLink(): void {
    const s = this.shadow;
    const href  = (s.getElementById('inp-href')  as HTMLInputElement).value.trim();
    const text  = (s.getElementById('inp-text')  as HTMLInputElement).value.trim();
    const title = (s.getElementById('inp-title') as HTMLInputElement).value.trim();
    const targetVal = (s.getElementById('sel-target') as HTMLSelectElement).value;
    ...
    if (!href || !hrefOk) {
      ...
      (s.getElementById('inp-href') as HTMLInputElement).focus();
      valid = false;
    } else {
      s.getElementById('err-href')!.classList.remove('show');
    }
    ...
    if (!text) {
      ...
      if (valid) (s.getElementById('inp-text') as HTMLInputElement).focus();
      valid = false;
    } else {
      s.getElementById('err-text')!.classList.remove('show');
    }
  ...
  private onBookmarkCreate(): void {
    const input = this.shadow.getElementById('inp-bm-label') as HTMLInputElement;
  ...
  private renderBookmarkList(): void {
    ...
    ul.addEventListener('click', (e) => {
      ...
      if (action === 'link')        this.onBookmarkLink(id);
      else if (action === 'edit')   this.onBookmarkEditStart(id);
      else if (action === 'save')   this.onBookmarkSave(id);
      else if (action === 'delete') this.onBookmarkDelete(id);
      else if (action === 'cancel-edit') { this.editingId = null; this.renderBookmarkList(); }
    }, { once: true }); // re-rendered each time so remove old listener
  }

  private onBookmarkLink(id: string): void {
  ...
  private onBookmarkEditStart(id: string): void {
    this.editingId = id;
    this.renderBookmarkList();
    (this.shadow.getElementById('inp-bm-edit') as HTMLInputElement | null)?.focus();
  }

  private onBookmarkSave(id: string): void {
    const input = this.shadow.getElementById('inp-bm-edit') as HTMLInputElement | null;
    const label = input?.value.trim() ?? '';
    if (!label) { input?.focus(); return; }
  ...
  private onBookmarkDelete(id: string): void {
    this.dispatch('poa-bookmark-delete', { id });
  }

  private buildDatetimePresets(): void {
