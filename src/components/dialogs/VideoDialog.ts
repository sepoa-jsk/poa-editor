import {
  ...
  buildEmbedHtml,
  type VideoMimeType,
} from '../../modules/insert/VideoInserter.js';

type DialogTab = 'video' | 'embed';
...
export class PoaVideoDialog extends HTMLElement {
  ...
  private activeTab: DialogTab = 'video';
...
<div class="overlay" id="overlay">
  ...
  private bindEvents(): void {
    ...
    // 탭 전환
    sd.querySelectorAll<HTMLButtonElement>('.tab-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        this.activeTab = btn.dataset.tab as DialogTab;
    ...
    sd.getElementById('v-autoplay')?.addEventListener('change', (e) => {
      const autoplay = (e.target as HTMLInputElement).checked;
      const mutedEl  = sd.getElementById('v-muted') as HTMLInputElement | null;
      if (mutedEl && autoplay) mutedEl.checked = true;
    });

    // 자막 입력 → a11y 힌트 숨김
    sd.getElementById('v-track-src')?.addEventListener('input', (e) => {
      const hint = sd.getElementById('a11y-hint');
      if (hint) hint.style.display = (e.target as HTMLInputElement).value ? 'none' : 'flex';
    });

    // 외부 URL 입력 → 실시간 미리보기
    sd.getElementById('e-url')?.addEventListener('input', (e) => {
      this.updatePreview((e.target as HTMLInputElement).value);
    });
  ...
  private buildVideoHtml(): string | null {
    ...
    const w       = parseInt((sd.getElementById('v-width')  as HTMLInputElement | null)?.value ?? '640', 10);
    const h       = parseInt((sd.getElementById('v-height') as HTMLInputElement | null)?.value ?? '360', 10);
    const controls = (sd.getElementById('v-controls') as HTMLInputElement | null)?.checked ?? true;
    const autoplay = (sd.getElementById('v-autoplay') as HTMLInputElement | null)?.checked ?? false;
    const loop     = (sd.getElementById('v-loop')     as HTMLInputElement | null)?.checked ?? false;
    const muted    = (sd.getElementById('v-muted')    as HTMLInputElement | null)?.checked ?? false;
    const trackSrc = (sd.getElementById('v-track-src') as HTMLInputElement | null)?.value.trim();
    const trackSrclang = (sd.getElementById('v-track-lang') as HTMLSelectElement | null)?.value ?? 'ko';
