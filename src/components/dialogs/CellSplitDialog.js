  ...
  <div class="dialog" role="dialog" aria-modal="true">
    ...
    <div class="dlg-body">
      ...
      <div class="actions">
        ...
        const colsInput = this.shadow.getElementById('cs-cols');
        const rowsInput = this.shadow.getElementById('cs-rows');
        const okBtn = this.shadow.getElementById('btn-ok');
        const updateOk = () => {
    ...
    confirm() {
        if (!this.targetCell)
            return;
        const cols = Math.max(1, parseInt(this.shadow.getElementById('cs-cols').value, 10) || 1);
        const rows = Math.max(1, parseInt(this.shadow.getElementById('cs-rows').value, 10) || 1);
        if (cols <= 1 && rows <= 1)
            return;
