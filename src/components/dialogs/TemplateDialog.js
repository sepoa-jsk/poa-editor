...
export class PoaTemplateDialog extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this.tree = this.shadow.querySelector('poa-template-tree');
    ...
    _bind() {
        ...
        this.shadow.getElementById('sf-name')
    ...
    async _refreshPreview() {
        ...
        const frame = this.shadow.getElementById('preview-frame');
        const appendBtn = this.shadow.getElementById('btn-append');
        const replBtn = this.shadow.getElementById('btn-replace');
...
.poa-field-drag-handle,.poa-field-resize-handle{display:none;}
    ...
    // ── 저장 ──────────────────────────────────────────────────────────────────
    _doSave() {
        const name = this.shadow.getElementById('sf-name').value.trim();
        if (!name) {
            this.shadow.getElementById('sf-name').focus();
            return;
        }
        const folderId = this.shadow.getElementById('sf-folder').value || null;
        const isPublicSelected = this.shadow.getElementById('sf-public').checked;
    ...
    _fillFolderSelect() {
        const sel = this.shadow.getElementById('sf-folder');
    ...
    async _adminCleanup() {
        ...
        try {
            ...
            try {
                localStorage.removeItem('poa-templates');
            }
            catch { /* ignore */ }
            if (serverNodes.length > 0) {
                try {
                    localStorage.setItem('poa-templates', JSON.stringify(serverNodes));
                }
                catch { /* ignore */ }
            }
        }
