...
export class PoaFormulaDialog extends HTMLElement {
    ...
    applyRange(sr, sc, er, ec) {
        this.exitPickMode();
        const q = (id) => this.shadow.getElementById(id);
        const set = (id, v) => { const el = q(id); if (el)
            el.value = String(v); };
        set('sr', sr);
        set('sc', sc);
        set('er', er);
    ...
    // ── 렌더 ─────────────────────────────────────────────────────────────────────
    render(existing, initialRange) {
        const fn = existing?.fn ?? 'SUM';
        const format = existing?.format ?? 'integer';
        ...
        const fnOpt = (v, lbl) => `<label><input type="radio" name="fn" value="${v}"${fn === v ? ' checked' : ''}> ${v} (${lbl})</label>`;
        const fmtOpt = (v, lbl) => `<label><input type="radio" name="format" value="${v}"${format === v ? ' checked' : ''}> ${lbl}</label>`;
        this.shadow.innerHTML = `<style>${CSS}</style>
<div class="overlay" id="overlay">
  <div class="dialog" id="dialog">
    ...
    bindEvents() {
        ...
        sd.querySelectorAll('input[name="target"]').forEach(radio => {
            radio.addEventListener('change', () => {
                const row = sd.getElementById('target-other-row');
                if (!row)
                    return;
    ...
    confirm() {
        if (!this.currentTable)
            return;
        const q = (id) => this.shadow.getElementById(id);
        const iv = (id) => Math.max(1, parseInt((q(id)?.value ?? '1'), 10));
        const fn = (this.shadow.querySelector('input[name="fn"]:checked')?.value ?? 'SUM');
        const format = (this.shadow.querySelector('input[name="format"]:checked')?.value ?? 'integer');
        const target = this.shadow.querySelector('input[name="target"]:checked')?.value ?? 'current';
