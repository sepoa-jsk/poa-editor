...
import { buildFormControlHtml, generateControlId, } from '../../modules/form/FormControlInserter.js';
// ── 아이콘 SVG ──────────────────────────────────────────────────────────────
const ICONS = {
...
const TYPES = ['text', 'textarea', 'checkbox', 'radio', 'select', 'button', 'date'];
// ── 스타일 ──────────────────────────────────────────────────────────────────
const STYLE = `
...
export class PoaFormControlDialog extends HTMLElement {
    ...
    open(existing) {
        ...
        this.shadow.getElementById('f-label')?.focus();
    }
  ...
  <div class="ftr">
    ...
    _secTextarea() {
      ...
      ${field('줄 수', `<input type="number" id="f-ta-rows" value="4" min="2" max="20" style="width:80px">`, '입력창의 기본 높이를 줄 수로 조정합니다')}
      ${this._adv(`
        <div class="chk-row" style="margin-bottom:10px">
    ...
    _secCheckbox() {
      ...
      ${field('체크박스 텍스트', inp('f-cb-lbl', '예) 개인정보 수집에 동의합니다'), '체크박스 옆에 표시되는 설명입니다')}
      <div class="field">
        <div class="chk-row">
    ...
    _secRadio() {
      ...
      ${field('그룹 제목', inp('f-radio-label', '예) 성별, 선호 연락 방법…'), '라디오 버튼 그룹 위에 표시되는 제목입니다')}
      <div class="field">
        <label>선택지 목록</label>
    ...
    _setType(type) {
        ...
        this.shadow.querySelectorAll('.type-card').forEach(c => {
            c.classList.toggle('active', c.dataset.type === type);
        });
        this.shadow.querySelectorAll('.specific-section').forEach(s => {
    ...
    _addRadioRow(opt) {
        ...
        if (opt?.defaultChecked) {
            defBtn.style.color = '#f59e0b';
            defBtn.dataset.default = '1';
        }
        defBtn.addEventListener('click', () => {
            // 같은 목록의 다른 ★ 초기화
            list.querySelectorAll('button[title="기본 선택"]').forEach(b => {
                b.style.color = '#d1d5db';
                delete b.dataset.default;
            });
            defBtn.style.color = '#f59e0b';
            defBtn.dataset.default = '1';
            this._updatePreview();
    ...
    _clearFields() {
        ...
        set('f-date-val', '');
        set('f-date-name', '');
        chk('f-aria-req', false);
        chk('f-ta-req', false);
        chk('f-cb-checked', false);
        chk('f-cb-req', false);
        chk('f-sel-multi', false);
        chk('f-date-req', false);
        const btnType = this.shadow.querySelector('input[name="btn-type"][value="button"]');
        if (btnType)
            btnType.checked = true;
        const btnStyle = this.shadow.getElementById('f-btn-style');
        if (btnStyle)
            btnStyle.value = 'default';
        ...
        this._addSelectRow();
        // 추가 설정 닫기
        this.shadow.querySelectorAll('[data-adv]').forEach(btn => {
    ...
    _fillFields(c) {
        ...
        switch (c.type) {
            ...
            case 'button':
                ...
                const t = this.shadow.querySelector(`input[name="btn-type"][value="${c.btnType ?? 'button'}"]`);
                if (t)
                    t.checked = true;
                const s = this.shadow.getElementById('f-btn-style');
                if (s)
                    s.value = c.btnStyle ?? 'default';
                break;
            case 'date':
    ...
    // ── config 수집 ───────────────────────────────────────────────────────
    _v(id) {
        return (this.shadow.getElementById(id)?.value ?? '').trim();
    }
    _c(id) {
        return this.shadow.getElementById(id)?.checked ?? false;
    }
    _buildConfig() {
        ...
        switch (this.activeType) {
            ...
            case 'radio': {
                ...
                this.shadow.getElementById('radio-opts').querySelectorAll('.opt-row').forEach(row => {
                    ...
                    const defBtn = row.querySelector('button[title="기본 선택"]');
                    const isDef = !!defBtn?.dataset.default;
                    if (lbl)
                        opts.push({ label: lbl, value: lbl, defaultChecked: isDef || undefined });
                });
            ...
            case 'select': {
                ...
                this.shadow.getElementById('select-opts').querySelectorAll('.opt-row').forEach(row => {
                    ...
                    if (lbl)
                        opts.push({ label: lbl, value: lbl, selected: isSel || undefined });
                });
                return {
                    type: 'select',
            ...
            case 'button': {
                const btnType = (this.shadow.querySelector('input[name="btn-type"]:checked')?.value ?? 'button');
                const btnStyle = (this.shadow.getElementById('f-btn-style')?.value ?? 'default');
                return {
                    type: 'button',
