...
import type { ImageAttributes } from '../../modules/insert/ImageInserter';
...
<div class="backdrop" id="backdrop">
  ...
  async open(src: string, existingAttrs?: Partial<ImageAttributes>): Promise<void> {
    this.originalSrc = src;
    this.currentDataUrl = src;
    this.busy = false;
    this.switchTab('canvas');

    if (existingAttrs) this.fillProps(existingAttrs);
  ...
  private switchTab(tab: 'canvas' | 'props'): void {
    const s = this.shadow;
  ...
  private fillProps(attrs: Partial<ImageAttributes>): void {
    const val = (id: string, v?: string) => {
      (this.shadow.getElementById(id) as HTMLInputElement).value = v ?? '';
    };
    val('p-alt',   attrs.alt);
    ...
    (this.shadow.getElementById('p-align') as HTMLSelectElement).value = attrs.align ?? '';
  ...
  private bindEvents(): void {
    ...
    s.querySelectorAll('.tab-btn').forEach((btn) => {
      btn.addEventListener('click', () =>
        this.switchTab((btn as HTMLElement).dataset.tab as 'canvas' | 'props'));
    });
  ...
  private setControlsDisabled(disabled: boolean): void {
    this.shadow.querySelectorAll<HTMLButtonElement>('.controls button, .btn-apply')
      .forEach((b) => { b.disabled = disabled; });
  }

  private readProps(): Partial<ImageAttributes> {
    const v = (id: string) => (this.shadow.getElementById(id) as HTMLInputElement).value.trim();
    return {
      ...
      align:     (this.shadow.getElementById('p-align') as HTMLSelectElement).value || undefined,
      id:        v('p-id')    || undefined,
  ...
  private onApply(): void {
    ...
    if (isPropsActive && !props.alt) {
      ...
      (this.shadow.getElementById('p-alt') as HTMLInputElement).focus();
      return;
    }
