...
import type { SignatureLayout } from '../../modules/signature/SignatureManager.js';

const LAYOUT_LABELS: Record<SignatureLayout, string> = {
...
export class PoaSignatureDialog extends HTMLElement {
  ...
  private mgr!:    SignatureManager;
  private selectedId: string | null = null;
  private currentLayout: SignatureLayout = 1;
  private currentLogo: string | undefined;
  private currentHeaderColor = '#2563eb';
  ...
  private _buildLayoutGrid(): void {
    const grid = this.shadow.getElementById('layout-grid')!;
    grid.innerHTML = '';
    ([1, 2, 3, 4, 5, 6] as SignatureLayout[]).forEach(n => {
  ...
  private _bind(): void {
    ...
    this.shadow.getElementById('f-hdr-color')!.addEventListener('input', e => {
      this.currentHeaderColor = (e.target as HTMLInputElement).value;
    ...
    this.shadow.getElementById('btn-logo')!.addEventListener('click', () => {
      (this.shadow.getElementById('logo-input') as HTMLInputElement).click();
    });
    ...
    this.shadow.getElementById('logo-input')!.addEventListener('change', e => {
      ...
      if (file) this._handleLogoUpload(file);
    });
  ...
  private _loadSignature(id: string): void {
    ...
    (this.shadow.getElementById('f-hdr-color') as HTMLInputElement).value = this.currentHeaderColor;
  ...
  private _newSignature(): void {
    this.selectedId     = null;
    this.currentLayout  = 1;
    this.currentLogo    = undefined;
    this.currentHeaderColor = '#2563eb';

    ['f-signame','f-name','f-title','f-dept','f-company','f-phone','f-email','f-website'].forEach(id =>
      this._val(id, '')
    );
    (this.shadow.getElementById('f-hdr-color') as HTMLInputElement).value = '#2563eb';
    ...
    this._renderList();
    this._refreshPreview();
    setTimeout(() => (this.shadow.getElementById('f-signame') as HTMLInputElement).focus(), 50);
  }

  // ── 레이아웃 ─────────────────────────────────────────────────────────────

  private _setLayout(n: SignatureLayout): void {
  ...
  private _syncLayoutCards(): void {
    this.shadow.querySelectorAll<HTMLElement>('.layout-card').forEach(card => {
      card.classList.toggle('active', card.dataset.layout === String(this.currentLayout));
    });
  }

  private _syncColorRow(): void {
  ...
  private _handleLogoUpload(file: File): void {
    ...
    reader.onload = () => {
      this.currentLogo = reader.result as string;
      this._syncLogoPreview();
      this._refreshPreview();
    };
  ...
  private _doSave(): void {
    const sigName = (this.shadow.getElementById('f-signame') as HTMLInputElement).value.trim();
    if (!sigName) {
      (this.shadow.getElementById('f-signame') as HTMLInputElement).focus();
      return;
    }
    const data = {
      name:        sigName,
  ...
  private _val(id: string, value: string): void {
    (this.shadow.getElementById(id) as HTMLInputElement).value = value;
  }

  private _collectFields() {
    return {
      displayName: (this.shadow.getElementById('f-name')    as HTMLInputElement).value.trim(),
      title:       (this.shadow.getElementById('f-title')   as HTMLInputElement).value.trim(),
      department:  (this.shadow.getElementById('f-dept')    as HTMLInputElement).value.trim(),
      company:     (this.shadow.getElementById('f-company') as HTMLInputElement).value.trim(),
      phone:       (this.shadow.getElementById('f-phone')   as HTMLInputElement).value.trim(),
      email:       (this.shadow.getElementById('f-email')   as HTMLInputElement).value.trim(),
      website:     (this.shadow.getElementById('f-website') as HTMLInputElement).value.trim(),
    };
  }
}
