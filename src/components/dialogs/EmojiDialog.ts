...
import type { EmojiCategory } from '../../modules/insert/EmojiInserter.js';
...
export class PoaEmojiDialog extends HTMLElement {
  ...
  private inserter = new EmojiInserter();
  private activeCatId = EMOJI_CATEGORIES[0].id;
...
<div class="dlg">
  ...
  open(): void {
    ...
    setTimeout(() => (this.shadow.getElementById('search') as HTMLInputElement).focus(), 50);
  }
  ...
  private _buildCatBar(): void {
    ...
    for (const cat of EMOJI_CATEGORIES) {
      ...
      btn.addEventListener('click', () => {
        ...
        (this.shadow.getElementById('search') as HTMLInputElement).value = '';
      });
      bar.appendChild(btn);
    }
  }

  private _syncCatBar(): void {
    this.shadow.querySelectorAll<HTMLButtonElement>('.cat-btn').forEach(btn => {
  ...
  private _renderCategory(catId: string): void {
    ...
    if (cat) this._appendSection(area, cat.label, cat.emojis);
  }
  ...
  private _appendSection(area: HTMLElement, label: string, emojis: string[]): void {
  ...
  private _bind(): void {
    ...
    const search = this.shadow.getElementById('search') as HTMLInputElement;
    search.addEventListener('input', () => {
      const q = search.value.trim();
      if (q) this._renderSearch(q);
      else   this._renderCategory(this.activeCatId);
    });
  ...
  private _pickEmoji(emoji: string): void {
    this.dispatchEvent(new CustomEvent('poa-emoji-insert', {
      bubbles: true, composed: true,
      detail: { emoji },
    }));
    // Recent 렌더링 갱신 (inserter.addRecent는 PoaEditor에서 insert() 호출 시 처리됨)
    const search = (this.shadow.getElementById('search') as HTMLInputElement).value.trim();
    if (!search) this._renderCategory(this.activeCatId);
  }
}

// EmojiCategory를 re-export하여 외부에서 타입 사용 가능
