import type { AccessibilityIssue, AccessibilityResult, IssueLevel } from '../../modules/accessibility/AccessibilityChecker.js';
...
export class PoaAccessibilityDialog extends HTMLElement {
  ...
  private rerunFn: (() => void) | null = null;
  private currentFilter: IssueLevel | 'all' = 'all';
...
<div class="overlay" id="overlay">
  ...
  setup(contentEl: HTMLElement, rerunFn: () => void): void {
  ...
  /** 검사 결과를 표시한다. */
  show(result: AccessibilityResult): void {
  ...
  private renderSummary(result: AccessibilityResult): void {
  ...
  private renderIssues(): void {
    ...
    const fixAllBtn = this.shadow.getElementById('btn-fix-all') as HTMLButtonElement | null;
    const hasAutoFix = this.issues.some(i => i.autoFix);
    if (fixAllBtn) fixAllBtn.disabled = !hasAutoFix;
  ...
  private renderCard(issue: AccessibilityIssue, idx: number): string {
...
<div class="issue-card" data-level="${issue.level}" data-idx="${idx}">
  ...
  private buildFixPanelHTML(issue: AccessibilityIssue, idx: number): string {
  ...
  private bindEvents(): void {
    ...
    this.shadow.getElementById('filter-select')?.addEventListener('change', (e) => {
      this.currentFilter = (e.target as HTMLSelectElement).value as IssueLevel | 'all';
      this.applyFilter();
    });
  ...
  private bindCardEvents(): void {
    ...
    list.addEventListener('click', (e) => {
      ...
      // 해당 요소로 이동
      const navBtn = target.closest<HTMLElement>('.btn-nav');
      if (navBtn) {
      ...
      // 직접 수정 패널 토글
      const fixBtn = target.closest<HTMLElement>('.btn-fix');
      if (fixBtn) {
      ...
      const applyBtn = target.closest<HTMLElement>('.btn-apply');
      if (applyBtn) {
        const idx = parseInt(applyBtn.dataset.idx ?? '-1');
        const action = applyBtn.dataset.action ?? '';
        const inp = this.shadow.getElementById(`fi-${idx}`) as HTMLInputElement | null;
      ...
      // 자동 수정
      const autoBtn = target.closest<HTMLElement>('[data-action="auto-fix"]');
      if (autoBtn) {
  ...
  private navigateTo(el: Element | null): void {
  ...
  private applyFix(idx: number, action: string, value: string): void {
    const issue = this.issues[idx];
    if (!issue) return;

    switch (action) {
      case 'set-alt':
        (issue.element as HTMLImageElement | null)?.setAttribute('alt', value || '이미지');
        break;
      case 'set-caption': {
        const table = issue.element as HTMLTableElement | null;
        if (table) {
          let cap = table.querySelector('caption');
          if (!cap) { cap = table.ownerDocument.createElement('caption'); table.prepend(cap); }
          cap.textContent = value || '표';
        }
        break;
      }
      case 'set-link-text':
        if (issue.element) issue.element.textContent = value;
        break;
      case 'auto-fix':
        issue.autoFix?.();
        break;
  ...
  private applyFilter(): void {
    const cards = this.shadow.querySelectorAll<HTMLElement>('.issue-card');
    cards.forEach(card => {
      const level = card.dataset.level as IssueLevel;
      card.hidden = this.currentFilter !== 'all' && level !== this.currentFilter;
    });
  }

  private syncFilterSelect(): void {
    const sel = this.shadow.getElementById('filter-select') as HTMLSelectElement | null;
