import type { PrivacyMatch, RiskLevel } from '../../modules/privacy/PrivacyChecker.js';
...
export class PoaPrivacyDialog extends HTMLElement {
  ...
  private items: PrivacyMatch[] = [];
  private filteredType = 'all';
  private onModified: (() => void) | null = null;
  private onConfirmFn: ((msg: string) => Promise<boolean>) | null = null;
...
<div class="overlay">
  ...
  show(matches: PrivacyMatch[]): void {
    this.items = matches;
...
<div class="overlay" id="overlay">
  ...
  private buildCard(m: PrivacyMatch): string {
    return `
<div class="card" data-level="${m.riskLevel}" data-id="${m.id}">
  ...
  private visibleMatches(): PrivacyMatch[] {
    if (this.filteredType === 'all') return this.items;
  ...
  private bindEvents(): void {
    ...
    sd.getElementById('filter-type')?.addEventListener('change', (e) => {
      this.filteredType = (e.target as HTMLSelectElement).value;
      this.renderList();
    });
    ...
    // 카드 아코디언 토글
    sd.querySelectorAll<HTMLElement>('[data-toggle]').forEach(header => {
  ...
  private renderList(): void {
    ...
    // 이벤트 재바인딩 (list 내부만)
    list.querySelectorAll<HTMLElement>('[data-toggle]').forEach(header => {
      header.addEventListener('click', () => {
        const id = header.dataset.toggle!;
  ...
  private navigateTo(match: PrivacyMatch): void {
