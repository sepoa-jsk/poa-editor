...
export class PoaToolbar extends HTMLElement {
    ...
    populateSelects() {
        const s = this.shadow;
        const selFamily = s.getElementById('sel-family');
        ...
        const selSize = s.getElementById('sel-size');
        ...
        selSize.value = '12pt';
        const selLh = s.getElementById('sel-lh');
        ...
        selLh.value = '1.5';
        const selLs = s.getElementById('sel-ls');
    ...
    bindEvents() {
        ...
        s.getElementById('fore-input')?.addEventListener('change', (e) => {
            ...
            s.getElementById('fore-bar').style.background = v;
            dispatch('foreColor', v);
        });
        s.getElementById('back-input')?.addEventListener('change', (e) => {
            const v = e.target.value;
            s.getElementById('back-bar').style.background = v;
            dispatch('backColor', v);
        });
        const bindAlign = (id, value) => {
            s.getElementById(id)?.addEventListener('mousedown', (e) => {
                e.preventDefault();
        ...
        s.getElementById('sel-family').addEventListener('change', (e) => dispatch('fontFamily', e.target.value));
        s.getElementById('sel-size').addEventListener('change', (e) => dispatch('fontSize', e.target.value));
        s.getElementById('sel-lh').addEventListener('change', (e) => dispatch('lineHeight', e.target.value));
        s.getElementById('sel-ls').addEventListener('change', (e) => dispatch('letterSpacing', e.target.value));
    }
    setState(state) {
        const s = this.shadow;
        ...
        if (state.fontFamily) {
            ...
            s.getElementById('sel-family').value = normalized;
        }
        if (state.fontSize)
            s.getElementById('sel-size').value = state.fontSize;
        if (state.lineHeight)
            s.getElementById('sel-lh').value = state.lineHeight;
        if (state.letterSpacing)
            s.getElementById('sel-ls').value = state.letterSpacing;
        if (state.foreColor) {
            s.getElementById('fore-bar').style.background = state.foreColor;
            s.getElementById('fore-input').value = state.foreColor;
        }
        if (state.backColor) {
            s.getElementById('back-bar').style.background = state.backColor;
            s.getElementById('back-input').value = state.backColor;
        }
    }
    /** 환경설정 기본값을 툴바 드롭다운에 반영한다. */
    setDefaults(fontFamily, fontSize, lineHeight) {
        const s = this.shadow;
        const selFamily = s.getElementById('sel-family');
        if (selFamily) {
            const opts = Array.from(selFamily.options);
            const match = opts.find((o) => o.value === fontFamily ||
                o.value.split(',')[0].trim().replace(/['"]/g, '') === fontFamily);
            if (match)
                selFamily.value = match.value;
        }
        const selSize = s.getElementById('sel-size');
        if (selSize)
            selSize.value = `${fontSize}pt`;
        const selLh = s.getElementById('sel-lh');
        if (selLh)
            selLh.value = String(lineHeight);
    }
    setHistoryState(canUndo, canRedo) {
        const s = this.shadow;
        const u = s.getElementById('btn-undo');
        const r = s.getElementById('btn-redo');
        if (u)
            u.disabled = !canUndo;
        if (r)
            r.disabled = !canRedo;
    ...
    applyUserMode() {
        ...
        this.shadow.querySelectorAll('button, select, input').forEach(el => { el.disabled = true; });
    }
}
