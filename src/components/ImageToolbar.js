...
<button class="btn" id="btn-reset">원본크기</button>`;
    ...
    show(img) {
        ...
        this.shadow.getElementById('cb-ratio').checked = false;
    ...
    bindEvents() {
        const s = this.shadow;
        const inpW = s.getElementById('inp-w');
        const inpH = s.getElementById('inp-h');
        const cbRatio = s.getElementById('cb-ratio');
        const btnReset = s.getElementById('btn-reset');
        ...
        // Enter 키로도 즉시 적용
        inpW.addEventListener('keydown', (e) => { if (e.key === 'Enter')
            this.onWidthChange(inpW); });
        inpH.addEventListener('keydown', (e) => { if (e.key === 'Enter')
            this.onHeightChange(inpH); });
    ...
    onWidthChange(inpW) {
        ...
        if (this.aspectLocked && this.naturalW > 0 && this.naturalH > 0) {
            ...
            this.shadow.getElementById('inp-h').value = String(h);
        }
        else {
    ...
    onHeightChange(inpH) {
        ...
        if (this.aspectLocked && this.naturalW > 0 && this.naturalH > 0) {
            ...
            this.shadow.getElementById('inp-w').value = String(w);
        }
        else {
