 ...
 */
export function syncInputValuesToAttributes(root) {
 ...
 */
export function getSelectedBlocks(contentEl, range) {
    if (range.collapsed) {
        ...
        if (range.startContainer === contentEl) {
            const at = contentEl.childNodes[range.startOffset];
            const prev = contentEl.childNodes[range.startOffset - 1];
            if (at && isBlock(at))
                return [at];
...
export function getImageAlign(img) {
...
export function getTableAlign(table) {
