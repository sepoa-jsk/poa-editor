...
import { 
// 서식 툴바
Bold, Italic, Underline, Strikethrough, Undo2, Redo2, AlignLeft, AlignCenter, AlignRight, AlignJustify, Indent, Outdent, Baseline, Highlighter, BetweenVerticalStart, MoveHorizontal, 
// 파일
...
// 삽입
Image, Images, Film, PlayCircle, Link, Link2Off, Bookmark, PenLine, Smile, MessageCircle, MessageSquare, Calendar, Minus, Omega, MonitorPlay, SeparatorHorizontal, LayoutTemplate, FormInput, 
// 보기
LayoutDashboard, Code2, Eye, Type, FileText, Maximize2, Minimize2, Ruler, Grid3x3, BoxSelect, 
// 표
Table, TableProperties, LayoutGrid, TableCellsMerge, TableCellsSplit, ArrowUpFromLine, ArrowDownFromLine, ArrowLeftFromLine, ArrowRightFromLine, Rows3, Columns3, Trash2, 
// 서식
...
// 템플릿 트리 / 사용자 식별
Folder, File, Users, UserCircle, ChevronRight, ChevronDown, Plus, Pencil, Hash, GripVertical, } from 'lucide-static';
/** SVG width/height 24 → N으로 정규화 */
export function pxN(svg, n) {
    return svg
        .replace(/width="24"/, `width="${n}"`)
        .replace(/height="24"/, `height="${n}"`);
}
function px16(svg) { return pxN(svg, 16); }
function px14(svg) { return pxN(svg, 14); }
function px12(svg) { return pxN(svg, 12); }
export const Icons = {
    // 서식 툴바
    bold: px16(Bold),
    italic: px16(Italic),
