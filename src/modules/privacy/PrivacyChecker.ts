export type RiskLevel = 'very-high' | 'high' | 'medium';
...
export interface PrivacyMatch {
  ...
  riskLevel: RiskLevel;
  label: string;
  raw: string;          // 탐지된 원문 텍스트
  masked: string;       // 마스킹 결과 미리보기
  textNode: Text;       // 실제 텍스트 노드 참조
  nodeOffset: number;   // 텍스트 노드 내 시작 오프셋
  locationLabel: string; // "2번째 단락" 등 사람이 읽을 수 있는 위치
  highlightEl: HTMLElement | null; // 하이라이트 span (삽입 후 채워짐)
}
...
interface PatternDef {
  ...
  riskLevel: RiskLevel;
  pattern: RegExp;
  /** 계좌번호처럼 컨텍스트 조건이 있는 경우 검증 함수 */
  contextGuard?: (fullText: string, matchIndex: number) => boolean;
...
function hasFinanceKeywordNear(text: string, index: number, radius = 100): boolean {
...
function maskPhoneGeneral(raw: string): string {
...
function maskResidentId(raw: string): string {
  const dash = raw.indexOf('-');
  if (dash === 6) return raw.slice(0, 7) + '*******';
  if (raw.length >= 13) return raw.slice(0, 7) + '*******';
...
function makeLocationLabel(root: HTMLElement, node: Text): string {
...
function tagToKo(tag: string): string {
...
export class PrivacyChecker {
  ...
  run(): PrivacyMatch[] {
      ...
      if (text.trim()) {
        for (const def of PATTERNS) {
          ...
          while ((m = def.pattern.exec(text)) !== null) {
            const raw = m[0];
            if (def.contextGuard && !def.contextGuard(text, m.index)) continue;
    ...
    // 위험도 높은 순 정렬
    const order: Record<RiskLevel, number> = { 'very-high': 0, 'high': 1, 'medium': 2 };
   ...
   */
  static highlight(matches: PrivacyMatch[]): void {
    // 동일 텍스트 노드 내 오프셋이 뒤에 있는 것부터 처리
    const sorted = [...matches].sort((a, b) => {
      if (a.textNode === b.textNode) return b.nodeOffset - a.nodeOffset;
      return 0;
    });
    ...
    for (const match of sorted) {
      try {
        ...
        if (before) parent.insertBefore(ownerDoc.createTextNode(before), node);
        parent.insertBefore(span, node);
        if (after) parent.insertBefore(ownerDoc.createTextNode(after), node);
  ...
  static removeHighlights(root: HTMLElement): void {
    root.querySelectorAll<HTMLElement>('[data-privacy-id]').forEach(span => {
  ...
  /** 단일 매치 삭제 */
  static deleteMatch(match: PrivacyMatch): void {
  ...
  /** 단일 매치 마스킹 */
  static maskMatch(match: PrivacyMatch): void {
    const span = match.highlightEl;
    if (span) {
  ...
  /** 모든 매치 삭제 */
  static deleteAll(matches: PrivacyMatch[]): void {
    for (const m of matches) PrivacyChecker.deleteMatch(m);
  }

  /** 모든 매치 마스킹 */
  static maskAll(matches: PrivacyMatch[]): void {
    for (const m of matches) PrivacyChecker.maskMatch(m);
  }
}
