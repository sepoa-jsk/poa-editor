...
let _seq = 0;
export function generateControlId(type) {
    return `poa-${type}-${(++_seq).toString(36)}`;
}
function boolAttrs(c) {
    return [
        c.disabled ? 'disabled' : '',
        c.readonly ? 'readonly' : '',
 ...
 */
export function buildFormControlHtml(control) {
    ...
    switch (control.type) {
        ...
        case 'checkbox': {
            ...
            const v = control.value ? ` value="${esc(control.value)}"` : '';
            return wrap(`<label class="poa-checkbox-label">\n    ` +
        ...
        case 'button': {
            ...
            return wrap(`<button type="${btnType}"${idAttr}${nameAttr} class="${cls}"${bool ? ' ' + bool : ''}>${text}</button>`);
        }
        case 'date': {
...
export class FormControlInserter {
    ...
    buildElement(config) {
        ...
        tmp.innerHTML = clean;
        const groupEl = tmp.firstElementChild;
        if (groupEl)
            groupEl.dataset.poaForm = JSON.stringify(config);
        return groupEl;
    }
    /**
    ...
    insert(config) {
        ...
        if (cell && inContent && range) {
            const controlEl = (groupEl.querySelector('input, textarea, select, button') ?? groupEl);
