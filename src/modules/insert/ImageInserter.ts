...
export class ImageInserter {
  ...
  insertFromUrl(attrs: ImageAttributes): void {
    if (!attrs.alt.trim()) {
  ...
  async uploadAndInsert(
    file: File,
    attrs: Omit<ImageAttributes, 'src'>,
    config: UploadConfig,
  ): Promise<void> {
  ...
  validateExtension(filename: string): void {
    ...
    if (!ALLOWED_IMG_EXTENSIONS.has(ext)) {
      throw new Error(
        `지원하지 않는 파일 형식입니다. (허용: ${[...ALLOWED_IMG_EXTENSIONS].join(', ')})`,
      );
    }
  }

  private async doUpload(file: File, config: UploadConfig): Promise<string> {
  ...
  private buildImg(attrs: ImageAttributes): HTMLImageElement {
