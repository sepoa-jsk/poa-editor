...
export const EMOJI_CATEGORIES: EmojiCategory[] = RAW.map(([id, label, icon, raw]) => ({
...
export const EMOJI_KEYWORDS = new Map<string, string[]>(KW);
...
export function searchEmojis(query: string): string[] {
  ...
  for (const [emoji, keywords] of EMOJI_KEYWORDS) {
    if (keywords.some(k => k.includes(q))) results.add(emoji);
  }
...
export class EmojiInserter {
  ...
  addRecent(emoji: string): void {
    ...
    try { localStorage.setItem(RECENT_KEY, JSON.stringify(list)); } catch { /* full */ }
  }

  insert(emoji: string, container: HTMLElement): void {
