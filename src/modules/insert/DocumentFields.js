...
// ── Persistence ──────────────────────────────────────────────────────────────
export function loadFieldCategories() {
    try {
...
export function saveFieldCategories(cats) {
    localStorage.setItem(KEY_CATEGORIES, JSON.stringify(cats));
}
export function loadFieldDefinitions() {
    try {
...
export function saveFieldDefinitions(fields) {
    localStorage.setItem(KEY_FIELDS, JSON.stringify(fields));
}
// ── Active field accessors ───────────────────────────────────────────────────
export function getActiveDocumentFields() {
    return loadFieldDefinitions();
}
export function getActiveFieldMap() {
    return Object.fromEntries(getActiveDocumentFields().map(f => [f.id, f]));
}
// ── Legacy exports (backward-compat) ─────────────────────────────────────────
export const DOCUMENT_FIELDS = BUILTIN_FIELDS;
export const FIELD_MAP = Object.fromEntries(BUILTIN_FIELDS.map(f => [f.id, f]));
