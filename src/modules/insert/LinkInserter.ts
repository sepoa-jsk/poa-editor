...
export interface LinkAttributes {
  ...
  text: string;
  title?: string;
  /** _blank 선택 시 rel="noopener noreferrer" 자동 주입 */
  target?: '_blank' | '_self';
}
...
export class LinkInserter {
   ...
   */
  insertLink(attrs: LinkAttributes): void {
    ...
    if (attrs.title) anchor.title = attrs.title;
    ...
    if (attrs.target === '_self') anchor.target = '_self';
    this.insertNode(anchor);
  }

  /** 기존 <a> 엘리먼트의 속성을 업데이트한다 */
  updateLink(anchor: HTMLAnchorElement, attrs: Partial<LinkAttributes>): void {
    if (attrs.href !== undefined) {
      if (!validateLinkUrl(attrs.href)) throw new Error('유효하지 않은 URL입니다.');
      anchor.href = attrs.href;
    }
    if (attrs.text !== undefined) anchor.textContent = attrs.text;
    if (attrs.title !== undefined) anchor.title = attrs.title;
  ...
  getFocusedAnchor(): HTMLAnchorElement | null {
    const sel = this.root.ownerDocument.getSelection();
