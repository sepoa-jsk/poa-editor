...
const TYPE_BADGE = { text: 'T', textarea: '☰', number: '#', date: '📅' };
...
const FIELD_INPUT_STYLE = `${BASE_FIELD_STYLE};display:inline-block;padding:1px 4px;vertical-align:middle;` +
    `resize:horizontal;height:auto;`;
const FIELD_TEXTAREA_STYLE = `${BASE_FIELD_STYLE};display:inline-block;padding:2px 4px;vertical-align:middle;resize:both;` +
    `max-height:400px;line-height:inherit;min-height:1.5em;height:auto;`;
// ── 숫자 포맷 헬퍼 ──────────────────────────────────────────────────────────
const KOR_NUMS = ['', '일', '이', '삼', '사', '오', '육', '칠', '팔', '구'];
function groupToKorean(n) {
    if (n <= 0 || n > 9999)
        return '';
    let result = '';
...
function toKoreanUnits(num) {
    ...
    for (const [unit, label] of groups) {
        ...
        if (q > 0) {
            parts.push(q.toLocaleString('ko-KR') + label);
            n %= unit;
        }
    }
    if (n > 0)
        parts.push(n.toLocaleString('ko-KR'));
    return sign + parts.join(' ');
}
function toKoreanFull(num) {
    if (num === 0)
        return '영원';
    ...
    for (const [unit, label] of groups) {
        const q = Math.floor(n / unit);
        if (q > 0) {
            parts.push(groupToKorean(q) + label);
            n %= unit;
        }
    }
    if (n > 0)
        parts.push(groupToKorean(n));
 ...
 * 파싱 실패 시 raw 그대로 반환.
 */
export function applyDateFormat(raw, format) {
...
export function formatNumber(raw, format) {
...
function injectPopupStyles(doc) {
...
export class FieldInserter {
    ...
    clickHandler = (e) => {
        ...
        if (!target.classList.contains('poa-field-input'))
            return;
        if (this.contentEl?.dataset.userMode === 'true')
            return; // 사용자 모드: 팝업 열지 않음
        e.preventDefault();
        e.stopPropagation();
        this.openPopup(target);
    };
    handleMouseDown = (e) => {
        const handle = e.target.closest('.poa-field-drag-handle');
        if (!handle)
            return;
        const span = handle.closest('.poa-field');
        if (!span)
            return;
        e.preventDefault();
        this.startFieldDrag(span, e);
    };
    startFieldDrag(span, startEv) {
        ...
        const onUp = (up) => {
            ...
            if (typeof doc.caretRangeFromPoint === 'function') {
                range = doc.caretRangeFromPoint(up.clientX, up.clientY);
            }
            else if (typeof doc.caretPositionFromPoint === 'function') {
                const pos = doc.caretPositionFromPoint(up.clientX, up.clientY);
                if (pos) {
    ...
    insertField(field, savedRange) {
        ...
        const fieldEl = this.createTextarea(ownerDoc, field.label, field.id);
        fieldEl.rows = 1;
        if (field.type === 'date')
            fieldEl.setAttribute('data-input-type', 'date');
        ...
        if (containerCell && this.isCellEffectivelyEmpty(containerCell)) {
            ...
            // 삽입 후 셀 내 잔여 빈 노드 제거
            if (containerCell)
                this.cleanCellAfterInsert(containerCell, span);
        }
        range.setStartAfter(span);
        range.collapse(true);
        sel?.removeAllRanges();
    ...
    createTextarea(doc, placeholder, fieldId) {
    ...
    cleanCellAfterInsert(cell, span) {
        ...
        Array.from(cell.querySelectorAll('p')).forEach((p) => {
            if (!span.contains(p) && (p.textContent ?? '').trim() === '')
                p.remove();
        });
    }
    attachResizeObserver(el, span) {
    ...
    attachResizeDrag(handle, fieldEl, span) {
        handle.addEventListener('mousedown', (startEv) => {
            ...
            const onMove = (mv) => {
                ...
                if (cell)
                    newW = Math.min(newW, cell.clientWidth - 20);
    ...
    openPopup(el) {
        this.closePopup();
        const span = el.closest('.poa-field');
        if (!span)
            return;
        ...
        const opt = (val, cur, text) => `<option value="${val}"${val === cur ? ' selected' : ''}>${text}</option>`;
      ...
      <div class="pf-field">
        ...
        const alignBtns = ['left', 'center', 'right'].map((a) => `<button class="pf-align-btn${textAlign === a ? ' active' : ''}" data-align="${a}"` +
            ` title="${a === 'left' ? '왼쪽' : a === 'center' ? '가운데' : '오른쪽'}">${ALIGN_ICONS[a]}</button>`).join('');
  ...
  <div id="pf-body-size" class="pf-sec-body">
    <div class="pf-row2">
      ...
      <div class="pf-field" style="margin-bottom:0;">
        ...
        if (popupTop + POPUP_H > vh)
            popupTop = rect.top - POPUP_H - 6;
        if (popupLeft + POPUP_W > vw)
  ...
  <div id="pf-body-display" class="pf-sec-body">
    <div class="pf-row2">
      ...
      <div class="pf-field" style="margin-bottom:0;">
        ...
        this.popupEl = popup;
        // ── 이벤트 바인딩 ──────────────────────────────────────────────
        const q = (id) => popup.querySelector(`#${id}`);
        const getVal = () => currentEl.tagName === 'TEXTAREA'
            ? currentEl.value
            : currentEl.value;
        const setVal = (v) => {
        ...
        // 기본값 입력
        const pfValueEl = popup.querySelector('#pf-value');
        ...
        q('pf-prefix').addEventListener('input', (ev) => {
            span.setAttribute(ATTR.prefix, ev.target.value);
        });
        q('pf-suffix').addEventListener('input', (ev) => {
            span.setAttribute(ATTR.suffix, ev.target.value);
        });
        q('pf-fontsize').addEventListener('change', (ev) => {
            const v = ev.target.value;
        ...
        q('pf-font').addEventListener('change', (ev) => {
            const v = ev.target.value;
            span.setAttribute(ATTR.fontFamily, v);
            currentEl.style.fontFamily = v || 'inherit';
        });
        q('pf-size-fixed').addEventListener('change', (ev) => {
            const v = ev.target.value;
        ...
        q('pf-multiline').addEventListener('change', (ev) => {
            const useMultiline = ev.target.value === '1';
            const wasMultiline = currentEl.tagName === 'TEXTAREA';
            if (useMultiline === wasMultiline)
                return;
            const val = getVal();
            ...
            if (dw)
                newEl.style.width = `${dw}px`;
            if (dh && useMultiline)
                newEl.style.height = `${dh}px`;
            // 값 이전 (줄바꿈 → input이면 공백 치환)
            if (useMultiline) {
                newEl.value = val;
            }
            else {
                newEl.value = val.replace(/\n/g, ' ');
            }
        ...
        if (isNumber) {
            q('pf-numformat').addEventListener('change', (ev) => {
        ...
        if (isDate) {
            q('pf-dateformat').addEventListener('change', (ev) => {
                const fmt = ev.target.value;
                span.setAttribute(ATTR.dateFormat, fmt);
                const raw = span.getAttribute(ATTR.rawValue) ?? '';
                if (raw)
                    setVal(applyDateFormat(raw, fmt));
                const pfInput = popup.querySelector('#pf-value');
                if (pfInput)
                    pfInput.placeholder = DATE_FORMAT_PLACEHOLDER[fmt] ?? '예) 20261231';
            });
        }
        // 가로/세로 크기 입력
        popup.querySelector('#pf-width')?.addEventListener('change', (ev) => {
            let v = parseInt(ev.target.value, 10) || 60;
        ...
        if (showHeight) {
            popup.querySelector('#pf-height')?.addEventListener('change', (ev) => {
                let v = parseInt(ev.target.value, 10) || 28;
        ...
        // 정렬 아이콘 버튼
        popup.querySelectorAll('.pf-align-btn').forEach((btn) => {
            btn.addEventListener('click', () => {
                popup.querySelectorAll('.pf-align-btn').forEach((b) => b.classList.remove('active'));
        ...
        // 섹션 접기/펼치기
        popup.querySelectorAll('.pf-sec-hdr').forEach((hdr) => {
            hdr.addEventListener('click', () => {
                const body = popup.querySelector(`#pf-body-${hdr.dataset.sec}`);
                const arrow = hdr.querySelector('.pf-sec-arrow');
                if (!body || !arrow)
                    return;
        ...
        const outsideHandler = (ev) => {
            if (!this.popupEl?.contains(ev.target)) {
                this.closePopup();
        ...
        // ── 팝업 드래그 ────────────────────────────────────────────────
        const header = popup.querySelector('#pf-header');
     ...
     */
    static exportFields(html) {
        const doc = new DOMParser().parseFromString(html, 'text/html');
        doc.querySelectorAll('.poa-field').forEach((span) => {
            ...
            const fieldEl = span.querySelector('.poa-field-input');
            let rawVal = '';
            if (fieldEl?.tagName === 'TEXTAREA') {
                rawVal = fieldEl.textContent?.trim() ?? '';
            }
            else {
                rawVal = fieldEl?.getAttribute('value')?.trim() ?? '';
            }
