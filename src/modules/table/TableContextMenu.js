...
export class TableContextMenu {
    ...
    navigator;
    cb;
    constructor(navigator, callbacks = {}) {
    ...
    show(x, y, cell) {
        ...
        requestAnimationFrame(() => {
            ...
            if (r.right > vw)
                this.menuEl.style.left = `${x - r.width}px`;
            if (r.bottom > vh)
                this.menuEl.style.top = `${y - r.height}px`;
        });
...
<div style="display:flex;justify-content:flex-end;gap:8px;margin-top:16px;">
  ...
  <button id="cp-ok" style="height:28px;padding:0 14px;border:1px solid #1565c0;border-radius:3px;background:#1565c0;color:#fff;cursor:pointer;">적용</button>
        ...
        dlg.querySelector('#cp-bs').value = cur.borderStyle ?? 'solid';
        // 세로 정렬 버튼 토글
        const vaBtns = dlg.querySelectorAll('[data-va]');
        ...
        dlg.querySelector('#cp-ok').addEventListener('click', () => {
            const activeVa = dlg.querySelector('[data-va][style*="#EFF6FF"]');
            const props = {
                borderStyle: dlg.querySelector('#cp-bs').value,
                borderWidth: parseInt(dlg.querySelector('#cp-bw').value, 10) || 0,
                borderColor: dlg.querySelector('#cp-bc').value,
                indent: parseInt(dlg.querySelector('#cp-ind').value, 10) || 0,
                bgColor: dlg.querySelector('#cp-bg').value,
                fontSize: parseInt(dlg.querySelector('#cp-fs').value, 10) || 0,
                verticalAlign: (activeVa?.dataset.va ?? curVa),
                id: dlg.querySelector('#cp-id').value.trim(),
                className: dlg.querySelector('#cp-cls').value.trim(),
            };
            for (const c of targetCells)
                CellMerger.applyCellProperties(c, props);
            this.cb.onModified?.();
            close();
        });
    }
