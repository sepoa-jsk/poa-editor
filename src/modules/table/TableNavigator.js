...
export class TableNavigator {
    ...
    keydownHandler = (e) => {
        ...
        const anchor = sel && sel.rangeCount > 0 ? sel.getRangeAt(0).startContainer : e.target;
    ...
    mousedownResizeHandler = (e) => {
        if (!this.pendingResize || e.button !== 0)
            return;
    ...
    appendRow(table) {
        ...
        const refStyle = lastRow.cells[0].style.cssText;
    ...
    makeEmptyRow(srcRow, table) {
        const ownerDoc = table.ownerDocument;
        const grid = buildGridMap(table);
        const rows = Array.from(table.rows);
        const rIdx = rows.indexOf(srcRow);
        const colCount = Math.max(...grid.map((r) => r.length));
        const refStyle = srcRow.cells[0]?.style.cssText ?? '';
        const newRow = ownerDoc.createElement('tr');
    ...
    insertColRight(cell, table) {
        const grid = buildGridMap(table);
        const colIdx = this.findCellCol(grid, cell);
        if (colIdx !== -1)
            this.insertColAt(table, grid, colIdx + Math.max(cell.colSpan, 1), cell);
        this.cb.onModified?.();
    }
    insertColAt(table, grid, atCol, refCell) {
        const rows = Array.from(table.rows);
        const ownerDoc = table.ownerDocument;
        for (let r = 0; r < rows.length; r++) {
            ...
            if (insertBefore)
                rows[r].insertBefore(nc, insertBefore);
            else
                rows[r].appendChild(nc);
        }
    }
    ...
    applyCellVerticalAlign(cell, table, align) {
        const selected = table.querySelectorAll('.poa-cell-selected');
...
<div style="display:flex;justify-content:flex-end;gap:8px;">
  ...
  <button id="cp-ok" style="height:28px;padding:0 14px;border:1px solid #1565c0;border-radius:3px;background:#1565c0;color:#fff;cursor:pointer;font-size:13px;">적용</button>
        ...
        // 초기값 설정
        dlg.querySelector('#cp-bstyle').value = current.borderStyle ?? 'solid';
        // 세로 정렬 토글 버튼
        let selectedVa = initVa;
        dlg.querySelector('#cp-va-wrap').addEventListener('click', (e) => {
            const btn = e.target.closest('.cp-va-btn');
            if (!btn)
                return;
            const va = btn.dataset.va;
            selectedVa = va;
            dlg.querySelectorAll('.cp-va-btn').forEach((b) => {
        ...
        dlg.querySelector('#cp-ok')?.addEventListener('click', () => {
            const props = {
                ...
                borderStyle: dlg.querySelector('#cp-bstyle').value,
                borderWidth: parseInt(dlg.querySelector('#cp-bwidth').value, 10) || 0,
                borderColor: dlg.querySelector('#cp-bcolor').value,
                indent: parseInt(dlg.querySelector('#cp-indent').value, 10) || 0,
                bgColor: dlg.querySelector('#cp-bg').value,
                id: dlg.querySelector('#cp-id').value.trim(),
                className: dlg.querySelector('#cp-class').value.trim(),
            };
     ...
     */
    executeAction(action, cell, table) {
        switch (action) {
            case 'table:row-above':
                this.insertRowAbove(cell, table);
                break;
            case 'table:row-below':
                this.insertRowBelow(cell, table);
                break;
            case 'table:col-left':
                this.insertColLeft(cell, table);
                break;
            case 'table:col-right':
                this.insertColRight(cell, table);
                break;
            case 'table:row-delete':
                this.deleteRow(cell, table);
                break;
            case 'table:col-delete':
                this.deleteCol(cell, table);
                break;
            case 'table:delete':
            ...
            case 'table:merge':
                ...
                break;
            case 'table:split-h':
                this.doSplitH(cell, table);
                break;
            case 'table:split-v':
                this.doSplitV(cell, table);
                break;
            case 'table:cell-props':
                this.showCellPropsModal(cell);
                break;
            case 'table:table-props':
                this.cb.onOpenTableProps?.(table);
                break;
            case 'table:align-left':
                this.applyTableAlign(table, 'left');
                break;
            case 'table:align-center':
                this.applyTableAlign(table, 'center');
                break;
            case 'table:align-right':
                this.applyTableAlign(table, 'right');
                break;
        }
    }
    // ── 열 너비 드래그 조정 ──────────────────────────────────────────
    startColResize(cell, table, startX) {
        const grid = buildGridMap(table);
        const colIdx = this.findCellCol(grid, cell);
        if (colIdx === -1)
            return;
        ...
        const onMove = (mv) => this.handleColDragMove(mv);
        const onUp = () => {
    ...
    handleColDragMove(e) {
        if (!this.dragState || this.dragState.type !== 'col')
            return;
    ...
    // ── 행 높이 드래그 조정 ──────────────────────────────────────────
    startRowResize(cell, table, startY) {
        const row = cell.closest('tr');
        if (!row)
            return;
        ...
        const onMove = (mv) => this.handleRowDragMove(mv);
        const onUp = () => {
            ownerDoc.removeEventListener('mousemove', onMove);
            ownerDoc.removeEventListener('mouseup', onUp);
            ownerDoc.body.style.userSelect = '';
    ...
    handleRowDragMove(e) {
        if (!this.dragState || this.dragState.type !== 'row')
            return;
        const s = this.dragState;
