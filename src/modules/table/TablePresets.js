...
function allCells(table) {
    return Array.from(table.querySelectorAll('td,th'));
}
function theadCells(table) {
    return Array.from(table.querySelectorAll('thead td,thead th'));
}
...
export const TABLE_PRESETS = [
    {
        ...
        apply: (table) => {
            for (const c of allCells(table))
                c.style.cssText = `border:1px solid #ccc;${PAD}`;
        },
    },
    {
        id: 'border-thick',
        ...
        apply: (table) => {
            for (const c of allCells(table))
                c.style.cssText = `border:1px solid #aaa;${PAD}`;
    ...
    },
    {
        ...
        apply: (table) => {
            for (const c of allCells(table))
                c.style.cssText = `border:1px solid #ddd;${PAD}`;
    ...
    },
    {
        ...
        apply: (table) => {
            for (const c of allCells(table))
                c.style.cssText = `border:1px solid #bbb;${PAD}`;
            tbodyRows(table).forEach((tr, i) => {
    ...
    },
    {
        ...
        apply: (table) => {
            for (const c of allCells(table))
                c.style.cssText = `border:none;${PAD}`;
    ...
    },
    {
        ...
        apply: (table) => {
            for (const c of allCells(table))
                c.style.cssText = `border:1px dashed #999;${PAD}`;
        },
    },
];
export function applyPreset(presetId, table) {
