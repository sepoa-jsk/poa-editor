...
export class TableHandle {
  ...
  private hideTimer = 0;
  private onSelectAll: ((table: HTMLTableElement) => void) | null = null;

  constructor(onSelectAll?: (table: HTMLTableElement) => void) {
  ...
  private readonly overHandler = (e: MouseEvent): void => {
    const table = this.findTable(e.target as Node);
    if (!table) return;
    clearTimeout(this.hideTimer);
  ...
  private readonly outHandler = (e: MouseEvent): void => {
    const rel = e.relatedTarget as Node | null;
    if (rel && (this.handle?.contains(rel) || this.currentTable?.contains(rel))) return;
    this.hideTimer = window.setTimeout(() => this.hideHandle(), 200);
  };

  // ── 핸들 생성/표시/숨김 ─────────────────────────────────────────

  private createHandle(doc: Document): void {
  ...
  private findTable(node: Node): HTMLTableElement | null {
    let cur: Node | null = node;
    while (cur) {
      if (cur.nodeType === Node.ELEMENT_NODE) {
        if ((cur as Element).tagName.toLowerCase() === 'table') return cur as HTMLTableElement;
      }
      cur = cur.parentNode;
    }
    return null;
