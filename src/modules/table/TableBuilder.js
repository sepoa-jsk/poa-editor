...
export class TableBuilder {
    ...
    static build(options, ownerDoc = document) {
        const { rows, cols, width, height, border = 1, marginLeft = 0, align = 'left', headerPosition = 'none', borderColor = '#000000', bgColor, id, className, caption, captionVisible = true, summary, } = options;
        ...
        // ── HTML 속성 ──────────────────────────────────────────────────
        if (border > 0)
            table.setAttribute('border', String(border));
        table.setAttribute('cellpadding', '4');
        table.setAttribute('cellspacing', '0');
        if (summary?.trim())
            table.setAttribute('summary', summary.trim());
        if (id)
            table.id = id;
        if (className)
    ...
    static readOptions(table) {
        ...
        let headerPosition = 'none';
        if (table.querySelector('thead'))
            headerPosition = 'top';
        else if (table.querySelector('tbody tr:first-child th'))
            headerPosition = 'left';
    ...
    static findBlockAncestor(node, root) {
        let cur = node;
        while (cur && cur !== root) {
