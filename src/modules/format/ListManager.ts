...
function getBlocksInRange(contentEl: HTMLElement, range: Range): HTMLElement[] {
  const blocks: HTMLElement[] = [];
  const children = Array.from(contentEl.children) as HTMLElement[];

  const expandIfList = (el: HTMLElement): HTMLElement[] =>
    LIST_TAGS.has(el.tagName.toLowerCase())
  ...
  if (range.collapsed) {
    ...
    if (block) blocks.push(...expandIfList(block));
    return blocks;
  }

  for (const child of children) {
    if (range.intersectsNode(child)) blocks.push(...expandIfList(child));
  }
  return blocks;
}
...
export class ListManager {
   ...
   */
  toggleSuperSub(tag: 'sup' | 'sub'): void {
    const sel = this.contentEl.ownerDocument.getSelection();
    if (!sel || sel.rangeCount === 0) return;
    const range = sel.getRangeAt(0);
    ...
    if (alreadyWrapped) {
      ...
      while (ancestorOpposite.firstChild) newEl.appendChild(ancestorOpposite.firstChild);
   ...
   */
  handleTab(e: KeyboardEvent): boolean {
    const sel = this.contentEl.ownerDocument.getSelection();
  ...
  private listifyBlocks(blocks: HTMLElement[], type: 'ul' | 'ol'): void {
    const ownerDoc = this.contentEl.ownerDocument;
  ...
  private unlistBlocks(blocks: HTMLElement[]): void {
    const ownerDoc = this.contentEl.ownerDocument;
  ...
  private indentListItem(li: HTMLLIElement): void {
    const prev = li.previousElementSibling as HTMLLIElement | null;
    if (!prev) return;

    const ownerDoc = this.contentEl.ownerDocument;
    const parentTag = li.parentElement?.tagName.toLowerCase() as 'ul' | 'ol' ?? 'ul';
  ...
  private outdentListItem(li: HTMLLIElement): void {
    const parentList = li.parentElement;
  ...
  private findAncestorTag(node: Node, tag: string): HTMLElement | null {
    let cur: Node | null = node.nodeType === Node.TEXT_NODE ? node.parentNode : node;
    while (cur && cur !== this.contentEl) {
      if (cur.nodeType === Node.ELEMENT_NODE && (cur as HTMLElement).tagName.toLowerCase() === tag) {
  ...
  private isWrappedWith(fragment: DocumentFragment, tag: string): boolean {
    ...
    while (textNode) {
      ...
      while (cur && cur !== fragment) {
        if ((cur as HTMLElement).tagName?.toLowerCase() === tag) { found = true; break; }
        cur = cur.parentNode;
      }
  ...
  private unwrapTag(node: Node, tag: string): void {
