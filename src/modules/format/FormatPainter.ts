...
type InlineProp = typeof INLINE_PROPS[number];

/** 인라인 서식 태그 (unwrap 대상) */
const FORMAT_TAGS = new Set(['strong', 'em', 'u', 's', 'sup', 'sub', 'span']);

export interface FormatPainterCallbacks {
  /** 페인터 모드 활성/비활성 변경 시 호출 */
...
export class FormatPainter {
  ...
  private readonly cb: FormatPainterCallbacks;

  private savedStyles = new Map<InlineProp, string>();
  private _active   = false;
  private _continuous = false;

  constructor(contentEl: HTMLElement, callbacks: FormatPainterCallbacks = {}) {
    this.contentEl = contentEl;
    this.cb = callbacks;
  }

  get isActive(): boolean { return this._active; }
  get hasSavedStyles(): boolean { return this.savedStyles.size > 0; }
  ...
  paste(range?: Range): void {
    if (this.savedStyles.size === 0) return;
    ...
    if (!this._continuous) this.deactivate();
  }
  ...
  handleMouseUp(): void {
    if (!this._active) return;
    const sel = this.contentEl.ownerDocument.getSelection();
    if (sel && !sel.isCollapsed) this.paste();
  }
  ...
  private collectStyles(node: Node): void {
      ...
      if (cur.nodeType === Node.ELEMENT_NODE) {
        ...
        for (const prop of INLINE_PROPS) {
          if (!this.savedStyles.has(prop)) {
            ...
            if (val) this.savedStyles.set(prop, val);
          }
        }
      }
      cur = cur.parentNode;
  ...
  private flattenInlineElements(node: Node): void {
