import {
  ...
  DATE_FORMAT_OPTIONS,
} from '../modules/insert/DocumentFields.js';
import type { FieldDefinition, FieldCategory } from '../modules/insert/DocumentFields.js';
...
export class FieldManagerPage {
  ...
  private categories: FieldCategory[] = [];
  private activeCatId: string = '';
  private selectedFieldId: string | null = null;
  private editingField: FieldDefinition | null = null;
  private isNew = false;
  ...
  private fieldToolbarTitle!: HTMLElement;
  private fieldCount!: HTMLElement;
  private editPanel!: HTMLElement;
  private epTitle!: HTMLElement;
  private epBody!: HTMLElement;
...
<div class="fm-cat-item${isActive ? ' active' : ''}" data-cat="${cat.id}">
  ...
  <span class="fm-cat-badge">${count}</span>
    ...
    this.catList.querySelectorAll<HTMLElement>('.fm-cat-item').forEach(el => {
      ...
      el.addEventListener('contextmenu', (e) => {
        e.preventDefault();
        this._showCatContextMenu(e as MouseEvent, el.dataset.cat!);
      });
    });
  }
...
<div class="fm-empty">
  ...
  <small>상단의 "필드 추가" 버튼을 눌러 첫 번째 필드를 추가하세요.</small>
    ...
    this.fieldList.querySelectorAll<HTMLElement>('.fm-field-row').forEach(row => {
      const fid = row.dataset.fid!;

      row.addEventListener('click', (e) => {
        if ((e.target as HTMLElement).closest('.fm-field-actions')) return;
        this.selectedFieldId = fid;
        this._openEditField(fid);
      });

      // Edit/Delete action buttons
      row.querySelector<HTMLElement>('[data-edit]')?.addEventListener('click', (e) => {
        e.stopPropagation();
        this._openEditField(fid);
      });
      row.querySelector<HTMLElement>('[data-del]')?.addEventListener('click', (e) => {
        e.stopPropagation();
  ...
  private _fieldRowHtml(f: FieldDefinition): string {
...
<div class="fm-field-row" data-fid="${f.id}">
  ...
  private _openEditField(fid: string): void {
  ...
  private _renderEditForm(): void {
    ...
    const typeOptions = (['text', 'number', 'date'] as const)
...
${formatSection}`;
    ...
    this.epBody.querySelector<HTMLSelectElement>('#ep-type')?.addEventListener('change', (e) => {
      if (!this.editingField) return;
      this.editingField.type = (e.target as HTMLSelectElement).value as 'text' | 'number' | 'date';
  ...
  private _syncEditFormToState(): void {
    if (!this.editingField) return;
    const get = (id: string) => (this.epBody.querySelector<HTMLInputElement | HTMLSelectElement>(`#${id}`)?.value ?? '').trim();
    if (!this.editingField.builtIn) this.editingField.id = get('ep-id');
    ...
    this.editingField.type        = get('ep-type') as 'text' | 'number' | 'date';
    const fmt = this.epBody.querySelector<HTMLSelectElement>('#ep-fmt');
    if (fmt) this.editingField.defaultNumberFormat = fmt.value;
  }
  ...
  private _applyEdit(): void {
    ...
    if (!f.label.trim()) { this._toast('표시 이름을 입력하세요.', 'error'); return; }
    if (!f.id.trim()) { this._toast('ID를 입력하세요.', 'error'); return; }
    if (!/^[a-z0-9_]+$/.test(f.id)) { this._toast('ID는 영문 소문자, 숫자, 밑줄만 허용됩니다.', 'error'); return; }

    if (this.isNew) {
      if (this.fields.some(x => x.id === f.id)) { this._toast(`ID "${f.id}" 이(가) 이미 존재합니다.`, 'error'); return; }
      this.fields.push({ ...f });
    } else {
      const idx = this.fields.findIndex(x => x.id === f.id);
      if (idx !== -1) this.fields[idx] = { ...f };
    }
  ...
  private _deleteField(fid: string): void {
    const field = this.fields.find(f => f.id === fid);
    if (!field) return;
    if (field.builtIn) { this._toast('기본 제공 필드는 삭제할 수 없습니다.', 'error'); return; }
    if (!confirm(`"${field.label}" 필드를 삭제하시겠습니까?`)) return;
    this.fields = this.fields.filter(f => f.id !== fid);
    if (this.selectedFieldId === fid) this.selectedFieldId = null;
    this._renderCategories();
    this._renderFields();
  ...
  private _reorderField(srcId: string, targetId: string): void {
    const catFields = this.fields
      .filter(f => f.category === this.activeCatId)
      .sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
    ...
    catFields.forEach((f, i) => {
      ...
      if (field) field.order = i * 10 + (this.categories.findIndex(c => c.id === this.activeCatId)) * 1000;
    });
  ...
  private _showCatContextMenu(e: MouseEvent, catId: string): void {
...
${cat.builtIn ? '' : `<hr class="ctx-menu-sep"><div class="ctx-menu-item danger" data-action="delete">카테고리 삭제</div>`}`;
    ...
    menu.addEventListener('click', (ev) => {
      ...
      menu.remove();
      if (action === 'rename') this._renameCategory(catId);
      if (action === 'delete') this._deleteCategory(catId);
    });
  }

  private _renameCategory(catId: string): void {
    const cat = this.categories.find(c => c.id === catId);
    if (!cat) return;
  ...
  private _deleteCategory(catId: string): void {
    const cat = this.categories.find(c => c.id === catId);
    if (!cat) return;
    if (cat.builtIn) { this._toast('기본 제공 카테고리는 삭제할 수 없습니다.', 'error'); return; }
    const fieldCount = this.fields.filter(f => f.category === catId).length;
    if (fieldCount > 0) {
      if (!confirm(`"${cat.label}" 카테고리에 ${fieldCount}개의 필드가 있습니다.\n카테고리를 삭제하면 필드도 함께 삭제됩니다. 계속하시겠습니까?`)) return;
  ...
  private _openAddCategoryModal(): void {
...
<div class="modal-box" id="add-cat-box">
  ...
  <div class="modal-actions">
    ...
    backdrop.querySelector<HTMLInputElement>('#new-cat-id')!.focus();

    const close = () => backdrop.remove();
    backdrop.addEventListener('mousedown', (e) => { if (e.target === backdrop) close(); });
    backdrop.querySelector<HTMLElement>('#cat-modal-cancel')?.addEventListener('click', close);
    backdrop.querySelector<HTMLElement>('#cat-modal-ok')?.addEventListener('click', () => {
      const id    = (backdrop.querySelector<HTMLInputElement>('#new-cat-id')!.value).trim();
      const label = (backdrop.querySelector<HTMLInputElement>('#new-cat-label')!.value).trim();
      if (!id) { this._toast('카테고리 ID를 입력하세요.', 'error'); return; }
      if (!/^[a-z0-9_]+$/.test(id)) { this._toast('ID는 영문 소문자, 숫자, 밑줄만 허용됩니다.', 'error'); return; }
      if (!label) { this._toast('표시 이름을 입력하세요.', 'error'); return; }
      if (this.categories.some(c => c.id === id)) { this._toast(`ID "${id}" 이(가) 이미 존재합니다.`, 'error'); return; }
  ...
  private _resetToDefaults(): void {
    if (!confirm('모든 사용자 정의 필드와 카테고리를 삭제하고 기본값으로 복원하시겠습니까?')) return;
