...
import {
  ...
  TableFormulaManager,
  type Formula,
} from '../../src/modules/table/TableFormula.js';

// ── 공통 헬퍼 ─────────────────────────────────────────────────────────────────
// vitest는 environment: 'jsdom' 으로 실행되므로 global document/MutationObserver
// 를 직접 사용한다. 별도 JSDOM 인스턴스 생성 불필요.

interface TableEnv {
  table: HTMLTableElement;
  manager: TableFormulaManager;
  container: HTMLDivElement;
}

function makeTable(data: (number | string)[][]): TableEnv {
...
function cleanup(env: TableEnv): void {
...
describe('applyFormula() — SUM', () => {
  ...
  it('결과 셀에 data-formula 속성 저장', () => {
    ...
    const stored: Formula = JSON.parse(cell.dataset.formula!);
