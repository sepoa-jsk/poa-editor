...
describe('checkImageAlt', () => {
  it('alt 속성이 없는 img → error', () => {
    ...
    const result = new AccessibilityChecker(root as unknown as HTMLElement).run();
  ...
  it('autoFix 실행 시 alt 속성이 추가됨', () => {
    ...
    issue!.autoFix!();
    expect((root.querySelector('img') as HTMLImageElement).alt).toBe('이미지');
  });
});
...
describe('checkTable', () => {
  ...
  it('caption + summary + scope 모두 있으면 표 관련 이슈 없음', () => {
    const root = makeRoot(
      '<table summary="요약"><caption>표</caption><tr><th scope="col">헤더</th></tr><tr><td>데이터</td></tr></table>'
    );
    const result = new AccessibilityChecker(root as unknown as HTMLElement).run();
