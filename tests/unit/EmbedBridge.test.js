...
describe('EmbedBridge', () => {
    ...
    beforeEach(() => {
        ...
        getHTML = vi.fn().mockReturnValue('<p>hello</p>');
        setHTML = vi.fn();
    ...
    it('attach: POA_READY 메시지를 setTimeout 후 parent 로 발송', () => {
        ...
        bridge.attach();
        vi.runAllTimers();
        expect(spy).toHaveBeenCalledWith(expect.objectContaining({ type: 'POA_READY', version: expect.any(String) }), '*');
    });
    ...
    it('POA_GET_CONTENT: source 에 POA_CONTENT 응답 발송', () => {
        ...
        expect(sourceMock.postMessage).toHaveBeenCalledWith({ type: 'POA_CONTENT', html: '<p>hello</p>' }, '*');
    });
    it('POA_SET_CONTENT: DOMPurify 정제 후 setHTML 호출', () => {
        ...
        const arg = setHTML.mock.calls[0][0];
