...
import { buildFormControlHtml, generateControlId, FormControlInserter, } from '../../src/modules/form/FormControlInserter.js';
// ── generateControlId ──────────────────────────────────────────────────────
...
describe('FormControlInserter', () => {
    ...
    it('삽입된 요소에 data-poa-form JSON이 저장된다', () => {
        const ins = new FormControlInserter(container);
        const cfg = { type: 'text', label: '테스트', name: 'test' };
        ins.insert(cfg);
        const group = container.querySelector('.poa-form-group');
