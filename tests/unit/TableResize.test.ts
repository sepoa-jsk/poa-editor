...
function buildTable(doc: Document, rows = 3, cols = 3): HTMLTableElement {
...
describe('열 너비 드래그 조정', () => {
  ...
  it('열 너비 드래그 후 td style.width 가 변경된다', () => {
    ...
    const col1 = colgroup?.children[0] as HTMLElement | undefined;

    // startColResize 직접 호출 (private 우회)
    const navAny = nav as unknown as Record<string, unknown>;
    (navAny['startColResize'] as (c: HTMLTableCellElement, t: HTMLTableElement, x: number) => void)(firstCell, table, 0);
  ...
  it('열 너비 최솟값 30px 미만으로 줄어들지 않는다', () => {
    ...
    const colgroup = table.querySelector('colgroup');
    const col2 = colgroup?.children[1] as HTMLElement | undefined;
    if (col2) {
...
describe('행 높이 드래그 조정', () => {
  ...
  it('행 높이 드래그 후 tr style.height 가 변경된다', () => {
    ...
    const firstCell = firstRow.querySelector<HTMLTableCellElement>('td,th')!;
    ...
    const navAny = nav as unknown as Record<string, unknown>;
    (navAny['startRowResize'] as (c: HTMLTableCellElement, t: HTMLTableElement, y: number) => void)(firstCell, table, 0);
...
describe('셀 세로 정렬', () => {
  ...
  it('세로 정렬 middle 적용 시 td style.verticalAlign 이 middle 이다', () => {
    ...
    const navAny = nav as unknown as Record<string, unknown>;
    (navAny['applyCellVerticalAlign'] as (c: HTMLTableCellElement, t: HTMLTableElement, a: string) => void)(cell, table, 'middle');
  ...
  it('세로 정렬 top 적용 시 td style.verticalAlign 이 top 이다', () => {
    const cell = table.querySelector<HTMLTableCellElement>('td')!;
    const navAny = nav as unknown as Record<string, unknown>;
    (navAny['applyCellVerticalAlign'] as (c: HTMLTableCellElement, t: HTMLTableElement, a: string) => void)(cell, table, 'top');
