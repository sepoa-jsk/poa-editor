...
import type { Signature, SignatureFields } from '../../src/modules/signature/SignatureManager.js';
...
function fresh(): SignatureManager {
  localStorage.clear();
  return new SignatureManager();
}

function sampleData(): Omit<Signature, 'id' | 'createdAt' | 'updatedAt'> {
...
describe('SignatureManager — max count', () => {
  it('throws when adding more than 10 signatures', () => {
    const mgr = fresh();
    for (let i = 0; i < 10; i++) mgr.add({ ...sampleData(), name: `서명 ${i}` });
