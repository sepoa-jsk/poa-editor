...
function makeTableWithRowspan() {
    const table = document.createElement('table');
    const tbody = document.createElement('tbody');
    const tr0 = document.createElement('tr');
    const spanned = document.createElement('td');
    spanned.rowSpan = 2;
...
function track(t) { cleanup.push(t); return t; }
// ───────────────────── buildGridMap ──────────────────────────────────
...
describe('CellMerger.mergeCells', () => {
    ...
    it('병합 시 비어있지 않은 내용을 첫 번째 셀에 합친다', () => {
        ...
        table.rows[0].cells[0].textContent = '왼쪽';
        table.rows[0].cells[1].textContent = '오른쪽';
        CellMerger.mergeCells([table.rows[0].cells[0], table.rows[0].cells[1]], table);
    ...
    it('셀 수가 1개이면 success:false 를 반환한다', () => {
        ...
        const result = CellMerger.mergeCells([table.rows[0].cells[0]], table);
        expect(result.success).toBe(false);
    });
});
// ───────────────────── splitCellHorizontal ───────────────────────────
describe('CellMerger.splitCellHorizontal', () => {
    ...
    it('colspan=1 셀에서 splitCellHorizontal 은 아무것도 하지 않는다', () => {
        const table = track(makeTable(2, 2));
        CellMerger.splitCellHorizontal(table.rows[0].cells[0], table);
...
describe('CellMerger.splitCellVertical', () => {
    it('rowspan=2 셀 분할 시 두 번째 행에 새 셀이 추가된다', () => {
        const table = track(makeTableWithRowspan());
        const spanned = table.rows[0].cells[0];
    ...
    it('rowspan=1 셀에서 splitCellVertical 은 아무것도 하지 않는다', () => {
        const table = track(makeTable(2, 2));
        CellMerger.splitCellVertical(table.rows[0].cells[0], table);
        expect(table.rows[0].cells).toHaveLength(2);
    });
});
// ───────────────────── applyCellProperties ───────────────────────────
...
describe('CellMerger.splitCell', () => {
    ...
    it('열 2 + 행 1: 3×3 표의 가운데 셀을 좌우 2개로 분할', () => {
        const table = track(makeTable(3, 3));
        const cell = table.rows[1].cells[1]; // r1c1
        ...
        expect(table.rows[2].cells[1].colSpan).toBe(2);
    });
    it('열 1 + 행 2: 3×3 표의 가운데 셀을 위아래 2개로 분할', () => {
        ...
        expect(table.rows[1].cells[2].rowSpan).toBe(2);
