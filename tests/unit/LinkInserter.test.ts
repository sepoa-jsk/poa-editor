...
describe('LinkInserter', () => {
  ...
  function setRange(container: Node, offset = 0): void {
  ...
  it('유효하지 않은 URL이면 에러를 던진다', () => {
    expect(() =>
      inserter.insertLink({ href: 'not-a-url', text: '링크' }),
    ).toThrow('유효하지 않은 URL');
  });
...
describe('BookmarkManager', () => {
  ...
  it('insert는 bm- 접두사를 가진 ID를 반환하고 DOM에 삽입한다', () => {
    ...
    const anchor = root.querySelector<HTMLAnchorElement>(`a#${id}`);
    expect(anchor).not.toBeNull();
  ...
  it('getAll은 삽입된 책갈피 목록을 반환한다', () => {
    const setRangeAt = (offset: number): void => {
      const range = document.createRange();
