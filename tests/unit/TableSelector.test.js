 ...
 */
function dragOnly(from, to) {
    const doc = document;
    const orig = doc['elementFromPoint'];
    doc['elementFromPoint'] = () => to;
    try {
        from.dispatchEvent(new MouseEvent('mousedown', { bubbles: true, button: 0, buttons: 1 }));
        document.dispatchEvent(new MouseEvent('mousemove', { bubbles: true, buttons: 1, clientX: 100, clientY: 100 }));
        document.dispatchEvent(new MouseEvent('mouseup', { bubbles: true }));
    }
    finally {
